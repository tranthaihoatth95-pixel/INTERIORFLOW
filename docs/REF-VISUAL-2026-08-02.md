...
## Bổ sung 12/08 — 5 ref Siri mới nhất + gradient kem (Hoà giao, chuẩn cho Vitals 3 cấp window)

| # | Ref | LẤY | Cho IF | TRÁNH |
|---|---|---|---|---|
| 14 | **Siri/iOS liquid glass mới nhất** (nút kính capsule · Reduce Bright Effects ON/OFF · thẻ kết quả nhỏ "Show Results" · trang hội thoại tối "Ask Siri" + mic/＋ · sheet "An All-New Siri") | **3 CẤP WINDOW Vitals**: ① PILL/THẺ KẾT QUẢ — viên kính nhỏ trả lời tại chỗ trên canvas, 1-2 câu + nút "Xem thêm" ② THẺ HỘI THOẠI NỔI — sheet kính giữa màn như "An All-New Siri", vài lượt hỏi đáp, ô nhập + mic + ＋ ③ TRANG PHIÊN ĐẦY ĐỦ — nền tối/be đơn giản như trang Ask Siri, lưới phiên cũ. Kèm bài học "Reduce Bright Effects": kính PHẢI có nấc giảm chói/độ đọc thắng (khớp iOS 27 tự sửa Liquid Glass đã ghi SPEC-APPLE-MOTION) | Vitals LM (thay khuôn §4b bằng chuẩn thị giác này) · glyph giữ cầu+quỹ đạo 1 accent | ❌ orb gradient (đã cấm) · ❌ chatbot toàn màn mặc định — cấp ③ chỉ khi người dùng chủ động vào |
| 15 | **Tanj card hover** (dãy card kem, card đang trỏ nổi gradient kem-cam ấm) | **HIGHLIGHT GRADIENT KEM khi hover/select**: nội dung chọn được trên canvas tone be/xám-đen đơn giản → trỏ vào là nền card chuyển gradient kem ấm mềm (radial, mờ dần), rời chuột trả lại phẳng. Chỉ áp cho PHẦN TỬ CHỌN ĐƯỢC (thẻ, card, item kệ) — đây là ánh sáng CÓ NGHĨA (khả-tương-tác), không trang trí | thẻ template · card kệ Thư viện · trang canvas Trình chiếu/moodboard | không áp lên chrome/toolbar (luật SPEC-HOVER: nút chỉ đổi nền) · reduce-motion: hiện tức thì không fade |
