...
[17/08 — WT-CHOT ĐÍNH CHÍNH T ở tiền đề phiếu, lỗi thứ 14 trong ngày] T viết *"interiorflow-wt-* chưa dùng lần nào"*. **SAI** — đo tại nguồn: grep `docs/**` = **57 dòng nhật ký nhắc pattern đó** (chủ yếu tháng 07 và đầu 08, chốt worktree song song đã dùng thật). Nhưng đó là **nhật ký đã đóng** — WT-CHOT đúng khi chỉ gỡ khỏi *luật sống* (CLAUDE.md, launch.json, gitignore) và **không đụng docs lịch sử** theo `IF-KIEN-TRUC` §4 (mục "nhật ký viết lại là viết lại lịch sử"). ⚠️ Bài học ngay: **T grep đường sống ≠ grep toàn repo** — trước khi khẳng định "chưa dùng lần nào" phải `grep -rn ... docs/` chứ không chỉ đọc `CLAUDE.md` hiện tại. Cùng họ lỗi 17/08 khác (đếm 9 tệp thay 106; số chép lại không phải phép đo).
· ⭐ CHỖ WT-CHOT ĐÚNG THÊM (khai chi tiết trong báo cáo): 5 máy soi khác (`soi-frontier`/`tu-dien`/`contract`/`thao-tac`/`hinh-hoc`) **walk từ scan-dirs khai sẵn**, KHÔNG chạm `.claude/worktrees/` — chỉ 2 máy walk-từ-ROOT (`soi-that` + `check-chot`) mới dính. Vá thêm cho 5 máy còn lại là **thêm nhiễu cho ca không xảy ra**, phạm luật over-engineering. Agent đúng khi từ chối vá — dù phiếu tôi để ngỏ "nếu cùng bug". ⇒ Van an toàn: phiếu để ngỏ ≠ giấy phép làm, agent phải kiểm cùng bug có thật không trước khi vá.

[17/08 tối — Hoà đưa mock Files hai TẦNG, T tư vấn KHÔNG tách route] Bố cục Hoà đưa (ảnh dán vào chat): tầng ① thư mục hệ thống 5 loại có quyền (Dự án · Studio · NCC · Đã duyệt · Lưu trữ) + tầng ② **Collection+** 8 gói component (Vật liệu · Furniture · Chi tiết điển hình · Cây·người · Design DNA · Gói học từ dự án · Mẫu trình bày · Cách làm, mã `COL-<LOẠI>-NNN`). ⇒ **Bản "hai NGĂN dự án ↔ phần thô"** (chốt sáng 17/08) **hết hiệu lực**; V2 đã dựng chiều nay (`HaiNgan.tsx` · `NganPhanTho.tsx`) sẽ dựng lại thành "hai TẦNG"; logic "phần thô" KHÔNG mất — gộp vào thư mục "Nhà cung cấp" ở tầng ①.
· ⭐ **T tư vấn: KHÔNG tách Collection+ thành route riêng** (Hoà uỷ quyền *"bạn soi toàn app, check đồng bộ rồi tư vấn"*). Ba lý do đồng bộ: ① bản đồ §5 dòng chảy VẬT không đổi — Collection+ là **cách TỔ CHỨC của Files**, không mắt xích thứ ba ② rail cụm XƯỞNG đã 5 mục cân đối ③ kệ Thư viện chia THEO CHẶNG (`lib/library/shelves.ts:49-63`: cad-*, render-*, present-*), Collection+ chia THEO LOẠI VẬT — **khác trục hoàn toàn**, không cạnh tranh.
· ⭐⭐ **Collection+ mở TẦNG THỨ HAI của DistillEngine**. Trước nay `distillDnaFromSources` chỉ chạy cấp DỰ ÁN (Thẻ DNA · Grounded Render · Cửa sổ Thảo Luận). Nay có cấp STUDIO — chưng cả gói áp cho nhiều dự án. Đúng cụm đẳng cấu §9 (một cỗ máy, nhiều mặt tiền): mặt tiền thứ 6 của cỗ máy chưng cất.
· Đã cập nhật: `HOP-DONG-CAU-TRUC-DIEU-HUONG.md §3` · `IF-KIEN-TRUC.md §5` · `docs/hoa-noi/SO-TONG.md` (nạp lần đầu vào kho Hoà nói).

[19/08 Hoà chốt — RECONCILIATION GATE + 4 DECISION CONFLICT + BLUEPRINT v1.0] Sau FINAL-AUDIT đóng khám: chạy Reconciliation 47 nhóm, gate cứng Hoà đặt "MAJOR MISSING = 0 mới được sinh Blueprint" — ĐẠT (52 concept: 0 MISSING · 1 AMBIGUOUS duy nhất là NODE unit-of-work, giữ [UNKNOWN] cấm bịa). Hoà chốt 4 câu qua trắc nghiệm: ① **Gateway** = giữ code, chốt từ điển ("Gateway" trần = AI Gateway · `lib/gateway/` = Format Router — đóng M-01) ② **overrides THẮNG variant** (effective = template → variant → overrides — đóng M-03) ③ **Workspace hai-chốt-là-một**: Workspace = môi trường compose giữ context; danh mục CẤP 0.5 (11/08) = các workspace INSTANCE chuẩn ④ **Canvas = LAI, định nghĩa Hoà sửa**: KHÔNG phải "1 canvas sản xuất + vài canvas phụ" — canonical model là **Project → nhiều Workspace → mỗi Workspace nhiều Canvas/Board + MỘT Project Flow/Timeline xuyên suốt** (graph nối [DESIGN DIRECTION]); Canvas = working surface · Workspace = working context · Project = identity+truth+genealogy; Canvas không độc lập dữ liệu — tham chiếu Project truth/DNA/decisions, không copy thành thế giới mới. Khuyến nghị một-canvas của T 16/08 SUPERSEDED hẳn. Sản phẩm: `docs/IF-ARCHITECTURE-BLUEPRINT.md` v1.0 (543 dòng, B1-B24, 11 mermaid, YAML, 6 SUPERSEDED cấm hồi sinh, commit mốc 3da4b8c) — vai: kiến trúc GHÉP thế nào; MAP = living direction; ADR thắng cả hai; ⚠️ ĐỪNG NHẦM với `IF-ARCHITECTURE-BLUEPRINT-v1.md` (file cũ khác hẳn — 8 luật vận hành). Chống mồ côi 4 dây: registry entry `blueprint-canonical` + con trỏ CLAUDE.md/MAP trỏ chéo + LATEST + memory sessions. Sau blueprint: Wave 1 chỉ là thi công theo bản vẽ, không tranh luận lại vocabulary/tầng kiến trúc.

[19/08 Hoà ban LUẬT — HEATMAP + NO-REBUILD, áp cho MỌI đề xuất] Vận-hành-hoá [Đ2] nhìn-vào-trong + "một cỗ máy nhiều mặt tiền" thành thang bậc máy-kiểm: **LOOK INSIDE → MAP EXISTING → CLASSIFY → CONNECT → EXTEND → NEW**. NEW đòi **negative evidence đủ 6 mục** (đã tìm · primitive gần nhất · vì sao REUSE/CONNECT/EXTEND không đủ · không tạo island) — thiếu là CẤM NEW. Vùng DÀY (2D · DS · deterministic · Material · Present · AI primitives · persistence · IDF*) default REUSE/CONNECT/TUNE — nhảy thẳng NEW = RED FLAG. Vùng MỎNG (Manifest · Workspace semantic · revision DAG · Decision · DNA xuyên workflow · Policy · AI Gateway · conflict/merge · ArchiNote contracts · Visual Pipeline) default EXTEND NEAREST CONTRACT — không tạo island. Heatmap CHỈ là định hướng, cấm nhớ hộ code — đo lại tại nguồn. CURRENT ≠ TARGET: ghi CURRENT/TARGET/GAP/TRANSITION (additive → bridge → migrate → verify → deprecate → remove later). Mọi nhóm đề xuất lớn phải có bảng | Need | Existing Primitive | Evidence file:line | Coverage | Action | Why |. Cấm lý do "làm mới cho sạch"/"viết lại cho dễ"/"framework mới đẹp hơn". Bản thi hành: `IF-ARCHITECTURE-BLUEPRINT.md` §B25 · nguyên văn: `docs/memory/sessions/2026-08-19/09-blueprint-canonical/ADDENDUM-NO-REBUILD.md` (addendum chỉ có trong chat, đã cứu vào repo).

[20/08 Hoà chốt — IF EXPERIENCE SYSTEM, bản tổng hợp 12 điều] "Coi như TRẠNG THÁI CHỐT hiện tại." Toàn văn + đối chiếu board/code: docs/CHOT-EXPERIENCE-SYSTEM-2026-08-20.md. Lõi: một shell 5 vùng trách nhiệm · sidebar BA CỤM 3 độ sâu (rail 52-56/shelf 220-280/panel 320-440) · spotlight primary+secondary theo stage · Home=Personal Work OS hero=Resume · Vitals Aperture TOP-EDGE 3 mức (signature) · voice contextual có human-confirm · right edge=Context Intelligence Stack (5 lens + Measured/Verified/Inferred/External/Stale + Go-to-Source/Where-Used/Blast-Radius) · Master Capability System (3 stage=3 Authoring Environments, capability 4 độ sâu, toolbar=working set 4-8) · non-destructive Library semantics (Definition→Representation→Instance→Override→Update→Where-Used, anchor first-class, Replace giữ vị trí). 🔴 ĐÈ 2 chốt cũ: sidebar HAI CỤM 16-17/08 → BA CỤM · Vitals neo-theo-ngữ-cảnh 16/08 → Aperture top-edge (MAP §2.2 + Blueprint B6 cần cập nhật — đã báo MAIN). Bản vẽ: 9 board EXS-A..I nhóm "Experience System" Claude Design + docs/nc/REF-VISUAL-EXS-2026-08-20.md (R1-R16, gồm chốt giữa lượt "cubic nét, sang, đơn giản cho stage Vẽ 3D" + collage moodboard là tuỳ chọn bố cục đặc trưng). HARD STOP giữ: design pass, 0 production, chờ eye-approve.
[20/08 Hoà ĐÍNH CHÍNH SCOPE — AUTO GRID] STOP bản generic: Auto Grid KHÔNG phải experience pattern 5 màn (Home/Gallery/Review/Files) — nó là MASTER CAPABILITY compose nội dung BÊN TRONG STAGE PRESENT, chạy ngay trên Present canvas (chọn 2+ block → Auto Grid → bố cục HÌNH THÀNH tại chỗ → cycle phương án → Compare/Apply/Undo → block editable lại bình thường). Layout Ghost/"Khung Mọc" = feedback/motion CỦA capability này, không phải loading skeleton toàn app. Semantic composition theo LOẠI block (hero render/plan giữ tỉ lệ/swatch strip/caption dính content/BOQ vùng dữ liệu); user giữ quyền (pin·hero·exclude·lock·regenerate vùng·không đè vùng đã custom). Board EXS-J viết lại theo P0-P7 + 6 phương án cùng content; board EXS-K (5 ca toàn app) ĐÃ XOÁ — sai scope, cấm trích lại. Pattern phụ "Layout Ghost khi AI sinh bố cục" trong CHOT-EXPERIENCE-SYSTEM đọc HẸP lại theo đính chính này.
[20/08 Hoà ban LUẬT CỨNG — DESIGN SYSTEM GEOMETRY, 14 mục] MỘT ngôn ngữ hình học xuyên app: RECTANGLE→ROUNDED RECT (chủ đạo)→CAPSULE→CIRCLE; học kỷ luật hình học Apple KHÔNG copy skin; hard test = che logo đặt 9 màn cạnh nhau vẫn nhận ra cùng một hệ. ĐỐI CHIẾU [Đ2] — ĐÃ CÓ MÁY GIỮ: thang bo 6/10/14/20 + --r-full (duyệt 12/08, hinh-hoc-ap-thang) · bo đồng tâm rInner=rOuter−pad (§2d, ToolbarBar tự đúng qua RADIUS.full) · soi:hinh-hoc · KB-1 capsule · depth ladder EXS-A · LightState. PHẦN MỚI THÀNH LUẬT: ① "FROM THE CENTER" — mọi surface mở ra TỪ TÂM/NGUỒN của chính nó (button→capsule nở · aperture→mở từ aperture · card→inspector từ vị trí card · Auto Grid cell hình thành từ composition frame, không teleport) ② CAPSULE CÓ VIỆC (compact action·mode·status·tool group·Vitals·segmented — không biến card thành pill) + CIRCLE CÓ VIỆC (avatar·icon action·ring·anchor·semantic dot — không trang trí) ③ MORPH GIỮ IDENTITY (rail icon→label→shelf · aperture→peek→engage · icon→capsule→ToolWindow · asset card→inspect→deep: cùng một vật nở, không hard-cut) ④ MỘT HỆ, BỐN MẬT ĐỘ (Home roomy · Files/Library balanced · 2D/3D compact · Present editorial — khác density KHÔNG khác DS, đi qua token --tap/--row/--gap sẵn có) ⑤ ICON CONTAINER thống nhất (active = rounded-square/circle/capsule THEO RULE, cấm mỗi toolbar tự chế nền icon) ⑥ tuyến tính: align theo trục, snap grid, panel mở theo hướng logic — trừ canvas authoring user tự đặt. Board: EXS-L. Nối entry hinh-hoc-ap-thang + máy soi soi:hinh-hoc (bổ tiêu chí from-center/morph khi thi công).
[20/08 Hoà REVIEW MẮT: PASS — bộ Experience System] Bộ board EXS trên Claude Design (EXS-A luật vật lý · B sáu khung · C Home Work OS · D sidebar 3 độ sâu · E context stack+aperture · F flows nghề · G+H+I ref-áp-DS · J Auto Grid Present · L ngữ pháp hình học) + CHOT-EXPERIENCE-SYSTEM + luật hình học 14 mục: 👁 QUA MẮT 20/08. DS KHÔNG vẽ mới — phần mới đã NỐI vào SPEC-DESIGN-SYSTEM-IF.md §7 (bổ chính §2/§2c/§2d/§3/§4/§5, số bo cũ 6/9/12/16 đọc theo token 6/10/14/20). Thi công vẫn theo phiếu/wave — pass mắt mở cổng design→build, không phải lệnh code ngay.
[20/08 Hoà chốt — IF MOTION & VISUAL LAW, toàn văn docs/IF-MOTION-VISUAL-LAW.md] Bộ luật vận động + cá tính thị giác toàn IF (0→IX): nhịp ms mặc định (hover 100-160 · reveal 140-200 · shelf 180-260 · stage 240-380 · morph 300-700, ĐÈ dải SPEC-APPLE-MOTION khi vênh — đã đóng dấu file cũ) · 3 stage chính có character riêng (2D precise-flat "đúng từng mm" · 3D spatial-tactile "chạm không gian", 2D→3D object MỌC từ footprint · Present editorial-cinematic "kể chuyện", Auto Grid hình thành không pop) · surface phụ (Home roomy-alive · Files visual-first source-aware · Review anchored decision>conversation · Vitals aperture CẮT vào shell biết im · Project identity+continuity · Profile quiet) · spotlight 1+1 mọi surface · depth L0-L4 càng cao càng tạm · master capability đổi representation KHÔNG đổi identity (Replace 2D=3D) · UI-sống theo TẦN SUẤT ("alive ≠ animated everywhere") · chuyển stage = đổi chế độ không mở app khác · §IX đoạn luật EN dán nguyên cho Claude Design khi dựng màn. Nối: SPEC-DESIGN-SYSTEM-IF §7 (bổ sung cùng ngày) · board EXS-M. DRIFT khai: token --dur-* mới theo vai trò khi thi công.
[20/08 chiều Hoà BÁC + đổi hướng — LIBRARY] ① "IDFC Real Asset Quality Pass" BÁC làm hình ảnh sản phẩm (đồ vẽ tay/ma trận mẫu/minh hoạ CAD ≠ trải nghiệm cao cấp); board EXS-N/O/P hạ thành THAM CHIẾU KỸ THUẬT NỘI BỘ (line-grammar·anchor·LOD·Place/Replace/Override vẫn có giá), CẤM thi công visual theo chúng. ② Hướng mới: Library = TRẢI NGHIỆM quanh NỘI DUNG THẬT ba tầng BROWSE (editorial, ảnh thật to) → OBJECT PASSPORT → TECHNICAL VERIFY (ma trận kỹ thuật CHỈ tầng 3); nội dung cỡ hãng thật hoặc ghi rõ PLACEHOLDER — cấm thương hiệu giả, cấm vẽ lại sản phẩm. ③ LUẬT KHUNG GIỜ thị giác: checkpoint nhìn được ~20', Hoà trả lời CÓ/KHÔNG/SỬA ~30', không đạt = khai STUCK đổi cách. ④ Việc thị giác chưa chốt CHỈ qua Claude Design. ⑤ Việc lớn kế (phiên UX/UI mới): FULL APP EXPERIENCE ATLAS — 1 artifact, 7 nhóm, mỗi khung dán nhãn trạng thái, không giấu màn xấu. Provenance bộ EXS đã khai thật trong BAN-GIAO-UXUI-EXS-2026-08-20.md (HTML tự tay + DesignSync đẩy, không native canvas, không subagent) — cách hiểu luật ④ là gate chờ Hoà.

[04/09 Hoà chốt — ĐÓNG HAI XUNG ĐỘT AUTHORITY D-DR1 + D-DR2] Hai mục treo từ đợt khám kiến trúc nay có quyết định, **không bàn lại**.

· **D-DR1 · CHỖ ĐỨNG VẬT LÝ CỦA VITALS — GIỮ EXS §7.** Vitals nằm **VẬT LÝ trong TOP EDGE như một khẩu độ sống**, 3 mức Ambient→Peek→Engage. Chỗ đứng **cạnh trục phải** (`VitalsRightEdgeHost` / `VitalsGesture` bản cũ) = **SUPERSEDED**, không lật được EXS. Nhưng **tách HÀNH VI khỏi CHỖ ĐỨNG**: hành vi hữu ích của Slice 12 mà không đụng EXS thì **ADAPT vào khẩu độ mép trên**, cấm xoá mù. Sau di trú phải còn **ĐÚNG MỘT chỗ đứng vật lý**.
  🔴 **ĐO 04/09 — đây không phải chuyện thẩm mỹ, có LỖI CHỨC NĂNG ĐANG SỐNG**: `VitalsGesturePanel` chỉ mount ở `StageSwitcher.tsx:446`, mà `StageSwitcher` **không còn được mount ở đâu** từ 17/08 (sidebar thành router). `StatusBar.tsx:101` vẫn gọi `useVitalsUi.open()` nhưng cố ý không mount panel; `⌘J` đăng ký ở `StageSwitcher:223` (chết) và `VitalsRightEdgeHost:49` (**chưa từng mount**, grep nơi dùng = 0). ⇒ **gõ câu hỏi ở thanh trạng thái rồi Enter là MẤT CÂU HỎI, ⌘J không làm gì.** Khẩu độ mép trên chính là chỗ nhận lại trọn hành vi mồ côi đó.

· **D-DR2 · BỐ CỤC HOME — GIỮ EXS §6.** Home khổ **rộng/desktop**: **ĐÚNG MỘT TIÊU ĐIỂM CHÍNH** (thẻ Resume / việc đang dở) rồi tới **một cụm phụ hạng dưới hẳn**. Home **không được đọc ra như dashboard SaaS**, **không phải lưới bento 9 thẻ**. Bản bento **không tự động bị xoá** — được ở lại làm bố cục **khổ hẹp / thích ứng**, nhưng **không định nghĩa thứ bậc Home ở desktop**. ⚠️ Hoà nói rõ: **đừng giữ bento chỉ vì mã đã có sẵn** — có sẵn không phải là lý do.

· **ĐỔI TRỌNG TÂM THI CÔNG kèm theo**: hai quyết định đóng ⇒ **ngừng dọn kỹ thuật chỉ vì việc đó sẵn có**. Nút cổ chai nay là **76 xong-máy / 1 qua mắt**. Chuyển sang **HOÀN TẤT THỊ GIÁC** theo `docs/delivery/VISUAL-APPROVAL-QUEUE.md`, đường đi cố định: AUTHORITY → THI CÔNG → MÁY KIỂM → ẢNH CHUẨN → DELTA → MẮT HOÀ. **Cấm trình việc thô cho Hoà** — chỉ đưa board đã qua hết những gì máy phán được, và **gộp các quyết định thị giác liên quan vào cùng một lô** để đỡ tốn băng thông mắt.

[04/09 Hoà ban — NORTH STAR SẢN PHẨM/TRẢI NGHIỆM `N-1…N-20`] Ghi vào **`docs/IF-KIEN-TRUC-OS.md`** (mở rộng hiến pháp gốc, KHÔNG đẻ tệp authority thứ hai — đúng lệnh *"update/extend canonical source"* và tránh đúng bệnh `KB-5`/`.idfnotes`/*"master tool"*). Lõi: IF = **hệ điều hành sáng tạo cho thiết kế nội thất**, lời hứa *"từ ý tưởng tới sự thật thiết kế — không đánh rơi ngữ cảnh"*; hào **không** nằm ở sinh ảnh/render/chat/số tính năng mà ở **liên tục · gia phả · quyết định người · sửa có cấu trúc · sự thật dự án · ký ức · DNA**; **model AI thay được, sự thật thiết kế thì không**. Bốn điều siết thẳng vào cách làm việc: **N-10** 13 cờ đỏ (SaaS dashboard · bento mặc định · tường widget · thẻ-cho-mọi-thứ · hộp rỗng khổng lồ · kính lỏng vô cớ · bo/bóng = cao cấp…) · **N-16** máy KHÔNG phán được bố cục/gu ⇒ bề mặt lớn phải đi `authority → nghiên cứu bố cục → mắt Hoà → thi công → máy kiểm → ảnh → mắt duyệt cuối` · **N-17** trượt vì bố cục thì DỪNG đánh bóng, cấm chữa bằng màu/bóng/bo/mờ · **N-20** cổng **hai câu** (*chạy được?* **và** *có đẩy về north star?*) — A đạt mà B trượt = **chưa xong về mặt sản phẩm**. North star **KHÔNG xoá EXS**; EXS đã duyệt giữ nguyên hiệu lực. Con trỏ đã nối: `docs/CLAUDE.md` mục 1a · `docs/delivery/VISUAL-APPROVAL-QUEUE.md` §AUTHORITY.
[04/09 Hoà chốt — HOME = HYBRID B×A, ambient C] Ba nghiên cứu A/B/C đã duyệt xong và **KHÔNG chọn nguyên bản nào**; chúng thành **ứng viên đã xét**, bị đè bởi quyết định này. Bản chính thức cho Home khổ rộng: **xương sống = B** (bỏ hẳn cột widget phải · hiện vật công việc THẬT làm đối tượng trội · nhìn phát biết *đang làm gì · đang ở đâu · quay lại thế nào*) + **ngôn ngữ = A** (typography · khoảng trắng · vạch mảnh · gom nhóm bằng canh lề/cỡ/vị trí — **không thẻ cho ghi chú/hoạt động/chặng/lời chào/tin tức**) + **C chỉ là lớp khí quyển** (LightClock theo giờ, rất tiết chế — *hỗ trợ việc, không cạnh tranh với việc*; cấm biến Home thành landing page điện ảnh). 🔴 Bốn ràng buộc kèm theo: **§24** cấm chữa "nghĩa địa widget" bằng cách ép hết thành MỘT HÀNG NGANG — *nghĩa địa widget không được thành nghĩa địa toolbar*; thứ phụ xếp theo **liên quan × tần suất × giá trị quyết định × ngữ cảnh**, được nén/gập/hé dần/đưa về theo-yêu-cầu · **§26** trạng thái rỗng KHÔNG phải "Home trừ ảnh hero" mà là `RESUME → BEGIN` (chính: tạo dự án · phụ: mở/nhập · thứ ba: khám phá), vẫn phải ra **studio sáng tạo tĩnh**, cấm 6 thẻ onboarding · **§28** phải dùng **hiện vật công việc THẬT**, cấm gradient giả để Hoà phán thứ bậc; dùng dữ liệu mẫu thì **ghi rõ DEMO** · **§30** nội dung ngoài tầm nhìn phải có **dấu hiệu còn tiếp** — cấm lặp lỗi `scrollHeight 1293 vs clientHeight 775` không dấu hiệu. Bento **không phải bản sắc Home**, chỉ được sống ở khổ hẹp nếu thật sự giải bài thích ứng, và **thứ bậc trội → đỡ → nền phải được giữ nguyên**. ⛔ CHƯA CODE: dựng **MỘT** nghiên cứu chi tiết (rộng-có-việc · rộng-rỗng · khổ hẹp hơn · biến thể nền) → máy kiểm → trình **tối đa 4 ảnh** → DỪNG chờ mắt.

[04/09 Hoà chốt — TRONG IF, MẶT AI LÀ VITALS. KHÔNG CÓ MẶT AI THỨ HAI] Nguyên văn: *"chat gpt ko liên quan, trong if AI tương tác là vitals"*. ⇒ Câu hỏi về ChatGPT/WorkHub **đóng**, nhưng nó để lộ một vi phạm kiến trúc đang sống, ghi lại để không ai coi là tính năng: `components/workhub/WorkHubShell.tsx:140` `submitMessage()` **nối thêm một câu trả lời gõ cứng** — `grep "fetch("` trong tệp = **0**, không có route API nào — mà mặt đó lại **tự xưng là trợ lý** và ở `:150`/`:195` còn khẳng định *"đang dùng ngữ cảnh từ …"* kèm công tắc *"Dùng ngữ cảnh cửa sổ · Bật"*, trong khi **không dòng nào đọc nội dung pane**. Hai lỗi chồng nhau: ① **mặt AI thứ hai** cạnh tranh với Vitals — trái chốt này ② **nút nói dối việc nó vừa làm**, tệ hơn nút chết (nút chết thì người dùng biết mà đi đường khác). ⇒ Việc phải làm: **gỡ mặt trợ lý khỏi WorkHub**, đường hỏi AI ở đó (nếu cần) đi qua **đúng khẩu độ Vitals** như mọi màn. Chưa thi công — Home đang giữ luật `KHÔNG CODE`.

[04/09 Hoà ban — CONTEXT DETOX + IF COMMAND, hai lệnh cùng lượt] ① **CONTEXT DETOX**: Hoà nghi giao diện đang bị nhiễm bởi giả định thị giác cũ còn sót trong repo/ngữ cảnh. Không xoá code, không xoá lịch sử git, không xoá năng lực — chỉ **phân loại sự thật** thành A sản phẩm · B nghề/luồng · C chức năng/hợp đồng · D **thẩm quyền thiết kế hiện hành** · E **lịch sử/đã bị đè** · F **bằng chứng thi công**; **chỉ A/B/C/D được định hướng thiết kế mới**, E chỉ dùng truy nguyên, **F dùng để giữ HÀNH VI, cấm dùng làm tiền lệ thị giác**. ⛔ Bốn câu cấm đọc ngược: *giao diện đang chạy ≠ thẩm quyền · component đang có ≠ khuôn mẫu · code đang có ≠ lý do giữ bố cục · đã qua mắt trong quá khứ ≠ đang được duyệt* (nếu thẩm quyền mới đã đè). ⭐ **Đọc lại NO-REBUILD cho đúng**: §B25 bảo vệ **NĂNG LỰC · HỢP ĐỒNG · DỮ LIỆU**, **KHÔNG** bảo vệ **bố cục thị giác lỗi thời** — trước 04/09 hai thứ này bị đọc lẫn và đó là một nguồn nhiễm thật. ⇒ Sản phẩm: **`docs/ACTIVE-DESIGN-CONTEXT.md`** (bộ ngữ cảnh thiết kế đang hiệu lực, 253 dòng — thứ tự thắng · north star · 12 điều EXS · quyết định Hoà · yêu cầu chức năng A-K · luồng nghề · token/trợ năng đo tại nguồn · hướng đã qua mắt · gu Hoà · chống chỉ định · luật phòng sạch) + **`docs/delivery/LEGACY-DESIGN-QUARANTINE.md`** (sổ cách ly, 3 nhãn 🟢🟡🔴, **12 hướng bị đè cấm hồi sinh**, KHÔNG xoá gì) + con trỏ khởi động `docs/CLAUDE.md` mục **0b**. 🔴 **XUNG ĐỘT NẶNG NHẤT TÌM ĐƯỢC, và nó giải thích cả chuỗi Home trôi về bento**: chính tệp `CHOT-EXPERIENCE-SYSTEM-2026-08-20.md` — bản Hoà **đã duyệt mắt** — ở mục *Hệ quả sổ sách §2* ghi drift sắp thi công là ***"bento thêm hero 2×2 Resume"***, tức **văn bản được duyệt đang RA LỆNH giữ bento**, trong khi N-10 (04/09) liệt `bento làm mặc định` là **cờ đỏ** và D-DR2 đòi **một tiêu điểm**. ⇒ **12 điều nguyên tắc EXS GIỮ TRỌN; riêng dòng drift bento SUPERSEDED** — phân biệt *nguyên tắc* với *giả định thi công* là chỗ phải cắt. Ba study H1/H2/H3 **giữ làm bằng chứng, KHÔNG làm điểm xuất phát thị giác** cho vòng sau (§8). ② **IF COMMAND**: Hoà thôi vận hành worker/phiên. T = **IF COMMAND + CHIEF ARCHITECT + DELIVERY ORCHESTRATOR**; Hoà chỉ giao **INTENT** và phán thứ cần đầu người. Mặc định **HÀNH ĐỘNG**, không hỏi dùng phiên/worker/model/tool nào. ⛔ **Hoà không phải đường truyền tin** — cấm bắt Hoà chép prompt, quản worker, kiểm nhánh, nhớ trạng thái, gom kết quả, viết bàn giao, theo dõi PR. **Bốn cửa duy nhất được ngắt Hoà**: quyết định sản phẩm · quyết định thị giác/gu · việc rủi ro cao hoặc không lùi được · thiếu thật sự ý định Hoà. Cửa thị giác: **máy kiểm trước**, gom thành **tối đa 2-4 quyết định trải nghiệm** mỗi lô, Hoà trả `ĐẠT/SỬA/A-B/GHÉP`. **Chat không phải trí nhớ dự án — repo mới là**: phiên MAIN mới phải tự khởi động từ repo, không cần Hoà kể lại. Thi hành: `docs/HOP-DONG-PHOI-HOP-T.md` **§0** (đè cách hiểu vai cũ ở §1) — kèm khuôn **TASK PACKET 9 ô** và luật worker giao diện **mặc định chỉ đọc ACTIVE-DESIGN-CONTEXT**.

[04/09 ĐÍNH CHÍNH — RỦI RO PHÁT HÀNH "migrations tụt sau schema" ĐÃ ĐÓNG, ba văn bản đang mang trạng thái cũ] T gọi mục này là *"nặng nhất trong sổ hiện giờ"* lúc 08:5x. **SAI — nó đóng từ 01:15 cùng ngày**, bởi một lane khác trong chính phiên này (`fd83f343` *"migration bù — migrate deploy dựng đủ 24/24 bảng"*). Đo lại, không chép: `prisma migrate diff --from-migrations → --to-schema-datamodel` trả **"This is an empty migration"** · `CREATE TABLE` dựng từ migrations **24**, khai trong schema **24**, bảng thiếu **0** · `ProductSpec.matId` **đã có** trong migrations. ⇒ Máy chủ mới chạy `migrate deploy` dựng **đủ 24/24 bảng**. Blocker 19/08 *"Hoà phải chạy tay Prisma migration cho ProductSpec.matId"* đổi bản chất: **tệp migration đã có sẵn**, việc còn lại chỉ là chạy `migrate deploy` trên CSDL thật — thao tác thường lệ, không còn phải soạn migration; phần **backfill** giá trị matId cho hàng cũ thì vẫn còn (`scripts/backfill-material-matid.ts`, mặc định dry-run). Đã sửa: thân PR #15 · `docs/bao-cao-phien/2026-09-04-tinh-trang-va-master-control.md` §④. 🔴 **BÀI HỌC, đắt hơn con số — TRẠNG THÁI CŨ TỰ LAN**: nó nằm trong mục "Nợ để lại" của PR, T chép sang báo cáo, rồi báo miệng cho Hoà — **ba lần khẳng định, không lần nào đo lại**, trong khi thứ đóng nó đã nằm trong repo **8 tiếng**. Đúng luật đã ghi *đo tại nguồn, đừng nhớ hộ máy* — và "nhớ hộ" gồm cả **nhớ hộ chính báo cáo của mình**. Cùng họ với ca `ProjectFile=0` · `IDB=0` · `grep=0` · `merge-base` rỗng, nhưng ngược chiều: bốn ca kia là *máy nói rỗng mà T tin là thế giới rỗng*; ca này là *T nói còn nợ mà thế giới đã hết nợ*. ⇒ Việc cho máy: `soi:frontier` mới canh sổ↔code, **chưa máy nào canh "khẳng định trong văn bản ↔ hiện trạng đo được"** — đó đúng là lỗ mà entry `may-doi-chieu-so-code` sinh ra để bịt, nay có ca thật thứ ba.

[04/09 — A11Y VÒNG FOCUS: đo thật ra 8 lỗ chứ không phải 32, và thế cân bằng dựa vào THỨ TỰ BUNDLE chứ không phải đặc hiệu] IF COMMAND giao phiếu với giả định *"`outline-none` tại chỗ đang giết luật focus toàn app ở `globals.css:435`"*. Worker đo trên Chromium thật (nạp **đúng CSS bundle đã build**, focus bằng phím Tab, đọc `getComputedStyle`) ⇒ giả định **đúng một nửa**, và mẫu grep cũ `outline:\s*none|outline-none` đang **gộp BA cơ chế khác hẳn nhau vào một rọ**: ① class `outline-none` **trần** — Tailwind v3 định nghĩa là `outline:2px solid transparent` (KHÔNG phải `outline:none`), đặc hiệu **0-1-0**, **HOÀ** với luật globals `:where(...):focus-visible` (cũng 0-1-0) ⇒ **thứ tự quyết định**, và trong bundle thật `.outline-none` ở byte **37.724** còn luật focus ở byte **47.057** ⇒ globals đứng SAU ⇒ **ring VẪN HIỆN `2px solid rgb(106,87,245)`** ⇒ **24/32 tệp KHÔNG phải lỗ** ② `focus:outline-none` = **0-2-0** ⇒ **THẮNG** globals ⇒ outline **trong suốt** ⇒ **lỗ thật** ③ `outline:none` viết thẳng trong chuỗi CSS / `style` inline ⇒ **`0px none`** ⇒ **lỗ thật**. ⇒ **8 lỗ thật, đã sửa** (gỡ hẳn class chứ không đổi thành `focus-visible:outline-none` — đổi thế vẫn giết ring lúc gõ phím; `globals.css:439` đã lo sẵn chiều chuột); đo lại sau sửa: `2px solid rgba(0,0,0,0)` → **`2px solid rgb(106,87,245)`**, 100% token, 0 hex. Mẫu soi thu hẹp `[\w-]+:outline-none|outline:\s*['"`]?none`, tự kiểm **8/8 ca**, luật này còn **21 tệp** (giảm 11). 🔴 **WORKER BÁC TRỤC PHÂN NHÓM CỦA CHÍNH PHIẾU — và bác đúng**: phiếu chia theo *"có `focus:` hay không"*, hoá ra nhóm B (khai là "13 lỗ thật") có **11 tệp không phải lỗ**, còn nhóm A (khai là "đã có affordance") lại **chứa 6 lỗ thật**. Trục đúng là **CƠ CHẾ**, không phải sự hiện diện của một biến thể. Worker cũng **từ chối** phần "đổi `focus:` → `focus-visible:` cho 19 tệp" vì ring ở đó **đã hiện**, đổi chỉ làm khác hình lúc bấm chuột — tức việc **thị giác**, đúng thứ phiếu tự cấm. ⏰ **RỦI RO HẸN GIỜ, ghi to**: 24 tệp kia an toàn **nhờ THỨ TỰ BUNDLE, không nhờ đặc hiệu** ⇒ đảo `@tailwind utilities` xuống cuối `globals.css`, **hoặc nâng lên Tailwind v4** (utilities vào `@layer`, thua mọi thứ ngoài layer), là **24 tệp thành lỗ thật ngay trong một lần nâng phiên bản** — cảnh báo đã ghi tại chỗ trong `thao-tac-registry.mjs`. 🔴 **LỖ CỦA CHÍNH MÁY SOI (§7.1)**: `mauThieu` xét theo **TỆP** nên tệp vừa dựng ring cho vật A vừa giết ring vật B thì **lọt sạch** — 5 ca thật đã đo (`three/ve3d-css.ts:34,105` · `library/library-sheet-css.ts:101` · `filemanager/files-mock-css.ts:89` · `dna/inspiration-css.ts:19`) ⇒ **21 là SÀN, không phải trần**; muốn hết lọt phải chuyển luật từ tệp sang **DÒNG**. 🟡 **Chưa kiểm, worker tự khai**: chưa mở app Next thật (đo trên trang tĩnh nạp bundle đã build) · **ring có bị `overflow:hidden` xén không thì CHƯA BIẾT** — 10 chỗ sửa nằm trong panel hẹp render-studio, `.if-focus-inset` có sẵn mà chưa cắm · **có một thay đổi NHÌN THẤY ĐƯỢC**: cả 10 chỗ là `<input>`/`<select>`, mà Chromium cho ô nhập chữ **luôn** khớp `:focus-visible` ⇒ ring nay hiện cả khi bấm chuột, giống 24 tệp kia vốn đã vậy (nhất quán hơn, nhưng vẫn là delta cần mắt) · chỉ đo Chromium 1194. ⇒ Phiếu đợt 2 soạn sẵn: `docs/phieu-giao/P-A11Y-VONG-FOCUS-DOT-2.md`. ⭐ **Đây là lần THỨ HAI trong ngày một máy soi gộp nhiều cơ chế vào một mẫu rồi báo quá tay** (lần đầu: `soi-thao-tac` đọc cả chữ trong chú thích, bắt đúng câu tự dặn *"Không có chữ 'tự động'"*). Cách chữa giống hệt: **ĐO, rồi thu hẹp mẫu ở MÁY, không nới luật** — và cả hai lần đều chỉ lộ ra khi có người/agent **được phép bác lại phiếu**.

[04/09 Hoà nêu — NGHI THẤT THOÁT TÀI NGUYÊN DO REPO NẰM TRONG ~/Downloads, "đã từng bị 1 lần"] IF COMMAND đi tìm bằng chứng TRONG REPO thay vì đoán. ĐO ĐƯỢC: repo trên máy Hoà đúng là ở **`~/Downloads/interiorflow`** (42 lần nhắc trong docs), cùng ~8 thư mục anh em cũng trong Downloads (`interiorflow-phu` · `-g4` · `-wt-p3c` · `ttt-tasks` · `archinote` · `_TTT-BRAND` · `interiorflow-reference` · `IF-git-backup`) · `.DS_Store` từng lọt vào lịch sử (máy macOS) · **KHÔNG** có rác đồng bộ nào trong tệp đang theo dõi · **KHÔNG** commit nào xoá hàng loạt · `git fsck` bản clone đám mây **sạch** · chữ "synwork" không tồn tại ở đâu trong repo. 🔧 **PHẢI NÓI THẲNG HAI CHIỀU**: ⓐ nghi ngờ của Hoà **có cơ sở về mặt cơ chế** — thư mục đồng bộ ôm `.git` là nguyên nhân hỏng kho kinh điển (ghi/khoá tệp giữa lúc git đang ghi · tệp bị "giải phóng dung lượng" thành vỏ rỗng · rác bản-sao-xung-đột), và `~/Downloads` tuy KHÔNG tự đồng bộ trên macOS (iCloud chỉ ôm Desktop+Documents) nhưng **Google Drive cho phép thêm thư mục bất kỳ** — mà Hoà đang dùng Google Drive cho cửa duyệt mắt; ⓑ **nhưng ca mất mát ĐÃ ĐƯỢC GHI trong sổ thì có nguyên nhân KHÁC, đã xác định**: lô ảnh `home-that-*.png` biến mất khỏi đĩa 04/09 là do `.nen-chrome-out/` **bị gitignore** + ba lane chạy song song dọn nó giữa chừng — **git không cứu thứ nó không theo dõi**. ⇒ **Đừng gộp hai thứ làm một**: chỗ đặt repo là rủi ro CHƯA KIỂM ĐƯỢC (phiên đám mây không thấy máy Hoà), còn thất thoát ĐÃ XẢY RA thì gốc là **thứ nằm ngoài git**. 🛠 Giao lại cho Hoà một máy chẩn đoán CHỈ-ĐỌC chạy trên máy thật: `bash scripts/chan-doan-noi-luu-repo.sh` — 7 mục: chỗ đặt · tiến trình đồng bộ đang chạy · **tệp dataless** (thủ phạm âm thầm nhất: nội dung không nằm trên đĩa, git đọc ra rỗng) · `git fsck` · rác đồng bộ · hệ tệp phân biệt hoa-thường (APFS mặc định KHÔNG phân biệt ⇒ hai tệp chỉ khác chữ hoa đè nhau trên máy Hoà mà không đè trên CI) · số mục chưa commit và bị gitignore. Kèm chặn rác đồng bộ trong `.gitignore`. 🔴 **MÁY CHẨN ĐOÁN NÓI DỐI NGAY LẦN CHẠY ĐẦU, T tự bắt**: `pgrep -fli` khớp **toàn bộ dòng lệnh**, mà danh sách tên công cụ nằm ngay trong chính script ⇒ nó tự khớp mình và báo **"9 công cụ đồng bộ đang chạy"** trong một container Linux trống trơn. Vá bằng cách khớp theo TÊN TIẾN TRÌNH + loại trừ `$$`/`$PPID`. ⇒ **Đây là lần THỨ BA trong ngày một máy soi báo quá tay vì mẫu quét bắt trúng chính nó hoặc chính chú thích của nó** (① `soi-thao-tac` đọc chữ trong chú thích ② mẫu `outline-none` gộp ba cơ chế ③ ca này). **Luật rút ra: máy soi nào quét văn bản thì phải tự loại trừ CHÍNH NÓ ra khỏi vùng quét — và phải chạy thử ở nơi biết chắc kết quả đúng trước khi tin nó.** Khuyến nghị bất kể kết quả chẩn đoán: **dời repo ra khỏi `~/Downloads`** (vd `~/Code/interiorflow`) — Downloads còn bị nhiều tiện ích "dọn tải về" quét; và **sao lưu đúng cách là ĐẨY LÊN GIT**, không phải để công cụ đồng bộ ôm thư mục `.git`.
