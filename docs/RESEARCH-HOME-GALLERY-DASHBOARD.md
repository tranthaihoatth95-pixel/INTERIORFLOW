...
|---|---|---|---|
| **M1** | `LarkTaskRef` schema (`db push`, additive) · script sync tay + nút bấm trong Gallery · `Project.larkProjectCode` (optional, gán qua bước "Liên kết Larkbase" tuỳ chọn khi tạo/sửa dự án, §2.4) · dữ liệu Larkbase bổ sung ĐÚNG VỊ TRÍ card đang có (§2.2(a)) · nút "Chi tiết" mở panel project-management sort được (§2.2(b)/(c)) | Câu hỏi mở (d)/(e)/(f) §5.2 trả lời (câu (a)/(g)/(h) không chặn M1, có thể build với khuyến nghị mặc định) | Thấp — toàn field optional/mới, giống mẫu additive đã dùng ở `RESEARCH-ACCESS-CONTROL.md §2.3`/`RESEARCH-MATERIAL-BRIDGE.md §2.2`. Riêng bước lọc `Mã DA` phi-số cần làm đúng ngay từ M1 (đã biết trước, không phải rủi ro ẩn) |
...
| **M3** | ~~Đẩy ngược lên Larkbase~~ **ĐÃ LOẠI BỎ HẲN theo quyết định 2 (§5.1)** — không có M3 cho hướng 2 chiều. Việc còn lại thuộc M-sau nếu có nhu cầu: mở rộng bộ lọc kanban theo phòng ban (câu hỏi (h)), hoặc chế độ xem "Kanban thay Gallery" nếu câu hỏi (g) chọn phương án 2 | M2 + quyết câu hỏi (g)/(h) §5.2 | Thấp — vẫn thuần đọc, chỉ là mở rộng UI |
...
| # | Câu hỏi | Khuyến nghị |
|---|---|---|
| **(a)** | Gallery (`ProjectSelect`) đã được xác nhận là nơi duy nhất chứa tính năng này (không route mới — xem cảnh báo đầu file). Câu hỏi còn lại: **lối vào chủ động** cho returning-user — vì `enterAfterAuth()` (`:97-149`) hay auto-skip qua Gallery (§1.6), cần 1 điểm neo khác (VD nút "Tổng quan" trong `Header`) để mở lại Gallery + bấm "Chi tiết" khi cần, hay chấp nhận returning-user chỉ thấy phần này lúc chủ động rời canvas quay lại Gallery? | Khuyến nghị thêm 1 nút nhỏ cố định trong `Header` (không đổi luồng resume hiện có, chỉ thêm lối tắt) — nhưng vị trí/nhãn cụ thể cần chủ dự án duyệt vì đụng UI đang ổn định |
...
| **(g)** | Kanban theo `Trạng thái` Larkbase (§2.2(d)) — đặt làm 1 tab trong panel "Chi tiết", hay 1 chế độ xem khác của chính Gallery (chuyển "Lưới bìa ↔ Kanban")? | Nghiêng nhẹ về **tab trong "Chi tiết"** (rẻ hơn, không đụng luồng carousel/grid Gallery đang chạy tốt) — nhưng để ngỏ, chưa tự quyết vì ảnh hưởng tần suất người dùng chạm tới kanban hàng ngày |
...
| Brief cũ "gallery cho phép tạo project mới/kanban" | §1.1 (tạo project: ĐÃ XONG) — kanban: CHƯA, đề xuất vị trí ở §2.2(d), còn để ngỏ câu hỏi (g)/(h) §5.2 |
