...
| Bộ lệnh dựng hình 3D — boolean (khoét/hợp/giao) qua `ops[]` | ✅ NC-12+SPEC-DUNG-BO-LENH-3D | — | 🔵 03/08 đêm: `three-bvh-csg` + `Base.ops` (3 phép) + `lib/three/csg.ts`+`build-ops.ts` (cache) + `commands.ts` `cutHoleInWall` + nút thật "Khoét hốc" (`Object3DInspector.tsx`) | 🔵 30 test pass + verify browser (ops ghi đúng Doc, CSG chạy thật không lỗi) — CHƯA audit độc lập | `extrude`/`arrayLinear` mới khai TYPE, chưa nối derive (N5) · phát hiện lỗi CÓ TRƯỚC "3D không autosave" khi verify — xem TECH-DEBT |
