...
| 6 | Sau brainstorm có bước DISTILL → PROJECT DESIGN DNA. | [DESIGN DIRECTION][ĐANG CÓ] | DistillEngine ĐÃ có 2 production caller (Gate C-4: `CuaSoThaoLuan.tsx:182` · `DesignDnaCardPanel.tsx:297`) |
...
| 8 | 2D plan có thể là spatial interface để đặt/đi camera nhất quán. | [DESIGN DIRECTION][CHƯA CẮM] | `IF_CAMPATH` + camera intent có; `CamPathPreview` chưa wire vào editor 2D (gap D5 sổ cũ) |
...
| 17 | Checkpoint/Review/Approval là primitive xuyên workflow. | [DESIGN DIRECTION][GAP] | Gate C-2: decision history ≈ 0, mới có `FfeApproval`; primitive chung = ADR-Q8, Wave 3 |
