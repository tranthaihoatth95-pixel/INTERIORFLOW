...
**Boundary-trace của HATCH (`lib/cad/hatch.ts`) không đáng tin cậy với phòng có T-junction**
(vách khác đâm vào tường bao — rất phổ biến trong mặt bằng nhiều phòng). Phòng đơn (tường bao
khép kín, không vách nào đâm vào) vẫn dò đúng (đã test kỹ — `hatch.test.ts` + phòng NGỦ/WC
trong `demo-plan.ts`). Nguyên nhân: `commands.ts` vẽ mỗi đoạn tường là 1 quad ĐỘC LẬP không vát
góc, chồng lấn ở góc/chữ T tạo khe hở hình học nhỏ khiến thuật toán half-edge face-traversal đôi
khi bắt nhầm. Xử lý AN TOÀN ở `checker.ts`: bỏ qua phòng không dò được biên (không báo sai) —
nghĩa là lệnh H và panel Kiểm chuẩn có thể KHÔNG hoạt động ở 1 số phòng cụ thể tuỳ hình học. Đã
tạo task riêng theo dõi nâng cấp thuật toán (xem chip "Fix hatch boundary-trace for T-junction
walls" trong phiên làm việc, hoặc tìm trong lịch sử session).
