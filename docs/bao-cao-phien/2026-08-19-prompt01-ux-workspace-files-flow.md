...
### ERRATA (cập nhật sau khi báo cáo phát hành)
- **22/08 — bảng P4 dòng "VitalsPill (Home) ✅" HẾT HẠN**: Vitals đã tái kiến trúc — `VitalsPill`
  nay **0 mount** (chỉ còn vỏ trình bày), live Vitals = `components/studio/VitalsAperture.tsx`
  (top-edge, theo chốt Experience System 20/08) + `VitalsChatSurface.tsx`. Dòng "StatusBar chip
  nút giả / VitalsGesture chết" cũng phải đo lại trước khi trích — vùng này đã được lane code
  làm lại sau audit.
- **20/08 — mục 2.4 "promote/normalize: 0 hit" ĐÃ HẾT HẠN**: commit `922af7e` (20/08) đưa chuỗi
  **ProjectFile → Đã xem (human gate) → Promote → LibrarySheet refresh** lên màn `/files`, browser
  verified bởi phiên MAIN. Finding "hai đường upload không nối nhau" thu hẹp lại: đường `/files`
  nay CÓ lối vào Library; đường `/library/ingest` (IDB manifest) vẫn tách — đo lại trước khi trích.
