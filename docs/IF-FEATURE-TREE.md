...
| 2.1.8.k | Xuất bộ hồ sơ — gộp nhiều tờ thành 1 PDF có mục lục | N | 🟡/⬜ chưa xác minh được cơ chế mục lục | không | `SPEC-CAD-MODES.md` |
...
| 7.3.30 | Dựng `/settings` 4 nhóm (Tài khoản·Giao diện·AI·Trải nghiệm): thêm link "Ảnh đại diện" (bỏ `/settings/avatar` mồ côi) · gỡ ngôn ngữ+"xem lại hướng dẫn" khỏi MoreMenu/MobileMenu, dời vào Giao diện/Trải nghiệm · **giữ credits ở MoreMenu** (trạng thái xem) | N | ✅ done (checklist 6 bước đủ) — commit theo sau `74cf4c5`: `app/settings/page.tsx` 4 nhóm (`AccountSettings`/`AppearanceSettings`/`AiDependencySettings`/`ExperienceSettings` mới), `Header.tsx`+`MobileMenu.tsx` gỡ Ngôn ngữ+"Xem lại hướng dẫn", thêm link "Cài đặt". Browser-verify thật 1440px+375px: đủ 4 nhóm, MobileMenu/MoreMenu xác nhận qua DOM không còn Ngôn ngữ/hướng dẫn. ✅ **Lệch so với đề xuất "gom sáng/tối về 1 chỗ" — Hoà XÁC NHẬN ĐÚNG (30/07), ticket gốc sai**: kiểm code phát hiện `Header.tsx` chỉ mount ở `HomeScreen` (route `/`), `StudioBar.tsx` mount ở `CadStageScreen`/`PresentStageScreen`/`PhotoEditorScreen` — KHÁC ROUTE, không hiện đồng thời cho cùng 1 người dùng, không phải trùng lặp thật; xoá 1 trong 2 sẽ làm mất nút đổi theme thật ở nhóm route kia. GIỮ CẢ 2 nút nhanh (đúng luật "nút nhanh" của ticket §4), chỉ gom phần LOGIC dùng chung vào `lib/theme-toggle.ts` (Luật #6 Đồng Bộ — một hàm tính "bước kế tiếp", nhiều mặt tiền hiển thị). `CommandPalette.tsx` giữ nguyên (lệnh bàn phím, không phải nút hiện — không cạnh tranh thị giác). **Ghi cố định**: đừng ai xoá lại 1 trong 2 nút trong tương lai tưởng là dọn trùng lặp — đây là 2 mặt tiền cần thiết của 2 route khác nhau. | 2.2.61.a (✅) | `docs/TICKET-SETTINGS-GOM-CAU-HINH-2026-07-29.md` §5 |

| 7.3.31 | 3 nút chặng nhảy vị trí khi chuyển chặng — 3 nguồn: ① `fontWeight` 600 active/500 inactive đổi bề rộng chữ ② nhãn CAD tự đổi độ dài theo cadStage ③ nhãn micro `· {label}` trùng thông tin với pill, đẩy Tệp/⋯/avatar theo | N | ✅ done (checklist 6 bước đủ, 30/07) — `.stage-btn::before{content:attr(data-label)}` (`app/globals.css`) chiếm sẵn bề rộng theo bản CHỮ ĐẬM RỘNG NHẤT mỗi nút có thể hiện (không phải nhãn đang hiện), bịt ①+② CÙNG LÚC; bỏ hẳn `· {label}` trùng lặp ở nhãn micro (`StageSwitcher.tsx:417` cũ), bịt ③. ② thực ra là code chết hôm nay — xác nhận `setStage()` **0 lần được gọi ở bất kỳ UI nào** (`cadStageFromProjectStage()` chỉ xuất hiện trong `lib/server/access.test.ts`), cadStage luôn = `'sketch'` trong thực tế; vẫn sửa theo yêu cầu "sửa cả ba" — không di chuyển gì sang `CadToolbar` `ModeSwitch` (dòng 243) như ticket đề xuất vì đó là toggle Sketch↔Pro (`cadMode`, input method) — KHÁC HẲN `cadStage` (giai đoạn bàn giao IF2 relay) mà ticket nhầm là cùng một thứ. Browser-verify thật: ép `workspace`/`cadStage` qua store trực tiếp (không qua click, tránh navigate đổi route) để đo `.if-dock` + nút "Tệp" ở CẢ 3 chặng + cả 3 biến thể cadStage, `getBoundingClientRect()` — left/width **giống hệt tới 6 số thập phân** (0 sai số, dưới ngưỡng ≤0.5px) ở cả 1183px lẫn 1440px | không | `docs/TICKET-STAGE-SWITCHER-NHAY-2026-07-30.md` |
