...
| 7.3.30 | Dựng `/settings` 4 nhóm (Tài khoản·Giao diện·AI·Trải nghiệm): thêm link "Ảnh đại diện" (bỏ `/settings/avatar` mồ côi) · gom sáng/tối về 1 chỗ duy nhất (hiện 3: `Header.tsx` MoreMenu/`StudioBar.tsx`/`CommandPalette.tsx`) · gỡ ngôn ngữ+"xem lại hướng dẫn" khỏi MoreMenu/MobileMenu · **giữ credits ở MoreMenu** (trạng thái xem, không phải cấu hình đặt — luật phân biệt trong ticket) | N | ⬜ `/settings` hiện chỉ có đúng 1 mục (`AiDependencySettings`, xác nhận `app/settings/page.tsx:36`); `/settings/avatar` tồn tại nhưng không link nào trỏ tới (grep `avatar` trong `page.tsx` = 0 kết quả) | 2.2.61.a (đã ✅, cùng cụm dọn Cài đặt) | `docs/TICKET-SETTINGS-GOM-CAU-HINH-2026-07-29.md` §5 |
