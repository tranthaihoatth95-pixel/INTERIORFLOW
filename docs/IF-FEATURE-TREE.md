...
| 7.3.32 | AppChrome cụm phải (Tasks/Home/⋯/UserChip, `hidden…sm:flex`) tràn viewport ở 640-1023px — nhảy thẳng ẩn→đủ desktop ngay tại `sm`=640, dải 640-1023 chưa từng được thiết kế cho cụm này | N | ⬜ chưa sửa — đo thật 30/07 (`docs/VERIFY-7.3.31.md`/báo cáo `2.2.86`): `header.scrollWidth` **819px ở cả 640px và 768px viewport** (tràn **179px@640 · 51px@768**), route render. 1024/1180/1440 sạch (0 tràn). Nguồn phát hiện: `7.1.20` bước 4 khảo dải 1024-1279, đo số thật khi verify `2.2.86`. Hướng sửa gợi ý (chưa chốt): bậc 3/4 thang ưu tiên đã ghi ở `AppChrome.tsx` (Home → gộp ⋯, "Việc" → gộp ⋯) — CẦN ĐO LẠI xem đủ bù 179px/51px hay cần thêm bậc 5 (Tệp bỏ chữ, chỉ áp route render). | 7.1.20 (✅ — nguồn hằng `BP`+giao thức verify 5 mốc dùng để đo mã này) | `docs/VERIFY-7.3.31.md` (số đo gốc) |
