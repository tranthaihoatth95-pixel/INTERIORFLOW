...
| 7.1.21 | Khai bộ test vào `package.json` — thêm script `"test"` (hiện KHÔNG có; 100+ file `*.test.ts` chỉ chạy được qua `node_modules/.bin/sucrase-node <file>` gõ tay từng file — đúng lý do `7.1.19` ship được mà chưa qua lớp test nào ở worktree phụ) | N | ⬜ đề xuất 30/07 (lúc merge `7.1.19`), **CHƯA CODE — chờ Hoà xác nhận mã** | không | STATUS.md 30/07 khuya |
| 7.1.22 | Bộ nhớ đo đạc — Tầng 1 (Không AI) "Đo món đồ" (`2.2.87`/`2.2.88`) tự học theo thời gian bằng dữ liệu nhà (bảng Prisma `MeasurementSample`: kích thước theo loại đồ TRUNG VỊ+độ lệch · tỉ lệ bộ phận · ⭐ chiều cao máy ảnh thật theo preset studio), KHÔNG bằng model — tất định, 0đ, dữ liệu không rời máy. Ngưỡng áp dụng n<5 dải chuẩn/n≥5 trộn/n≥20 ưu tiên dữ liệu nhà, luôn hiện nguồn số, sửa số tại chỗ không ma sát (không modal/không nút Lưu) | N | ⬜ đề xuất 30/07 (ngay sau khi cascade 4-bậc "không-bao-giờ-fail" của `2.2.87` xong), **CHƯA CODE — theo đúng yêu cầu "báo mã, chờ xác nhận"**. ⚠️ **Va số với `7.1.21` ở lần Hoà gửi mã** — cả 2 đều được gắn "7.1.21": `7.1.21` đã đề xuất TRƯỚC (script test, đang chờ xác nhận từ lúc merge `7.1.19`) nên giữ nguyên số đó; việc "Bộ nhớ đo đạc" này tự xếp `7.1.22` (số trống kế tiếp) — Hoà xác nhận lại hoặc đổi nếu muốn thứ tự khác. | 2.2.87 (cascade 4 bậc phải xong trước, ĐÃ xong 30/07 khuya) | tin nhắn Hoà 30/07 khuya (Bộ nhớ đo đạc) |
