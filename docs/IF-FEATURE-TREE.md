...
| 7.3.30 | Dựng `/settings` 4 nhóm (Tài khoản·Giao diện·AI·Trải nghiệm): thêm link "Ảnh đại diện" (bỏ `/settings/avatar` mồ côi) · gỡ ngôn ngữ+"xem lại hướng dẫn" khỏi MoreMenu/MobileMenu, dời vào Giao diện/Trải nghiệm · **giữ credits ở MoreMenu** (trạng thái xem) | N | ✅ done (checklist 6 bước đủ) — commit theo sau `74cf4c5`: `app/settings/page.tsx` 4 nhóm (`AccountSettings`/`AppearanceSettings`/`AiDependencySettings`/`ExperienceSettings` mới), `Header.tsx`+`MobileMenu.tsx` gỡ Ngôn ngữ+"Xem lại hướng dẫn", thêm link "Cài đặt". Browser-verify thật 1440px+375px: đủ 4 nhóm, MobileMenu/MoreMenu xác nhận qua DOM không còn Ngôn ngữ/hướng dẫn. ⚠️ **Lệch so với đề xuất "gom sáng/tối về 1 chỗ"**: kiểm code phát hiện `Header.tsx` (app chính) và `StudioBar.tsx` (CAD/Present/Photo-editor) là 2 thanh đầu KHÁC ROUTE, không hiện đồng thời — xoá 1 trong 2 sẽ làm mất nút đổi theme ở nhóm route kia, không phải trùng lặp thật. Đã GIỮ CẢ 2 nút nhanh (đúng luật "nút nhanh" của ticket §4), chỉ gom phần LOGIC dùng chung vào `lib/theme-toggle.ts` (Luật #6 Đồng Bộ — một hàm tính "bước kế tiếp", nhiều mặt tiền hiển thị). `CommandPalette.tsx` giữ nguyên (lệnh bàn phím, không phải nút hiện — không cạnh tranh thị giác). Cần Hoà xác nhận lại nếu ý định ban đầu là bắt buộc chỉ 1 nút hiển thị duy nhất bất kể route. | 2.2.61.a (✅) | `docs/TICKET-SETTINGS-GOM-CAU-HINH-2026-07-29.md` §5 |
