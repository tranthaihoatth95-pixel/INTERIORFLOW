...
| 2.1.6.d | 🔴 BUG — Nhập DWG treo vĩnh viễn trên file thật (Worker cô lập GPL, `lib/cad/dwg.ts`/`dwg-worker.ts`/`dwg-map.ts`) — **HÀNG ĐỢI, KHÔNG cắt ngang việc đang làm** (lỗi ghi đĩa im lặng B3 ưu tiên cao hơn, Hoà chỉ định rõ) | N | ⬜ chưa sửa, chỉ mới cấp mã + ghi nhận đầy đủ bằng chứng (31/07). **HIỆN TƯỢNG**: file `01_8314_PRG SU_MBBT_T1-2-3_2060731.dwg` (1,8MB, AC1032/AutoCAD 2018-2020, 6 mặt bằng 3 tầng×2 phương án) — CAD → Mở tệp → chọn .dwg → "Đang đọc…" KẸT VĨNH VIỄN, chờ nhiều phút không trả lời, không lỗi. **ĐÃ LOẠI TRỪ** (đừng dò lại): thư viện `@mlightcad/libredwg-web ^0.7.7` có cài đủ trong `node_modules` · Worker cô lập ĐÚNG (UI không chết — panel Lớp mở được, toạ độ chuột vẫn chạy, việc nặng không lọt lên luồng chính) · file KHÔNG hỏng. **MANH MỐI MẠNH NHẤT** (Hoà + Cowork, thu hẹp phạm vi rất nhiều): CLI `~/Downloads/dwg2dxf` parse ĐÚNG file này trong VÀI GIÂY — ra 822 đối tượng/88 layer, bỏ qua 719/1541. Khác biệt DUY NHẤT giữa CLI và IF: CLI KHÔNG gửi bảng block ⇒ KHÔNG chạy block-flatten INSERT (`dwg-map.ts:57`) — IF CÓ gửi ⇒ CÓ chạy. ⇒ NGHI (giả thuyết, KIỂM rồi hãy tin — chưa xác nhận): WASM parse KHÔNG chậm, chỗ treo là BƯỚC GIẢI NÉN INSERT/MINSERT trong `dwg-map.ts` (block-flatten, `MAX_INSERT_DEPTH=8`) — kiểm cụ thể MINSERT hàng×cột lớn · block lồng sâu · block tự tham chiếu (cycle) · tổng entity sau giải nén bùng nổ tổ hợp. **BỔ SUNG (31/07, củng cố nghi vấn)**: file `.bak` CÙNG bản vẽ là AC1021/AutoCAD 2007, 2,53MB — LỚN HƠN bản `.dwg` hiện tại (AC1032, 1,80MB) 40%, tức bản vẽ CŨ ĐƯỢC NÂNG ĐỜI nhiều khả năng mang block lồng/trùng tích tụ qua nhiều đời + đã PURGE một phần — càng củng cố hướng "block-flatten bùng nổ", không phải "WASM chậm". Khi sửa: BẮT BUỘC thử CẢ HAI file (.dwg + .bak) — nếu AC1021 cũng treo thì loại được yếu tố phiên bản định dạng khỏi danh sách nghi phạm. **PHẢI SỬA — 4 việc**: ① Timeout — `openDwgFile()` (`lib/cad/dwg.ts`) hiện CHỈ có `onmessage`/`onerror`, KHÔNG có `setTimeout` — WASM không trả lời ⇒ Promise không bao giờ settle. Thêm timeout (đề xuất 60s) → `terminate()` worker → reject lỗi tiếng Việt rõ. ② Tiến độ — worker `postMessage` tiến độ định kỳ, tối thiểu "đã đọc N/M đối tượng" — hiện KHÔNG có cách nào biết worker còn sống hay đã chết, đây là chỗ TỆ NHẤT (tệ hơn cả chậm). ③ Nút Huỷ — đang đọc mà muốn dừng hiện chỉ còn cách tải lại trang; cần nút huỷ gọi `terminate()` + reset UI. ④ Chặn bùng nổ ở block-flatten — giới hạn tổng entity sau giải nén (có "van an toàn 200k entity" theo commit `e65970f` nhưng rõ ràng chưa đủ chặn ca này), phát hiện MINSERT hàng×cột quá lớn/block lồng quá sâu, dừng CÓ KIỂM SOÁT và báo ra thay vì nhai vô hạn — CẦN KHÁM LẠI code hiện tại của van an toàn này khi bắt tay sửa (chưa rõ vì sao 200k không chặn được ca thật). **NGHIỆM THU**: mở ĐÚNG file thật này (không phải file mẫu nhỏ) — trong vòng 60s phải rơi vào MỘT trong hai: mở được + báo số đối tượng bỏ qua, HOẶC báo lỗi rõ ràng đọc hiểu được. "Kẹt Đang đọc mãi mãi" KHÔNG được tính là đạt. **TIỆN THỂ khi làm** (ngoài repo IF, không phải tree code riêng — CLI cá nhân `~/Downloads/dwg2dxf`, ghi nhớ ở đây để không quên): `cli.js:46` vẫn `require lib/cad/dwg.ts` — file này có `import.meta` nên `require()` ném "exports is not defined in ES module scope"; comment đầu `dwg.ts` đã dặn đổi sang `require lib/cad/dwg-map.ts` nhưng CLI chưa ai đổi, sửa 1 dòng. Cân nhắc cho CLI gửi luôn bảng block để khớp hành vi IF — hiện CLI bỏ 47% đối tượng nên KHÔNG dùng để đối chứng IF được nữa nếu không sửa. Có file rác Cowork để lại `dwg2dxf/cli.tmp-cowork.js` — xoá khi vào việc. | không | tin nhắn Hoà 31/07 (bug report + bổ sung .bak) |
...
| 2.1.8.k | Xuất bộ hồ sơ — gộp nhiều tờ thành 1 PDF có mục lục | N | ✅ done (checklist 6 bước đủ, 30/07) — `buildSheetSetPdf()`/`exportSheetSetPdf()` (`lib/cad/pdf.ts`): trang 1 mục lục (số tờ·tên·khổ·tỉ lệ) + mỗi tờ 1 trang, TÔN TRỌNG `paperKey`/`printScale` riêng từng tờ (mỗi trang `addPage([pw,ph], orientation)` riêng, không ép chung 1 khổ) + bookmark PDF thật (`pdf.outline.add`, Outline PlugIn lõi jsPDF, không thêm dependency). Nối vào IOMenu chặng CAD cạnh `.idf` (`CadEditor.tsx`) qua CustomEvent `cad:sheetset-pdf-export-request` — CÙNG pattern bắc cầu đã dùng cho `.idf`/`.ifpack` (`CadSheets.tsx` giữ `sheets[]`, `CadEditor.tsx` không giữ). Luật #9 (≥300dpi): KHÔNG áp dụng — hàm này 100% vector (dùng lại `drawEntityPdf`, 0 `addImage()` trong file, đã grep xác nhận), Luật #9 chỉ áp cho export RASTER (`docs/LUAT-300DPI-2026-07-29.md`) nên không có nhánh "báo dpi" — không phải bỏ sót. 13/13 test mới (`pdf-sheetset.test.ts`) — 3 tờ khác khổ → 4 trang đúng khổ + đúng bookmark; 0 tờ → 1 trang mục lục không crash. ⚠️ Lệch nhẹ so với brief: test dùng khổ **A1/A2/A3** thay vì "A2/A3/A4" — `PaperKey` (`model.ts:490`) chỉ có 3 giá trị này, KHÔNG có 'A4', xác nhận 30/07. Sửa kèm doc drift `docs/MULTI-SHEET-PROPOSAL.md` §7 ("không persist qua reload" — sai, đã persist IndexedDB từ J-3 Sprint 2). 🔄 **CẬP NHẬT sau `2.1.8.m`**: `PaperKey` nay ĐÃ có A4 (và A0) — lệch "A1/A2/A3 thay A2/A3/A4" ở trên chỉ còn đúng LỊCH SỬ (tại thời điểm `2.1.8.k` code), test `pdf-sheetset.test.ts` đã cập nhật dùng đúng A2/A4/A1 khi `2.1.8.m` làm xong. | không | `SPEC-CAD-MODES.md` |
| 2.1.8.l | Sửa va chạm `CadTouchDock` (dock cảm ứng Sketch đè lên: chữ toạ độ trên canvas, panel Khung tên, panel chọn BIM/ShapeInfo — lần thứ 3 né dock bằng số cứng) — sửa CƠ CHẾ, không né lần thứ tư | N | ✅ done (checklist 6 bước đủ, 30/07) — `lib/cad/safe-area.ts` mới: `canvasSafeAreaInsets(cadMode)` (nguồn DUY NHẤT khai vùng dock chiếm, đọc TỪ dock — `DOCK_HEIGHT`/`DOCK_OFFSET` khớp `CadTouchDock.tsx` thật, không đoán số). Gỡ 2 hard-code cũ đọc hàm này: `CadEditor.tsx` `TitleBlockPanel` (`maxHeight: calc(100% - 130px)` đoán chừng → tính thật từ `safeArea.bottom`) + `SelectionInfoPanel` (`bottom: 110` đoán chừng → `safeArea.bottom`, đo thật gap còn 12px thay vì 42px lãng phí trước đó). Xoá HẲN (không né) chữ toạ độ vẽ trên canvas (`CadCanvas.tsx` `drawCrosshair()`) — xác nhận TRÙNG dữ liệu với `StatusBar.tsx` (cả 2 đọc `ix.current.cursorWorld`/`useCadLiveStatus`, throttle 100ms), không phải 2 ý nghĩa khác nhau; browser-verify: mouse rê liên tục, StatusBar cập nhật đúng nhịp ~100ms (đo thật bằng dispatch `pointermove`), không giật nhận thấy được (crosshair LINE vẫn vẽ mỗi frame, chỉ chữ số bị xoá). Bậc ③ (30/07, cùng luật `2.2.86` quyết định ④ TICKET-CHAY-FLOW): nút BẬT trong `CadTouchDock`/`CadToolbar` đổi từ TÔ ĐẶC `var(--accent)` sang GHOST (viền `--accent-ring`+nền `--accent-soft`+chữ `--accent`) — browser-verify: "Sketch"/"Số liệu" hiện ghost đúng, không đổi kích thước khi toggle active (border 1px giữ cả 2 trạng thái, tránh nhảy layout). | không | `docs/CHOT-LARK-3-PHAT-HIEN-2026-07-30.md` (không liên quan — mã này phát hiện từ ảnh Hoà gửi kèm tin nhắn, không có ticket gốc riêng) |
| 2.1.8.m | Sửa lỗi gốc "nướng hướng giấy vào tuple kích thước" (Luật #10, không hỏi) — khổ giấy (ISO 216) và hướng giấy (ISO 5457) là HAI TRỤC ĐỘC LẬP, không phải "A3-ngang" là khổ thứ 6 | N | ✅ done (checklist 6 bước đủ, 30/07) — `model.ts`: `PAPER_SIZES_MM` đổi sang lưu CHUẨN DỌC [ngắn,dài] + đủ A0-A4 (trước chỉ A1/A2/A3); `paperSizeMm(key, orientation)` hàm DUY NHẤT đọc kích thước hiệu dụng (hoán vị khi ngang); `defaultPaperOrientation(key)` (A0-A3 ngang, A4 dọc, đúng ISO 5457); `Doc.paperOrientation?` field mới, tương thích ngược (Doc cũ không field này → A3 ngang, hành vi cũ nguyên vẹn, xác nhận bằng test). `docPaperMm()` cập nhật đọc qua `paperSizeMm()`. UI `CadEditor.tsx` `TitleBlockPanel`: thêm nút "⇄ Ngang/Dọc" RỜI cạnh dropdown Khổ giấy (không đẻ "A3-portrait" làm khổ thứ 6, đúng yêu cầu). `pdf.ts`: `buildSheetSetPdf()` mục lục in "Khổ · Hướng" (`paperKeyOrientationLabel()` mới, export riêng để test trực tiếp — không spy `jsPDF.prototype.text` vì jsPDF gắn `text` làm own-property mỗi instance, patch prototype vô tác dụng, xác nhận bằng `node -e`). Quét kèm (Luật Đồng Bộ #6): `lib/imaging.ts` xác nhận KHÔNG liên quan (board Render dùng hằng số A3-ngang cố định riêng, không đọc `PAPER_SIZES_MM`) — không sửa, không phải bỏ sót. 3 test file cập nhật theo API mới (`cad-core-b1.test.ts`, `pdf-scale.test.ts` thêm test A4 dọc/ngang/A0/tương thích ngược, `pdf-sheetset.test.ts` đổi sang A2/A4/A1 đúng brief gốc + test hướng trộn). 103/103 test, tsc+eslint sạch. | 2.1.8.l (chung số commit thực tế — CadEditor.tsx đụng cả 2 mã cùng lúc, tách file không thực tế) | `docs/CHOT-LARK-3-PHAT-HIEN-2026-07-30.md` (không liên quan — cùng nguồn phát hiện với `2.1.8.l`, từ ảnh+tin nhắn Hoà, không có ticket gốc riêng) |
| 2.1.8.n | Ctrl/⌘+S (ép autosave chạy ngay + toast "Đã lưu — HH:MM", KHÔNG đường lưu mới) · Ctrl/⌘+Shift+S (xuất `.idf`, tái dùng luồng có sẵn) · chỉ báo "Đã lưu lúc HH:MM" luôn hiện trong StatusBar — GẤP, chặn buổi thử CAD trên LAN 31/07 | N | ✅ done (31/07, checklist rút gọn theo yêu cầu "việc nhỏ, đừng mở rộng phạm vi") — `CadCanvas.tsx` thêm nhánh Ctrl/⌘+S vào ĐÚNG khối ⌘Z/⌘Y/⌘C/⌘V có sẵn (dòng ~1977, cùng khuôn input-guard ở đầu handler) — `e.preventDefault()` bắt buộc rồi phát `cad:force-save-request` (S thường) hoặc tái dùng `cad:idf-export-request` có sẵn (Shift+S, đúng event `doExportIdf` đã dùng — không viết luồng xuất mới). `CadSheets.tsx` nghe `cad:force-save-request` → `saverRef.current.flush()` (ép ghi ngay, bỏ qua debounce ~1.2s) + `setStatus('Đã lưu — HH:MM')` NGAY LẬP TỨC (không đợi promise ghi xong — mục đích là xác nhận thị giác trạng thái hiện tại, không phải theo dõi 1 lần ghi cụ thể, IndexedDB cục bộ gần như luôn thành công tức thời). `lib/save-status.ts` thêm `lastSavedAt` (set trong `onSaved` — CHỈ khi ghi THẬT thành công `bytes>0`, không set trong `onSavingChange(false)` vì nhánh đó cũng chạy khi không có gì để lưu) — dùng CHUNG cho CAD (`CadSheets.tsx`) lẫn Present (`PresentSheets.tsx`, sửa kèm 1 dòng để 2 chặng không lệch trạng thái cùng 1 store). `StatusBar.tsx` (đã tồn tại sẵn từ VIỆC A 28/07, chỉ THIẾU giờ) — thêm `formatHHMM()` + hiện "Đã lưu lúc HH:MM" thay "Đã lưu" trơn, `fontSize:12`/`lineHeight:1.5` ghi đè RIÊNG dòng này (đúng 7.1.23 ⑤c cho phần mới thêm — KHÔNG đụng `fontSize:11.5` chung của cả StatusBar, thuộc đợt sửa token 7.1.23 khác đang chờ Hoà gật). **PHÁT HIỆN khi khám trước khi code**: VIỆC 3 (chỉ báo trạng thái) ĐÃ CÓ SẴN gần như đủ (`useSaveStatus`/`StatusBar.tsx` từ VIỆC A) — chỉ thiếu đúng phần giờ HH:MM, không phải xây từ đầu như brief mô tả; và trích dẫn "auto-backup.ts:266 có sẵn hook onSaved" trong brief bị lệch dòng (hook `onSaved` thật nằm ở `lib/sheets-persist.ts`, không phải `auto-backup.ts` — dùng đúng chỗ thật, không theo số dòng brief nêu). Verify browser thật (`127.0.0.1:3000`): dispatch KeyboardEvent với `ctrlKey` set đúng (computer-tool mô phỏng tổ hợp phím không set `ctrlKey` chuẩn — phát hiện qua debug, không phải bug code) → tool KHÔNG đổi (an toàn, input-guard/preventDefault đúng) + status "Đã lưu — HH:MM" hiện đúng; Ctrl+Shift+S chạy THẬT luồng xuất, thấy "Đã xuất project.idf…" trên command line; focus vào ô lệnh rồi bấm Ctrl+S → KHÔNG bắn sự kiện (input-guard đúng). tsc+eslint sạch, `npm test` 0 fail. | không | tin nhắn Hoà 31/07 (GẤP, khám sẵn CadCanvas.tsx:1942-1965/PhotoEditor.tsx:71/auto-backup.ts:10) |
...
| 2.1.9.i | Vùng tô: màu (hiển thị) ≠ vật liệu (dữ liệu matId→hãng/mã/giá) — 4 chế độ hiển thị chung dữ liệu | N | ⬜ chỉ 1 kiểu vẽ hatch, chưa có 4-chế-độ. ✅ **CHỐT 30/07** — câu hỏi "matId nối vào đâu": `MaterialDef` (`lib/cad/materials.ts`, VẼ TEXTURE) CHỈ có 1 field neo `atlasRecordId?: string` — **CODE XONG 30/07 khuya** (`2.1.9.r`), trỏ sang `ProductSpec.larkRecordId`. **SỬA lại nửa sau (30/07 khuya, `2.1.9.r`)**: "bảng Prisma riêng AtlasMaterial" ban đầu → **MỞ RỘNG `ProductSpec{kind:'material'}` có sẵn** (Luật Đồng Bộ #6 — khám ra `ProductSpec` đã cùng hạt dữ liệu, Q-L2 trước đó đã chốt "1 bảng duy nhất") — xem `2.1.9.r` chi tiết đủ. Lý do gốc vẫn đúng: texture đổi theo thiết kế, giá đổi theo NCC — 2 nhịp sống khác nhau, KHÔNG trộn vào `MaterialDef`. Quyết định này mở khoá `2.1.9.p` (BOQ) — xem đó. | 4.7, 2.2.9 Material Pipeline; 2.1.9.r | `SPEC-SEMANTIC-MODEL.md` |
...
| 2.1.9.n | Present — liên kết sống về CAD (sửa bản vẽ → deck tự cập nhật) — moat | L | 🟡 **đính chính 01/08** (nhãn cũ sai — `linked-assets.ts` ĐÃ CHẠY THẬT, không phải "chưa xác minh"): `setLinkedAssetSrc()`/`attachElementToAsset()` nối vào `PresentEditor.tsx::onAddImageUrl` — chèn cùng ảnh Render (id ổn định `render:<nodeId>`) vào nhiều slide ⇒ tự hội tụ về 1 asset, sửa 1 nơi cập nhật mọi nơi. Cái THẬT SỰ còn thiếu (⬜, khác hẳn): mở rộng nguồn liên kết từ ẢNH sang **BẢN VẼ CAD** (`cad:<sheetId>`, cùng khuôn `render:<nodeId>`) — xem `docs/NGHIEN-CUU-PRESENT-VS-DOI-THU-2026-08-01.md` §4 | 2.1.9.a | `SPEC-SEMANTIC-MODEL.md` |
...
| 2.1.9.q | Groundwork đo bóc hình học cho BOQ — `polygonPerimeter(poly, edgeMask?)` (`hatch.ts`, cùng phong cách `polygonArea`, `edgeMask` vì "nẹp cạnh chỉ lấy cạnh biên") + trừ lỗ mở Đường A (entity `ElementType.door`/`.window` đã phân loại nằm trong polygon, ngưỡng m² vào CONFIG không hard-code) | N | ✅ done (30/07 khuya) — `polygonPerimeter(poly, edgeMask?)` thêm vào `hatch.ts` cùng phong cách `polygonArea` (shoelace), `edgeMask[i]===false` bỏ cạnh `i`, mặc định lấy hết. `openingsAreaInPolygon(entities, poly, thresholdM2?)` — đi Đường (a) khuyến nghị của review: trừ theo entity đã phân loại `elementType==='door'|'window'` có `.at` nằm trong polygon (`pointInPolygon`, TÁI DÙNG), KHÔNG dò ring con hình học. Ngưỡng `BOQ_OPENING_MIN_AREA_M2=0.5` vào CONFIG. **PHÁT HIỆN khi khám code (Luật #4/#5) — spec review giả định sai**: `docs/REVIEW-SPEC-BOQ-LARK-2026-07-30.md` §2② ví dụ "cửa 900×2200=1,98m²" ngầm coi `BlockDef.h` là chiều cao cửa thật, nhưng đọc `furniture.ts:599-619` xác nhận `h` là ĐỘ SÂU MẶT BẰNG (vd `door(900)` có `h:900` = bán kính cung mở cửa vẽ trên mặt bằng, `window` có `h:100` = độ dày tường tại vị trí cửa sổ) — dùng thẳng `w×h` sẽ ra số SAI (0,81m² thay vì ~1,89m² cho cửa 900). Sửa: dùng `w` (rộng cửa/sổ, dữ liệu thật từ `BLOCK_MAP` qua `blockInfo()` export từ `schedule.ts` — TÁI DÙNG Luật #6) × `OPENING_STANDARD_HEIGHT_MM` (CONFIG chuẩn nghề mới — door 2100mm cùng dải `ANCHOR_CONFIG.door` của `2.2.87`, window 1500mm — Luật #10, không đoán mò theo từng block). 17 test mới (`hatch.test.ts` [9][10]: chu vi đầy đủ/từng cạnh/edgeMask rỗng · cửa trong/ngoài polygon · cộng dồn nhiều lỗ mở · bỏ qua furniture · ngưỡng CONFIG điều khiển được · block lạ không đoán mò) + 28 test cũ cùng file không hồi quy (45/45), toàn bộ 100+ test khác trong repo sạch, tsc+eslint sạch. | 2.1.9.p (BOQ dùng nó) | `docs/REVIEW-SPEC-BOQ-LARK-2026-07-30.md` §2 |
| 2.1.9.r | ATLAS Material cache + adapter pull-only (Sprint BOQ ĐỢT 2, 30/07) — `ProductSpec` mở rộng 6 field thương mại + route `/api/atlas-materials/sync` | N | ✅ done (30/07 khuya) — **PHÁT HIỆN khi khám trước khi code (Luật #4/#5)**: chỉ đạo gốc yêu cầu bảng Prisma `AtlasMaterial` riêng, nhưng schema đã có `ProductSpec{kind:'material'}` với ghi chú "Q-L2: GỘP MaterialRef vào đây, 1 bảng duy nhất" — CÙNG hạt dữ liệu (không `projectId`, có `larkRecordId`+`raw`+`syncedAt` y hệt mẫu `LarkTaskRef`). Báo Hoà qua `AskUserQuestion` (Luật Đồng Bộ #6 — không để 2 tính năng gần nhau tồn tại rời rạc) — **xác nhận mở rộng `ProductSpec`, KHÔNG tạo bảng mới**. Thêm 6 field: `unit`/`priceVnd`(Decimal, KHÔNG Float — tránh sai số nhị phân khi tính BOQ)/`wastagePercent`(Decimal)/`packagingSpec`/`altSku`/`styleTags`(JSON string[], cùng mẫu `materials`/`finishes`) — `priceNote` (text tự do) GIỮ NGUYÊN song song `priceVnd` (số thật): `priceVnd=null` ⇒ BOQ phải hiện "chưa có giá", KHÔNG parse `priceNote` sang số (sai 1 dòng = sai tiền thật âm thầm). `npx prisma db push` (KHÔNG `migrate dev` — phát hiện drift giữa `prisma/migrations/` và `dev.db` thật do repo dùng `db push` cho local từ trước, `migrate dev` đòi RESET toàn bộ DB — huỷ ngay, xác nhận `dev.db` nguyên vẹn 10 User/4 Project/10 ProductSpec trước/sau). `lib/integrations/providers/lark.ts`: `listAllRecords()` thêm tham số `pageSize` (mặc định 100, TÁI DÙNG cho cả task/HR cũ) + `listAtlasMaterialRecords()` mới (page_size 500, đòi `LARK_ATLAS_MATERIAL_TABLE_ID` — KHÔNG có default như 2 bảng cũ, chưa từng verify bảng này). `lib/lark/atlas-material-map.ts`: `mapAtlasRecordToProductSpec()` thuần, `ATLAS_FIELD_NAMES` ghi rõ **PLACEHOLDER chưa xác minh** (đoán theo thuật ngữ Hoà, không phải field_name thật qua MCP) — `raw` JSON luôn giữ bản gốc để sửa map + re-sync không mất dữ liệu. Route `/api/atlas-materials/sync` (POST, pull-only, `getSessionUser()` dòng đầu) mirror đúng `/api/lark-tasks/sync`. **`docs/SPEC-THU-VIEN-D-2026-07-30.md` §2 sửa**: "Một thư viện, ba họ" KHÔNG cần model union mới — `ProductSpec` đã có đủ 3 khoá nối (`imageAssetId`→họ A, `drawingBlock`→họ B, `larkRecordId`→họ C), là KHUNG NHÌN join 3 bảng, không phải bảng mới. Test: 22/22 (`atlas-material-map.test.ts` — map đủ field/thiếu field ra null không bịa/giá dạng chuỗi có dấu phẩy/nhiều kiểu dấu phân cách styleTags), toàn bộ suite `npm test` sạch (0 fail). ⚠️ **CHƯA chạy thật** — chặn ở `atlasConfigured()` (thiếu 3 khoá Lark) + `LARK_ATLAS_MATERIAL_TABLE_ID` (chưa có), field mapping cần đối chiếu lại khi có bảng thật. | 2.1.9.i (✅ chốt hướng 30/07 — sửa lại thành mở rộng ProductSpec, không bảng riêng) | tin nhắn Hoà 30/07 khuya (ĐỢT 2 sprint BOQ) |
...
| 2.1.11 | Sprint "Lộ nền" VIỆC 2+3+4 — 3 thao tác chuẩn CAD đã có ở Present nhưng thiếu ở CAD (`docs/KIEM-NEN-2026-07-31.md` mục 🔴②③④): ⌘A chọn tất cả · ⌘D nhân bản · ⌘0/⌘=/⌘- zoom discoverable | N | ✅ done (31/07) — cả 3 việc trong `CadCanvas.tsx`, 1 khối liền kề (xem `2.1.11.a`/`.b`/`.c`). **Lệch nghiệm thu đã disclose**: gộp 3 việc vào **1 commit** thay vì 3 riêng như yêu cầu — lý do: cả 3 chèn liên tiếp cùng 1 vị trí file (ngay sau nhánh ⌘V có sẵn), `git diff` ra ĐÚNG 1 hunk liền mạch (0 dòng code cũ ở giữa để tách), tách thủ công cần viết patch tay dưới áp lực thời gian, rủi ro hỏng file cao hơn giá trị tách commit — mỗi việc vẫn có comment mã riêng (`2.1.11.a`/`.b`/`.c`) để tra/revert theo dòng nếu cần sau này. tsc+eslint sạch (1 warning pre-existing dòng 786, xác nhận qua `git stash` không phải lỗi mới), test 0 fail, `npm run build` thành công. Browser-verify thật qua `window.__cadStore` (thêm 2 entity thật, dispatch KeyboardEvent metaKey đúng — `computer` tool không tạo đúng `metaKey`, tiền lệ `2.1.8.n`): ⌘A chọn đúng 2/2 + status đúng số ĐÃ chọn (không phải số CŨ trước khi lọc layer khoá) · ⌘D ra 4 entity + selection trỏ 2 entity MỚI + status đúng · ⌘=/⌘-/⌘0 đổi scale đúng hướng (×1.2 / ÷1.2 khớp lại giá trị gốc / nhảy hẳn sang scale fit-to-extents khác hẳn). | 2.2.89 (không trực tiếp — cùng file `CadCanvas.tsx`, không phụ thuộc chức năng) | `docs/KIEM-NEN-2026-07-31.md` |
| 2.1.11.a | ⌘A chọn tất cả entity trên TỜ HIỆN TẠI (không phải toàn tài liệu nhiều tờ) | N | ✅ done (31/07) — tái dùng `st.select()` có sẵn (tự loại entity layer khoá/ẩn), không viết luồng chọn mới. `CadCanvas.tsx` ngay sau nhánh ⌘V. | 2.1.11 | " |
| 2.1.11.b | ⌘D nhân bản đối tượng đã chọn | N | ✅ done (31/07) — CHỈ nối phím vào `cadMenuDuplicate()` đã có sẵn (dùng cho menu chuột phải "Nhân bản") — không viết luồng nhân bản mới, đúng yêu cầu "tuyệt đối không viết luồng mới". | 2.1.11 | " |
| 2.1.11.c | ⌘0 vừa khung (discoverable) · ⌘=/⌘- phóng to/thu nhỏ | N | ✅ done (31/07) — **PHÁT HIỆN khi khám (Luật #4/#5)**: brief gốc nói CAD "chưa có phím/nút Vừa khung" — SAI, khám ra đã có SẴN phím `f` (`CadCanvas.tsx`, `zoomExtents()`) VÀ nút toolbar "Zoom Extents (F)" (`CadToolbar.tsx`, event `cad:zoom-extents` wired đủ khắp app — `CadEditor.tsx`/`SchedulePanel.tsx`/`CadSheets.tsx`/`AiBriefPanel.tsx`) — chỉ thiếu `⌘0` kiểu combo quen thuộc, KHÔNG xoá `f`, thêm song song. `⌘=`/`⌘-` dùng `zoomAt()` có sẵn (`lib/cad/model.ts`), neo tâm màn hình, factor 1.2/÷1.2 — cùng khuôn `PhotoEditor.tsx`(⌘0)/`PresentEditor.tsx`(⌘=/⌘-). | 2.1.11 | " |
...
| 2.2.70 | Hệ minh hoạ nét — 45 bìa SVG inline (`stroke-width:1.5`, không gradient/bóng/icon mua sẵn), ưu tiên 24 thẻ hay dùng | N | ⬜ 0 kết quả `stroke-width` trong `lib/render-studio/` — chưa có bộ SVG bìa nét riêng cho thẻ node. **2 lỗi nhỏ phát hiện KÈM lúc làm việc này, ĐÃ SỬA riêng (30/07 khuya, `df74551`)** — không phải nội dung chính `2.2.70`: (a) `RenderToolModeOverlay.tsx` báo động giả "còn node khác đang ẩn" khi `detectGraphPattern()` trả `'complex'` cho cả trường hợp 1 node mồ côi — sửa chỉ hiện khi `nodeCount > 1`; (b) mô tả node `ai.upscale` gắn cứng "khổ A3" — sửa chung chung. Phần chạy AI thật (5 node lấy ảnh thật để dựng 24 bìa ưu tiên) VẪN CHỜ Hoà duyệt credit, tách riêng khỏi 2 fix trên. | 7.1.18 (phần "còn lại") | `docs/SPEC-TONG-COWORK-2026-07-29.md` §6.2 |
...
| 2.2.79 | Renderer Tool Mode 3 dải màn (<8" 1 cột / 8"-laptop 2 cột / desktop 3 cột — dải màn ≠ lựa chọn giao diện, 2 trục độc lập) | N | 🟡 mới có gate nhị phân `useIsSmallScreenForCanvas()` (`lib/render-studio/tool-mode-ui.ts:114`, nay đọc `BP.md`=768 sau `7.1.20`) khoá/mở Node Mode — chưa có renderer 3-dải/3-cột thật. 🔴 **THÊM 30/07 (`7.1.20` bước 4 phát hiện)**: `ToolModeForm.tsx:138` `gridTemplateColumns: '1fr 1fr'` CỨNG, 0 breakpoint nào — 2 cột ẢNH GỐC+KẾT QUẢ sẽ chật trên điện thoại/tablet hẹp (<768px), đúng khoảng trống mã này phải lấp. GỘP vào đây, không cấp mã riêng (đã có mã đúng chỗ). | 7.1.18 (phần "còn lại") | `docs/SPEC-TONG-COWORK-2026-07-29.md` §5.2 |
...
| 2.2.86 | ⭐ **ĐỔI PHƯƠNG ÁN 30/07 (Hoà chốt, GHI ĐÈ mô tả cũ "pill nổi trên canvas" — xem CHANGELOG lý do đổi)**: "Chạy flow" XOÁ HẲN khỏi headbar (không di chuyển đi đâu). Tách KHỞI CHẠY khỏi THEO DÕI — khởi chạy cạnh ĐỐI TƯỢNG (▶ trên node, "Kết xuất" thẻ Tool Mode, "Run flow" Command Palette — cả 3 đã có), theo dõi ở menu "Việc" = HÀNG ĐỢI THẬT (đơn vị `FlowRun`, không phải node lẻ), tuần tự tuyệt đối 1 lượt/lúc, bấm khi bận → XẾP HÀNG (không từ chối), huỷ được (đang chờ = xoá hẳn; đang chạy = dừng ở ranh giới node kế tiếp) | N | ✅ done (checklist 6 bước đủ, 30/07) — `FlowRun` type mới (`lib/types.ts`: `id·label·nodeIds·status·queuedAt·currentIndex·stopOnFirstFailure·snapshotOnStart·cancelRequested`), store thêm `flowRuns[]`+`enqueueFlowRun()`+`updateFlowRun()`+`requestCancelFlowRun()` (`lib/store.ts`), XOÁ `isRunningFlow`/`setRunningFlow` (chết, thay hẳn bởi `flowRuns`). `lib/execution.ts`: `execNode()` GIỮ NGUYÊN 100% (không đụng, đúng yêu cầu) — thêm hàng đợi FIFO (`drainQueue()`/`executeRun()`) điều phối BÊN NGOÀI nó; `runNode()`/`runFlow()` đổi từ chạy-ngay-hoặc-im-lặng-từ-chối (`if (isRunningFlow) return`) sang LUÔN xếp hàng qua `enqueueRun()`, giữ đúng khác biệt hành vi cũ (`stopOnFirstFailure`: node ▶ dừng cả chuỗi khi 1 node lỗi · Run flow bỏ nhánh lỗi chạy tiếp nhánh khác). `estimateRunCredit()` mới — tổng `creditCost` node CHƯA `done` (xấp xỉ có chủ đích, hash thật chỉ biết lúc chạy). `TasksDropdown.tsx` viết lại — 3 nhóm Đang chạy/Đang chờ (có thứ tự)/Đã xong, mỗi dòng hiện tiến độ `N/M node` hoặc `Đứng thứ N`, credit ước tính, nút Huỷ. `MobileMenu.tsx`+`StatusBar.tsx` cũng đổi từ đếm `jobs[]` (node lẻ) sang `flowRuns[]` (Luật Đồng Bộ #6 — 2 mặt tiền suýt lệch đơn vị đếm, phát hiện + sửa kèm). Verify: store actions (enqueue/update/cancel) đúng qua `window.__flowStore` — huỷ mục đang chờ xoá hẳn khỏi mảng, huỷ mục đang chạy chỉ đặt cờ `cancelRequested`; UI đúng — badge "Việc2", 2 nhóm "Đang chạy"/"Đang chờ" hiện đúng, dòng hiện "1/2 node"/"Đứng thứ 1 · 1 node". Header 5 breakpoint (640·768·1024·1180·1440, Luật #11): 1024/1180/1440 sạch (0 tràn); 640/768 VẪN tràn (179px/51px) — xác nhận đây là gap `sm:flex` cụm phải đã ghi trong `7.1.20` bước 4, KHÔNG phải do việc này gây ra (xoá "Chạy flow" giảm tải nhưng không đủ đóng hẳn gap ở 2 mốc nhỏ) — theo đúng "đừng sửa hết một lượt" của `7.1.20`, để nguyên chưa sửa. 103/103 test, tsc+eslint sạch. | 7.1.20 (✅ — không còn phụ thuộc `2.1.8.l` sau khi đổi phương án, bỏ pill nổi ⇒ bỏ luôn nhu cầu safe-area trước khi làm mã này) | `docs/TICKET-CHAY-FLOW-KHONG-GHIM-BAR-2026-07-30.md` (chỉ phần chẩn đoán §2, BỎ phần giải pháp §5 pill nổi — Hoà chốt phương án khác 30/07) |
| 2.2.87 | ⭐ Engine đo món đồ từ 1 ảnh (single-view metrology) — **Lát cắt 1** (`docs/TU-VAN-ANH-SANG-BAN-VE-2026-07-30.md` §4): CHỈ bước ①②③⑦ của dây chuyền 7 bước, KHÔNG dựng mesh 3D (⑤), KHÔNG sinh góc phối cảnh mới (⑥). Thuần lib, 0 credit mặc định, tất định, KHÔNG UI (mặt tiền ở `2.2.88`) | N | 🟡 **HẠ TỪ ✅ (Hoà chốt 30/07 khuya)**: "46 test đơn vị sạch NHƯNG đường hiển thị end-to-end chưa verify — chưa có ảnh thật nào chạy thông ra số trên màn hình. Tính năng này xuất số đi tới xưởng; lỗi tầng hiển thị gây thiệt hại tiền thật mà test đơn vị không bắt được. Nâng ✅ khi Hoà xác nhận 1 ảnh thật chạy đúng." — bản gốc (checklist 6 bước, 30/07) dựng đúng engine điểm tụ (`calibrateFromVanishingPoints()`/`anchorScale()`/`measureObject()`, xem lịch sử git cho chi tiết công thức Caprile-Torre + `ANCHOR_CONFIG` 8→9 nguồn neo), NHƯNG bắt lỗi trung thực khi ảnh thiếu cạnh kiến trúc thẳng 2 phương (phổ biến ở ảnh render đẹp — rèm/thảm cong/sáng mềm) ⇒ Hoà nhận lỗi thiết kế gốc ("bắt đi đường điểm tụ"), yêu cầu **SỬA NGAY cùng ngày**: "Bấm Render KHÔNG BAO GIỜ được trả về tay không." **SỬA (30/07 khuya, cùng ngày)** — thêm cơ chế **4 BẬC tự tụt, KHÔNG BAO GIỜ throw vì thiếu cấu trúc/neo** (chỉ throw khi ảnh thật sự hỏng — CORS/corrupt, ở tầng gọi `metrology.ts` node, không phải trong hàm này): Bậc 1 = dải chuẩn nghề theo loại đồ (`FURNITURE_SIZE_PRIORS`, 14 loại, ~50% tin, ĐÁY không bao giờ fail) → Bậc 2 = tỉ lệ khung bao mặt nạ tinh chỉnh Bậc 1 (~65%) → Bậc 3 = neo tay 2-điểm-khoanh (`tierManualAnchor`, vật chuẩn bất kỳ trong khung — `ANCHOR_CONFIG` thêm `bed` 1500-2000mm "mốc rất đáng tin") hoặc người dùng tự xác nhận 1 kích thước của chính món (`tier3`, ~80%) → Bậc 4 = engine điểm tụ gốc không đổi (`tryTier4`, ~90%, chỉ chạy khi CÓ đủ mặt nạ+ảnh, trả `null` không throw nếu thiếu cấu trúc, caller tự tụt Bậc 3). Điều phối bởi `measureObjectTiered()` — hàm DUY NHẤT node gọi, không bao giờ trả null. **"Phương pháp" (bậc 1-4, đo chất lượng số liệu) TÁCH BẠCH KHỎI `AiTier` hệ thống** (`lib/ai/tiers.ts`, 1-4 chọn provider/chi phí) theo đúng yêu cầu Luật #6 (không dựng 2 mặt quản chi phí) — kể cả `aiTier=1` "Không AI" vẫn đạt phương pháp Bậc 3/4 nếu ảnh/thao tác cho phép. Test: **46/46 pass** (28 gốc giữ nguyên chứng minh toán đúng + 18 mới cho cascade: ảnh mềm không cạnh thẳng vẫn ra số, nền trắng+1 vật → Bậc 1-2, gọi chỉ với `category` (0 ảnh/mặt nạ/neo) vẫn ra số hữu hạn hợp lệ, neo tay 2 điểm → Bậc 3 đúng `kind:'measured'`). ⚠️ **CHƯA LÀM, disclose rõ không giấu** (Hoà yêu cầu ánh xạ thêm AI vào Bậc 1-2/tìm-neo-tự-động — nằm ngoài phạm vi engine thuần đợt này): Tầng 2 (oneAI/ComfyUI tự tìm vật neo qua object-detection+bbox) và Tầng 3 (VLM tự ước category/tỉ lệ) của bản đồ 4-AiTier — chưa tìm thấy năng lực object-detection-có-bbox nào qua oneAI/ComfyUI trong codebase hiện tại; VLM sẵn có (`captionImage()`, `lib/ai/providers/nvidia.ts`) tách biệt kiến trúc khỏi hệ `fal`-tier Hoà kỳ vọng gắn vào — cần hạ tầng riêng, để đợt sau. tsc+eslint sạch cả 2 lần sửa. | không | `docs/TU-VAN-ANH-SANG-BAN-VE-2026-07-30.md` |
| 2.2.88 | Mặt tiền "Đo món đồ" cho `2.2.87` — thẻ Tool Mode chặng 2 thứ 7, hiện R×S×C 🟢/🟡 phân màu, xuất SPEC SHEET (dấu cảnh báo bắt buộc) | N | 🟡 **HẠ TỪ ✅ (Hoà chốt 30/07 khuya, cùng lý do `2.2.87`)**: "46 test đơn vị sạch NHƯNG đường hiển thị end-to-end chưa verify... lỗi tầng hiển thị gây thiệt hại tiền thật mà test đơn vị không bắt được. Nâng ✅ khi Hoà xác nhận 1 ảnh thật chạy đúng." — bản gốc (30/07) dựng node `vision.measureobject` (`lib/nodes/defs/metrology.ts`, 0 credit, TÁI DÙNG `extractForeground()` — CHÍNH hàm `ai.furnitureextract` gọi ở tầng lõi, Luật #6), `MeasurementPanel`/xuất spec sheet — verify UI thật xác nhận đường LỖI TRUNG THỰC chạy đúng qua `runNode()`, nhưng CHƯA verify được đường ĐO THÀNH CÔNG bằng ảnh thật. **SỬA CÙNG ĐỢT VỚI 2.2.87** (30/07 khuya) khớp cascade 4 bậc mới: `ToolModeForm.tsx` viết lại — (1) `category` hiện chính, `cameraHeightMm`/`bgTolerance` gom vào mục **"Tinh chỉnh" thu gọn, mặc định ĐÓNG** (trước bày cả 2 slider ra trước khi chạy — Hoà chỉ ra là sai, "bắt người dùng chỉnh tham số của thứ chưa chạy"); (2) **UI khoanh tay 2-điểm mới** trên ảnh gốc — bấm 2 điểm → chọn loại vật chuẩn (`ANCHOR_CONFIG`, có `bed`) + nhập mm thật → "Dùng vật chuẩn này, đo lại" ghi `manualAnchorJson` qua `store.updateParam()` (param KHÔNG khai trong `NodeDefinition.params`, tránh lộ ra vòng lặp render tham số chung) rồi chạy lại ngay; toạ độ quy đổi từ px màn hình → px ảnh gốc có trừ letterbox `object-fit:contain`; (3) `MeasurementPanel` hiện dòng bắt buộc **"Tầng N · <tên tầng AI> · Bậc M · độ tin K%"** + `upgradeHint` khi `needsManualAnchor` — KHÔNG BAO GIỜ chỉ hiện chữ đỏ dừng lại (đúng nguyên tắc "không tay không"), dấu cảnh báo "Mặt khuất là suy diễn" vẫn bắt buộc không tắt được. `metrology.ts` node rewrite: parse `manualAnchorJson` phòng thủ (JSON hỏng → bỏ qua, không throw), gọi `measureObjectTiered()` (2.2.87), payload thêm `aiTier`/`aiTierName` (chỉ để GHI NHÃN hiển thị, không điều khiển bậc). `measurement-spec-sheet.ts` sửa theo: nhận `TieredMeasurement` + `methodLine` string dựng sẵn ở caller (không dựng lại logic ghép chuỗi lần 2). Verify: tsc sạch, eslint sạch (chỉ 2 warning `<img>` có sẵn từ trước, không phải lỗi mới), 46/46 test + toàn bộ 100+ test khác trong repo chạy sạch không hồi quy, browser thật xác nhận layout tĩnh đúng yêu cầu (category hiện chính, "Tinh chỉnh" đóng mặc định, nút Render tắt khi chưa có ảnh). ⚠️ **CHƯA verify được** đường khoanh-tay-2-điểm + `MeasurementPanel` hiện số thật trong browser — môi trường test không có cách đưa file ảnh thật qua `<input type=file>` (đã thử: gán `files` qua native setter + dispatch `change`, không kích hoạt được handler React; điều hướng canvas để chạy node thủ công qua `window.__flowStore` cũng không thao tác được do panel node bị khoá zoom trong demo project) — bù lại bằng: đối chiếu kiểu dữ liệu TỪNG TRƯỜNG (`TieredMeasurement`/`ANCHOR_CONFIG`/`MeasurementValue`) với export thật trong `single-view-metrology.ts` (khớp chính xác), tsc/eslint sạch, 46 test đơn vị phủ đúng hàm `measureObjectTiered()` mà node gọi (kể cả nhánh neo tay). Khuyến nghị Hoà tự thử 1 ảnh thật trong browser trước khi tin số đầu tiên hiện ra. | 2.2.87 | `docs/TU-VAN-ANH-SANG-BAN-VE-2026-07-30.md` |
| 2.2.89 | Menu chuột phải CAD (`Popover` dùng ở `CadCanvas.tsx`) không có đường thoát nào — bấm ra ngoài không đóng, Escape cũng không, chỉ thoát được bằng bấm trúng 1 mục | N | ✅ done (31/07, ĐỢT DEMO GẤP, ƯU TIÊN 1) — `components/ui/Popover.tsx` thêm prop TUỲ CHỌN `onDismiss?` (chép khuôn `MenuButton.tsx`: document `mousedown` + `keydown` capture, cleanup đủ) — 2 nơi dùng cũ (`FlowCanvas.tsx`/`present-editor/EditorCanvas.tsx`) KHÔNG truyền prop nên hành vi y hệt trước (guard `if (!onDismiss) return`, đã browser-verify code không đổi qua tsc/eslint/build sạch). `CadCanvas.tsx:3221` truyền `onDismiss={() => setCadMenu(null)}`; `CadCanvas.tsx:1993` (nhánh Escape) thêm `setCadMenu(null)` bị quên trước đó. **Bẫy "chính cú chuột phải mở menu bị listener mới bắt và đóng ngay"** — xử lý CẢ HAI lớp theo đúng yêu cầu Hoà: (a) bỏ qua nếu `e.target` nằm TRONG popover (`ref.contains`) — cần vì listener nghe `mousedown` còn Popover tự chặn `pointerdown` lan lên, 2 loại event khác nhau không tự loại nhau; (b) cờ `armed` chỉ bật sau `requestAnimationFrame` — mousedown trong cùng tick mở menu bị bỏ qua dù listener đã gắn. Test tay bắt buộc (dispatch `PointerEvent`/`MouseEvent` button:2 qua JS vì `computer` tool không tạo đúng `button:2`, ghi chú tiền lệ ở `2.1.8.n`): chuột phải → menu hiện VÀ Ở LẠI (không nháy) ✅ · bấm ra ngoài → đóng ✅ · Escape → đóng ✅ · bấm mục "Thêm block" → chạy lệnh VÀ đóng, không hồi quy ✅ · chuột phải chỗ khác khi đang mở → 1 menu duy nhất nhảy vị trí mới (`document.querySelectorAll('[role="menu"]').length === 1`), không nhân đôi ✅. tsc+eslint+test sạch, `npm run build` thành công. | không | báo cáo trực tiếp Hoà 31/07 |
| 2.2.90 | Nợ lại từ `2.2.89`: rút hook dùng chung `useDismissable` (mousedown-outside + Escape-capture) cho `MenuButton`/`IOMenu`/`TextToolbar`/`AiBriefPanel`/`VitalsGesture`/`Popover` dùng chung — hiện mỗi chỗ tự viết, đúng gốc "chỗ được chỗ không" | N | ⬜ chưa làm — Hoà yêu cầu KHÔNG làm trong ĐỢT DEMO 31/07, để sau buổi thử | 2.2.89 | — |
| 2.2.92 | Overlay "Chọn việc muốn làm" (màn mặc định chặng Render, `ToolModeHome.tsx:24`, `position:absolute` phủ kín canvas + `zIndex: 35`, cố ý để che `FlowCanvas.tsx` toast z-30/empty-state z-10 — xem comment tại chỗ) ĐÈ LÊN LUÔN popover "Tệp" của `RenderIOMenus.tsx` dù popover đó khai `zIndex: 40` (40 > 35) — vì popover đó render CỤC BỘ (`position:absolute` bên trong header, KHÔNG portal ra `document.body` như `components/ui/Popover.tsx` đã làm đúng cho đúng lớp bệnh này), so z-index diễn ra ở tầng ancestor khác/paint-order khác chứ không phải so trực tiếp 40 với 35 trong cùng 1 stacking context — `elementFromPoint` xác nhận phần tử z=35 nằm TRÊN tại toạ độ popover. Phát hiện khi browser-verify `2.2.90` ĐỢT 1 (menu "Tệp" mở nhưng không thấy trên màn hình dù DOM có `role="menu"` đúng vị trí). **Mở rộng khi verify ĐỢT 2 (01/08)**: CÙNG BỆNH xảy ra với `MoreMenu`/`UserChip`/Tasks dropdown trong `AppChrome.tsx` (cũng `position:absolute` cục bộ, không portal) — bị overlay đè y hệt khi mở trên màn mặc định Render; đã né được để verify bằng cách chuyển sang CAD/Present (không có overlay này) — KHÔNG PHẢI do ĐỢT 2 gây ra, pattern cũ (backdrop) cũng sẽ dính y vậy, chỉ là chưa ai để ý vì backdrop che hết cả trang nên hiếm khi mở popover khi đứng trên màn mặc định Render | N | ⬜ chưa làm — chỉ xảy ra khi canvas Render trống (màn mặc định) và popover mở đè lên nó; vá bằng cách portal hoá `RenderIOMenus.tsx`/`MoreMenu`/`UserChip`/Tasks dropdown giống `Popover.tsx`, hoặc hạ `zIndex` overlay xuống dưới 1 stacking context chung — CẦN Hoà quyết hướng, chưa tự chọn | 2.2.90 | báo cáo trực tiếp Hoà (không có file spec gốc riêng) |
| 2.2.91 | Hành vi nguyên liệu **Toolbar** dùng chung (P0, `docs/CHOT-HUONG-SAU-DEMO-2026-07-31.md` — 1 trong 4 nguyên liệu nền `Popover`·`Menu`·`Panel`·`Toolbar`, hạt giống `2.2.90`) — thanh công cụ nổi của phần tử đang chọn KHÔNG tự thu khi kéo, bám theo con trỏ và ĐÈ lên vùng đang thao tác (Hoà phát hiện khi tự xem ảnh chụp lúc verify roundtrip `4.1.b`/B2, KHÔNG phải bug do B2 gây ra — hành vi toolbar có sẵn từ trước). BA TRẠNG THÁI bắt buộc: (1) đã chọn + đứng yên → hiện đầy đủ; (2) đang kéo di chuyển HOẶC đang kéo tay nắm co giãn → THU LẠI (mờ đi); (3) vừa thả → bung lại, tính lại vị trí mới. BỐN CHI TIẾT BẮT BUỘC: ① ngưỡng kéo ~4px trước khi coi là "đang kéo" — ẩn ngay từ `pointerdown` là SAI (bấm để chọn sẽ làm thanh chớp tắt); ② kéo TAY NẮM co giãn (resize handle) cũng phải kích hoạt ẩn, không chỉ kéo di chuyển (move); ③ mờ đi ~80ms / hiện lại ~150ms CÓ TRỄ NHẸ (không trễ → tay rung là nhấp nháy); ④ tính lại vị trí CHỈ lúc THẢ (không phải mỗi khung hình lúc đang kéo) + LẬT XUỐNG DƯỚI phần tử khi phía trên không đủ chỗ. **PHẠM VI BẮT BUỘC**: sửa ở TẦNG NGUYÊN LIỆU dùng chung (hook/component `Toolbar` mới hoặc mở rộng, dùng được cho mọi chặng có phần-tử-nổi-theo-selection), KHÔNG vá riêng trong `components/present-editor/TextToolbar.tsx` — vá riêng thì CAD/Render tự dựng toolbar tương tự sau này lại dính đúng lỗi này lần nữa (đúng bằng chứng "làm lại" mà P0 nêu ra: tràn headbar 3 lần/đóng popover mỗi chỗ tự viết). Hiện diện tại chỉ có ở Present (`TextToolbar.tsx`, neo bởi `EditorCanvas.tsx:518`) — khám kỹ hơn khi bắt tay làm, chưa xác nhận CAD/Render có toolbar-nổi tương tự nào khác cần gộp cùng lúc. | N | ⬜ chưa làm — làm SAU khi ĐỢT B xong (Hoà yêu cầu ghi hàng đợi, đừng cắt ngang B2/B3-B5) | 2.2.90 (cùng nhóm P0 nguyên liệu Toolbar, không phụ thuộc kỹ thuật trực tiếp) | `docs/CHOT-HUONG-SAU-DEMO-2026-07-31.md` P0 |
...
| 2.3.32 | Editor toolkit Deck — align/distribute/khoá tỉ lệ | N | 🟡 align+distribute ✅ đủ dùng (`lib/present-editor/align.ts`, có test riêng) · khoá tỉ lệ khi resize **đính chính 01/08 — ĐÃ CÓ**, không phải "hoàn toàn không có": `Element.tsx:14` + `:225-231`, Shift+kéo góc = giữ tỉ lệ (mọi loại element) | 2.1.10.c | `SPEC-EDITOR-TOOLKIT.md`, `docs/AUDIT-EDITOR-TOOLKIT.md` |
...
| Mã | Tên | Bậc | Trạng thái | Phụ thuộc | File spec gốc |
|---|---|---|---|---|---|
| 4.1 | Cây thư mục thật theo dự án trên đĩa — 🔄 **ĐƯỜNG DẪN CHỐT LẠI 31/07** (`docs/QUYET-DINH-HA-TANG-2026-07-31.md`, thay `~/InteriorFlow/Projects/...` cũ chưa verify bằng `~/InteriorFlow/<mã dự án> — <tên>/` phẳng, không có tầng `Projects/`) | N | 🟡 ĐANG LÀM ĐỢT B (5 pha B1-B5, mỗi pha báo+chờ gật riêng — xem `4.1.a`-`.e`) | không | `SPEC-FILE-MANAGER.md`, `PLAN-LIBRARY-GATEWAY.md` NT5, `docs/QUYET-DINH-HA-TANG-2026-07-31.md` |
| 4.1.a | ĐỢT B — B1: chọn thư mục gốc `~/InteriorFlow` một lần, cất handle. CHƯA đổi nguồn sự thật gì | N | ✅ done (31/07) — `lib/root-folder.ts` (mới) CHÉP KHUÔN `lib/cad/auto-backup.ts:70,78` (`storeHandle`/`loadHandle`, File System Access API + IndexedDB) — IndexedDB/key RIÊNG (`interiorflow-root`/`rootDir`, KHÔNG dùng chung `interiorflow-backup`/`backupDir` của auto-backup — 2 vai trò khác nhau dù có thể cùng 1 thư mục vật lý). `components/settings/StorageSettings.tsx` (mới) — nhóm "Lưu trữ" thứ 5 trong `/settings` (bên cạnh Tài khoản/Giao diện/AI/Trải nghiệm, `7.3.30`), hiện trạng thái đã/chưa chọn + nút chọn/đổi thư mục. `loadRootFolderHandle()` export sẵn (khác `auto-backup.ts` giữ private) cho B2-B4 đọc lại — bản thân export KHÔNG phải đổi nguồn sự thật, chỉ là hạ tầng, chưa nơi nào gọi đọc/ghi dữ liệu dự án. Browser-verify thật: `/settings` hiện đúng nhóm "Lưu trữ", trạng thái ban đầu "Chưa chọn thư mục", nút "Chọn thư mục" có mặt. **Giới hạn công cụ disclose** (cùng B3): `showDirectoryPicker()` đòi hộp thoại OS thật + gesture người dùng, không tự động hoá được trong môi trường phiên này — không bấm thử để tránh treo phiên, xác nhận gián tiếp qua `rootFolderSupported()` (nhánh "chưa hỗ trợ" KHÔNG hiện, đúng vì Chromium test có `showDirectoryPicker`). tsc+eslint+test+build sạch. ⏸️ **DỪNG Ở ĐÂY, CHỜ HOÀ GẬT** trước khi làm B2 (đúng luật "mỗi pha báo, chờ gật, tuyệt đối không chạy thẳng tới B4"). | 4.1 | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` ⑥ |
| 4.1.b | ĐỢT B — B2: định dạng `.idfp` cho chặng 3 (Present) + xuất + nhập | N | ✅ done (31/07) — `lib/present-editor/idfp.ts` (mới) ĐÚNG khuôn `lib/cad/idf.ts` (JSON versioned, `IDFP_VERSION`/migration scaffold/`lastImportIdfpError()`) — serialize NGUYÊN VẸN cả object `EditorDeck` (không hand-pick field) nên tự động mang đủ slide/bố cục/ảnh nhúng dataURL/`customFonts` (font nhúng-theo-deck, dataURL base64 sẵn trong model)/`stagePreset` per-sheet — tất cả ĐÃ NẰM SẴN trong `EditorDeck` (`lib/present-editor/model.ts`), khám trước khi code xác nhận không thiếu field nào. `brandKitSnapshot` — đọc `getActiveBrandKit()` ĐÚNG 1 lần lúc xuất (do UI truyền vào, giữ `idfp.ts` THUẦN không đụng localStorage), nhúng bản CHỤP, không phải id tham chiếu sống — test xác nhận đổi Brand Kit gốc SAU khi xuất không ảnh hưởng file đã xuất. UI: `Toolbar.tsx` (Present) thêm mục "Mở project (.idfp)"/"Toàn bộ project (.idfp)" vào 2 IOMenu Nhập/Xuất có sẵn, bắc cầu CustomEvent `present:idfp-export-request`/`present:idfp-import-request` ĐÚNG pattern `cad:idf-export-request`/`cad:idf-import-request` (không viết cơ chế mới) — `PresentSheets.tsx` (nơi giữ `sheets[]`) lắng nghe, `.idfp` THAY THẾ TOÀN BỘ project như `.idf`, có `window.confirm` hỏi trước (khớp UX Present đã dùng cho Brand Kit `7.1.25`). Toast kết quả DÙNG CHUNG `exportMsg` có sẵn của PDF/PPTX/PNG (CustomEvent `present:idfp-*-done`, không viết toast mới). 41 test mới (round-trip 1/nhiều sheet, font dataURL giữ nguyên byte, brandKitSnapshot không phải tham chiếu sống, file hỏng→null không throw, migration path). **BUG THẬT bắt được khi browser-verify** (không phải suy đoán từ code — đúng lý do Hoà bắt buộc phép thử này thay vì tin unit test): nhập `.idfp` giữ nguyên id sheet trong file (vd tự xuất rồi tự nhập lại) ⇒ `activeId` sau import TRÙNG trước import ⇒ `key={activeId}` của `PresentEditor` không đổi ⇒ KHÔNG remount ⇒ canvas giữ deck CŨ dù tab đã đổi tên đúng (state sheets đổi được, canvas thì không) — thêm `importGen` (state, tăng mỗi lần nhập) ghép vào key `${activeId}:${importGen}` để LUÔN ép remount sau import bất kể id trùng hay không. Verify sau khi sửa: browser thật — xuất `.idfp` (chặn `URL.createObjectURL` bắt blob thật) → xoá sạch `interiorflow-sheets` IndexedDB (đủ 3 DB tên xác nhận) → reload (xác nhận NGUỘI về sample deck, chứng minh xoá thật) → nhập lại nội dung ĐÃ ĐỔI (không phải giống hệt — bài test mạnh hơn "nhập lại y hệt") → canvas hiện ĐÚNG nội dung mới, tên tab đổi đúng, font pairing giữ đúng. Phục hồi lại nội dung gốc sau khi test xong (không để lại rác trong demo account). tsc+eslint+test(41/41 mới, 0 fail toàn repo)+build sạch. | 4.1.a | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` ⑥ |
| 4.1.c | ĐỢT B — B3: `brand-kit.json` ra tệp trong thư mục dự án | N | ✅ done (31/07) — `lib/root-folder.ts` mở rộng (KHÔNG viết cơ chế mới, hạ tầng CHUNG cho B3+B4): `projectFolderName(id, tên)` (khớp sơ đồ Hoà chốt `<mã dự án> — <tên>/`, lọc ký tự OS cấm khỏi TÊN) · `getProjectFolderHandle(id, tên, {create})` (tự tạo thư mục con dưới root nếu chưa có, `ensurePermission()` xin lại quyền readwrite CÙNG khuôn `auto-backup.ts`) · `writeTextFile`/`readTextFile` (CÙNG khuôn `getFileHandle`→`createWritable`→`write`→`close` của `auto-backup.ts`, không throw — trả `false`/`null`). `lib/present-editor/brand-kit-disk.ts` (mới) — cầu nối MỎNG, TÁI DÙNG NGUYÊN hàm THUẦN của `7.1.25` (`exportBrandKitsJson`/`importBrandKitsJson`, không viết lại parse/merge lần 2, đúng ý định ghi sẵn trong comment `BrandKitPanel.tsx` lúc làm `7.1.25`): `writeBrandKitToProjectFolder`/`projectFolderHasBrandKit`/`importBrandKitFromProjectFolder`. UI `BrandKitPanel.tsx`: thêm `useSheetsBucketId()` (Luật Đồng Bộ #6, hook project-id ĐÃ DÙNG bởi `PresentSheets.tsx`/`CadSheets.tsx`, không tự chế hook mới) + `useFlowStore(s => s.flowName)` cho tên dự án; `onSave()` sau khi lưu localStorage xong THÊM ghi bản sao nền vào `brand-kit.json` trong thư mục dự án (fire-and-forget, thất bại im lặng khi chưa chọn thư mục gốc B1/quyền bị từ chối — không được làm gãy thao tác Lưu chính); nút "Nhập từ thư mục dự án" MỚI (nối đúng đường Nhập .json đã có ở `7.1.25`/0b — cùng hộp thoại hỏi ghi đè/gộp, gọi thẳng `importBrandKitsJson()`), CHỈ hiện khi `projectFolderHasBrandKit()` xác nhận file đã tồn tại (không hiện nút dẫn tới lỗi chắc chắn). Browser-verify thật (demo account, `127.0.0.1`): mở Present → Brand Kit panel render đúng · nút "Nhập từ thư mục dự án" ĐÚNG như kỳ vọng KHÔNG hiện (chưa chọn thư mục gốc) · bấm "Lưu Brand Kit" xong không crash, không lỗi console, kit lưu đúng vào danh sách. **Giới hạn công cụ disclose (như `4.1.a`)**: `showDirectoryPicker()` cần hộp thoại OS + gesture người dùng thật, không tự động hoá được trong phiên trình duyệt headless này — vòng ghi→đọc thật trên đĩa (chọn thư mục → Lưu Brand Kit → xác nhận `brand-kit.json` xuất hiện → nút Nhập hiện ra → nhập lại) CHƯA được verify bằng tay thật, code mirror ĐÚNG khuôn `auto-backup.ts` đã chạy sản xuất. Không thêm test file cho `root-folder.ts`/`brand-kit-disk.ts` — ĐÚNG tiền lệ `auto-backup.ts` (0 test, lý do giống nhau: cần API File System Access thật, sucrase-node không giả lập được). tsc+eslint+test(0 fail toàn repo)+build sạch. ⏸️ **DỪNG Ở ĐÂY, CHỜ HOÀ GẬT** trước khi làm B4 (đảo nguồn sự thật, rủi ro nhất). **🔴 SỰ CỐ 31/07 bắt được NGAY SAU BÁO CÁO TRÊN, TRƯỚC KHI GẬT B4** — Hoà thử tay thật: chọn thư mục → Settings hiện đã chọn → bấm "Lưu Brand Kit" → báo "đã lưu" → quét `~/Downloads/interiorflow` KHÔNG có `brand-kit.json` nào, không thư mục dự án nào, bấm lại KHÔNG có hộp xin quyền nào hiện ra ⇒ MẤT DỮ LIỆU IM LẶNG. Verify độc lập (không đoán, không tin lời tự khai cũ): thay `showDirectoryPicker()` thật bằng handle **OPFS thật** (`navigator.storage.getDirectory()`, KHÔNG phải mock — vẫn là `FileSystemDirectoryHandle` thật, structured-clone được, cùng interface) đặt thẳng vào IndexedDB `interiorflow-root`/`rootDir` — xác nhận Settings hiện "đã chọn" giống hệt Hoà thấy, rồi bấm "Lưu Brand Kit" thật qua UI. **Kết quả cô lập chính xác nguyên nhân**: LOGIC ghi/đọc/đặt tên thư mục ĐÚNG 100% (file lên đúng chỗ `<mã dự án> — <tên>/brand-kit.json`, đúng nội dung — OPFS không cần permission nên phép thử này loại trừ hẳn giả thuyết "logic sai") — lỗi nằm TRỌN Ở TẦNG QUYỀN: quyền `readwrite` cấp lúc chọn thư mục RESET về `'prompt'` sau mỗi lần điều hướng/tải lại trang (ĐÚNG đặc tả File System Access API, không phải bug trình duyệt), `requestPermission()` gọi lại không đủ "user activation" mới ⇒ Chrome ÂM THẦM trả `'denied'`, KHÔNG hiện hộp thoại nào — khớp CHÍNH XÁC quan sát "bấm lại không thấy hộp xin quyền" của Hoà. `ensurePermission()` cũ nuốt gọn thành `false`, `getProjectFolderHandle()` cũ trả `null` mập mờ (không phân biệt "chưa chọn thư mục" / "mất quyền" / "lỗi khác"), `BrandKitPanel.onSave()` cũ coi ghi đĩa là "tiện nghi nền" rồi bỏ qua hoàn toàn — không có bất kỳ tín hiệu nào cho người dùng. **SỬA (3 việc bắt buộc, đúng yêu cầu)**: ① `saveBrandKit()` (máy) và ghi đĩa (dự án) tách THÀNH HAI thông báo — `saved` ("Đã lưu Brand Kit (trên máy) ✓", tự tắt nhanh) và `diskMsg` MỚI (banner riêng, không tự tắt nhanh khi lỗi, `role="alert"`) — `getProjectFolderHandle()` đổi từ `null` sang `FolderAccessResult` có LÝ DO (`'no-root'|'no-permission'|'no-project-id'|'error'`), `writeBrandKitToProjectFolder()` (`brand-kit-disk.ts`) trả `WriteBrandKitResult` cùng kiểu — `no-root` (chưa bật lưu trữ) vẫn im lặng ĐÚNG THIẾT KẾ (tính năng opt-in), mọi lý do khác BẮT BUỘC hiện banner đỏ nêu rõ lý do + hướng xử lý. ② Không tách nút "xin quyền" riêng (cân nhắc rồi bỏ, tránh 2 cơ chế xin quyền song song) — thay vào đó banner lỗi `no-permission` trỏ thẳng người dùng tới nút Kiểm tra kết nối (③) ở Settings, ĐÚNG luật "một cỗ máy, nhiều mặt tiền" (`CLAUDE.md`) — nút đó tự nó GỌI `ensurePermission()` với gesture thật của chính cú bấm, vừa kiểm vừa cấp lại quyền cùng lúc. ③ `testStorageConnection()` MỚI (`root-folder.ts`) — ghi 1 tệp `.interiorflow-connection-test` thật vào thư mục GỐC, đọc lại, so khớp token ngẫu nhiên, dọn rác, báo lý do cụ thể (`ConnectionTestResult`); nút "Kiểm tra kết nối thư mục" MỚI trong `StorageSettings.tsx` (B1, hiện khi đã chọn thư mục) gọi hàm này, hiện kết quả ✓/⚠ kèm hướng dẫn cụ thể theo từng lý do thất bại. **RÀ SOÁT CÙNG LỚP** (Hoà yêu cầu, không chỉ vá Brand Kit): `ensurePermission()` (`root-folder.ts`) bọc thêm try/catch quanh `queryPermission()` (trước đây có thể throw unhandled nếu handle hỏng) — B1 và B3 dùng chung 1 hàm này nên sửa 1 chỗ, cả hai cùng được vá; B1 (`4.1.a`) qua đó cũng có LẦN ĐẦU cách verify vòng ghi→đọc thật (nút Kiểm tra kết nối), đóng đúng lỗ hổng "chưa verify" đã tự khai trong `4.1.a`. **Verify lại bằng browser thật, KHÔNG unit test** (đúng yêu cầu, unit test không bắt được lớp lỗi permission-timing này): lặp lại đúng kịch bản OPFS ở trên với code ĐÃ SỬA — Settings "Kiểm tra kết nối thư mục" báo "✓ Ghi/đọc thử thành công", xác nhận OPFS KHÔNG còn sót tệp test (dọn rác đúng) · Present → Nhận diện → "Lưu Brand Kit" → đọc trực tiếp OPFS xác nhận `brand-kit.json` MỚI ghi đúng nội dung (timestamp khớp lượt test) VÀ nút "Nhập từ thư mục dự án" xuất hiện đúng lúc (chứng minh `hasProjectFile` cập nhật theo kết quả ghi thật, không phải giả định). **Giới hạn còn lại, disclose rõ**: OPFS không cần permission nên phép thử này KHÔNG lặp lại được chính xác nhánh "mất quyền sau reload" bằng browser tự động (native `showDirectoryPicker()` + reload thật vẫn cần Hoà bấm tay, như B1 đã disclose) — nhưng bài học cốt lõi (KHÔNG BAO GIỜ báo "đã lưu" khi ghi đĩa chưa chạy) đã sửa ở tầng CODE, không phụ thuộc việc tái hiện đúng nhánh lỗi cụ thể. tsc+eslint+test(0 fail)+build sạch lại sau sửa. | 4.1.b | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` ⑥ |
| 4.1.d | ĐỢT B — B4 ⚠️ PHA RỦI RO NHẤT: đảo nguồn sự thật (tệp thật thành nguồn, IndexedDB tụt xuống cache) + di trú dữ liệu cũ | N | ✅ done (31/07) — kế hoạch trình + Hoà gật kèm 4 bổ sung TRƯỚC khi code (đúng luật). **Hạ tầng chung mới**: `lib/disk-sync.ts` — `resolveSourceOfTruth()` THUẦN (① ngưỡng dung sai `TIE_TOLERANCE_MS=2000` — 2 mốc thời gian không nguyên tử, `modifiedAt` đặt lúc serialize khác `IndexedDB.ts` đặt lúc ghi record, lệch trong ngưỡng ⇒ `tie` ⇒ DÙNG CACHE, không bao giờ thay thứ đang nhìn bằng thứ ngang tuổi; ② guard `disk-incomplete` — đĩa đọc được nhưng SỐ SHEET ÍT HƠN cache ⇒ nghi ghi dở/hỏng, KHÔNG thay im lặng, giữ cache + báo rõ), `createDiskWriter()` (③ THROTTLE — không phải debounce, ghi trong vòng `intervalMs` kể từ touch() ĐẦU, `flushNow()` cho ⌘S/rời trang, coalesce đúng khi touch() đến giữa lúc đang ghi — 5 test riêng xác nhận), `watchProjectPresence()` (④ BroadcastChannel + HEARTBEAT/TTL — chọn nhánh (a) phát hiện+cảnh báo, không khoá/gộp). 14 test thuần mới (`disk-sync.test.ts`, 0 fail). **Nối vào CAD** (`CadSheets.tsx`): `writeIdfToDisk()`/`resolveAndSyncCadDisk()` (ghi-rồi-đọc-lại-xác-nhận đúng kỷ luật `testStorageConnection()` B3) + `applyIdfSheets()` (đường áp dụng DUY NHẤT dùng chung nhập thủ công/tự động từ đĩa) — CAD dùng `useCadStore.setState()` (Zustand) nên KHÔNG dính lớp lỗi remount B2 (khác kiến trúc key-remount của Present), xác nhận qua đọc code `applySnapshot()` đã dùng cho nhập `.idf` thủ công từ trước. **Nối vào Present** (`PresentSheets.tsx`): `writeIdfpToDisk()`/`resolveAndSyncPresentDisk()` + `applyIdfpSheets()` — BẮT BUỘC tăng `importGen` (đúng cơ chế B2) vì Present dùng `key={activeId}:{importGen}`, đường nạp tự động từ đĩa ĐI QUA đúng 1 hàm với nhập thủ công `.idfp`. ⌘S (`present:force-save-request`) mới thêm ở `PresentEditor.tsx` cùng mẫu `cad:force-save-request` (2.1.8.n) — Present trước đây CHƯA có. `lib/save-status.ts` thêm `diskStatus`/`diskMessage` (tách khỏi `status` IndexedDB, đúng bài học sự cố 31/07 — "đã lưu"/"đã ghi đĩa" 2 việc 2 trạng thái); `lib/project-presence-ui.ts` (mới, store nhỏ riêng cho ④); `StatusBar.tsx` hiện cả hai. **2 bug thật bắt được khi browser-verify** (không phải unit test, đúng lý do Hoà bắt buộc phép thử này): ① race `projectName` — `flowName` nạp bất đồng bộ qua `ensureProjectScope()` (chạy trong `useProjectScopeSync` ở trang), disk-resolve có thể chạy TRƯỚC khi nạp xong ⇒ tạo NHẦM 2 thư mục khác tên cho ĐÚNG 1 dự án (`<id> — Untitled flow/` VÀ `<id> — Dự án mẫu/` cùng tồn tại) — sửa bằng gọi lại `ensureProjectScope()` (idempotent, tái dùng cơ chế có sẵn) TRƯỚC khi đọc `flowName` để ghi/di trú. ② `watchProjectPresence` bản đầu CHỈ gửi `'bye'` lúc `beforeunload` — tab đóng KHÔNG SẠCH (crash/force-quit, hoặc verify xác nhận đóng tab qua công cụ tự động cũng không luôn chạy `beforeunload`) ⇒ banner cảnh báo TREO VĨNH VIỄN dù tab kia đã mất — sửa bằng HEARTBEAT (`hello` lặp lại mỗi 4s) + TTL sweep (12s, ≥2× nhịp) thay vì phụ thuộc 1 sự kiện unload duy nhất; `'bye'` giữ lại làm đường tắt best-effort. Cũng phát hiện thiếu `diskWriterRef.current?.touch()` trong effect "cấu trúc tab đổi" của CAD (Present có, CAD quên) — dự án CAD mới hoàn toàn không ai vẽ gì sẽ KHÔNG BAO GIỜ có `ban-ve.idf` đầu tiên nếu thiếu dòng này, đã thêm. **Verify thật** (OPFS thay `showDirectoryPicker`, nhiều tab thật qua `tabs_create`/`tabs_close`, KHÔNG suy đoán): di trú CAD+Present ra đúng `<mã dự án> — <tên>/ban-ve.idf`+`trinh-bay.idfp`, tên thư mục ĐÚNG sau khi vá bug ① · đĩa thắng với id sheet TRÙNG — canvas Present hiện ĐÚNG nội dung marker từ đĩa (chứng minh `importGen` hoạt động qua đường tự động, không chỉ nhập tay B2) · CAD disk-thắng qua Zustand xác nhận trên canvas thật · guard ② — đĩa 1 sheet/cache 2 sheet ⇒ cache giữ nguyên (KHÔNG mất sheet) + banner "Chưa ghi ra đĩa" đỏ, rồi disk-writer tự lành (ghi lại đúng 2 sheet) trong lượt kế · ④ — 2 tab thật cùng dự án hiện banner "Đang mở nơi khác" HAI CHIỀU, đóng 1 tab (kể cả đóng không sạch) → banner tự hết sau ≤TTL, xác nhận đúng bằng cách chờ thật, không giả định. **Ranh giới KHÔNG đụng**: `.ifpack`/`auto-backup.ts` (khái niệm khác, vai trò tách từ B1) · tên file/vị trí giữ nguyên `docs/QUYET-DINH-HA-TANG-2026-07-31.md` đã chốt · KHÔNG giải bài đồng bộ/khoá đa tab (④ chỉ cảnh báo, đúng phạm vi Hoà xác nhận). **Phép thử tay còn lại** (Hoà tự làm — công cụ này không điều khiển được `showDirectoryPicker`/mất quyền thật qua reload thật): chọn thư mục gốc thật → sửa 1 dự án → mô phỏng mất quyền (đổi tên/di chuyển thư mục ngoài Finder, hoặc đợi hết phiên rồi reload) → sửa tiếp trong lúc mất quyền → khôi phục quyền (bấm "Kiểm tra kết nối thư mục") → xác nhận bản MỚI HƠN (đang sửa) thắng, KHÔNG mất gì — checklist đầy đủ đã gửi kèm báo cáo B4. tsc+eslint+test(14 test mới+0 fail toàn repo 850+)+build sạch. ⏸️ **DỪNG Ở ĐÂY, CHỜ HOÀ GẬT** trước khi làm B5. | 4.1.c | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` ⑥ |
| 4.1.e | ĐỢT B — B5: phép thử nghiệm thu — copy thư mục dự án sang máy khác, mở lên, chạy đủ không thiếu gì | N | ✅ done (01/08, Hoà tự chạy thật — cửa sổ ẩn danh/IndexedDB rỗng, thư mục gốc trỏ `Downloads/IF-MAYKHAC` là bản copy của thư mục dự án gốc, Cowork đo trên đĩa). **7/7 ĐẠT**: (a) CAD đúng 2 tab Bản vẽ 1/2 · (b) Bản vẽ 1 — 2 rect + 1 line khớp entities trên đĩa · (c) Bản vẽ 2 — có tab, 0 hình (không bịa, không nuốt) · (d) Present — deck LUMEN VILLA đủ 8 slide · (e) **ĐÍNH CHÍNH 01/08** (bảng khám `4.1.f` làm lộ ra ghi quá rộng) — 6 màu + Elegant + đúng tên đến từ `BrandKitPanel.tsx:62` `[...deck.palette]`/`:66` `useState(deck.fonts)`/`:59` `deck.brand` — panel khởi tạo TỪ DECK (`.idfp` → `deck.brand`/`deck.palette`/`deck.fonts`), KHÔNG từ thư viện kit (`getActiveBrandKit()` chỉ override SAU nếu localStorage có kit — hồ sơ sạch B5 thì không, nên panel giữ nguyên giá trị từ deck). ⇒ (e) chứng minh THEME CỦA DECK sống sót trên đĩa qua `.idfp`, KHÔNG chứng minh `brand-kit.json` được đọc — giải thích luôn vụ id phân nhánh ở `4.1.f` (profile sạch → panel hiện màu/font từ deck → bấm Lưu → localStorage rỗng → `makeId()` đúc id mới). B5 vẫn ĐẠT 7/7 ở mức đúng: deck là thứ giao khách, sống sót đúng. · (f) StatusBar "Đĩa đồng bộ", không đỏ · (g) bản gốc `IF-DUAN` còn nguyên — **verify TRÊN ĐĨA bằng diff 3 tệp gốc/copy sau khi bỏ mốc thời gian**, KHÔNG phải mở lại bản gốc trong cửa sổ thường (mức verify yếu hơn, không dùng ở đây). **ĐỢT B (B1-B5) HOÀN TẤT.** Phát sinh khi verify (không chặn B5, tách mã riêng): Brand Kit KHÔNG đảo nguồn theo B4 — vẫn đọc/ghi qua `localStorage`, `brand-kit.json` chỉ xuất một chiều qua nút thủ công ⇒ hồ sơ sạch (localStorage rỗng) tạo `bk_` id MỚI thay vì nhận id trên đĩa (nội dung giống hệt, chỉ id phân nhánh) — xem `4.1.f` (khám xong) + `docs/CHOT-BRAND-KIT-2026-08-01.md` (Hoà quyết Brand Kit thuộc DỰ ÁN, 01/08). | 4.1.d | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` ⑥ |
| 4.1.f | Brand Kit KHÔNG đảo nguồn theo B4 — KHÁM (chưa code, phát sinh khi verify B5). Đếm bằng lệnh, đủ file:dòng: (1) `brand-kit.ts:58-59` `read()`/`69-71` `write()` — RAW localStorage (`interiorflow.brandKits`/`interiorflow.brandKitActive`), nguồn sự thật DUY NHẤT hiện nay, KHÔNG project-scoped (1 danh sách phẳng dùng chung mọi dự án). (2) `brand-kit-disk.ts:37` ghi `brand-kit.json` — gọi DUY NHẤT từ `BrandKitPanel.tsx:157` (`onSave()`, fire-and-forget SAU localStorage). (3) `brand-kit-disk.ts:46` đọc kiểm tồn tại — gọi từ `BrandKitPanel.tsx:103` (chỉ quyết hiện/ẩn nút, không áp nội dung). (4) `brand-kit-disk.ts:66` đọc để NHẬP — gọi từ `BrandKitPanel.tsx:179` (`onImportFromProjectFolder()`, CHỈ khi bấm nút + xác nhận hộp thoại). (5) `PresentSheets.tsx:455-456` đọc `getActiveBrandKit()` 1 lần, nhúng `brandKitSnapshot` vào `.idfp` lúc XUẤT. (6) `idfp.ts:204` `parseIdfp()` đọc lại `brandKitSnapshot` lúc NHẬP nhưng KHÔNG nơi nào tiêu thụ (`grep '\.brandKitSnapshot'` loại `idfp.ts`/export path = 0 kết quả — `PresentSheets.tsx` nhập `.idfp` chỉ lấy `parsed.sheets`, bỏ `parsed.brandKitSnapshot`) ⇒ ảnh chụp đông cứng, không phải nguồn sống. `disk-sync.ts`/`project-scope.ts` (engine B4): `grep -c brand` = 0/0 — B4 chưa từng chạm Brand Kit. **Hệ quả đo được**: hồ sơ sạch (localStorage rỗng, đúng cảnh ẩn danh B5) bấm "Lưu Brand Kit" → `saveBrandKit({id: editingId \|\| undefined})` (`BrandKitPanel.tsx:149`) với `editingId=''` ⇒ `makeId()` (`brand-kit.ts:78`) đúc id MỚI bất kể `brand-kit.json` trên đĩa đã có kit nội dung y hệt — `bk_ms91ahvi_mrbu` (gốc) → `bk_ms9unabh_ktsm` (copy), tên/6 màu/Elegant giống hệt, chỉ id phân nhánh. **`resolveSourceOfTruth()` (`disk-sync.ts`) KHÔNG tái dùng thẳng được** — hàm đó so 1 TÀI LIỆU (mtime đĩa vs cache) theo model "1 file = đúng 1 bản của 1 dự án" (`.idf`/`.idfp`); Brand Kit lệch model: `brand-kit.json` mỗi dự án là bản XUẤT TOÀN BỘ danh sách global (không phải phần riêng của dự án đó) — áp "đĩa thắng nếu mới hơn" nguyên khối sẽ có nguy cơ NẶNG HƠN lỗi hiện tại: mở dự án A → đĩa (snapshot cũ của A) thắng → GHI ĐÈ TOÀN BỘ danh sách kit global → kit của dự án B tạo sau ở máy khác bị mất nếu chưa kịp đồng bộ nơi khác. Brand Kit là THƯ VIỆN DÙNG CHUNG nhiều dự án (giống "Bộ học Gu" ở `docs/QUYET-DINH-HA-TANG-2026-07-31.md` ③), không phải tài liệu 1-dự-án-1-file — đảo nguồn đúng cách (nếu làm) cần hợp nhất CẤP TỪNG KIT (mỗi `BrandKit` có `id`+`updatedAt` riêng), không phải so nguyên cụm — thiết kế MỚI, không phải gọi lại hàm cũ. **CÂU HỎI CẦN HOÀ QUYẾT, KHÔNG TỰ CHỌN**: Brand Kit của DỰ ÁN hay của STUDIO? Sơ đồ gốc `docs/QUYET-DINH-HA-TANG-2026-07-31.md` ② dự tính CẢ HAI lớp (`<dự án>/brand-kit.json` VÀ `_studio/brand-kit-mac-dinh.json`, cùng kiểu 2-tầng với phương án C ở mục ③) nhưng lớp `_studio/` CHƯA TỪNG ĐƯỢC CODE (`grep -rn "_studio\|brand-kit-mac-dinh"` toàn repo = 0 kết quả) — nếu quyết CẢ HAI thì đảo nguồn kiểu B4 (disk-thắng-nếu-mới-hơn, 1 tầng) SAI KHUÔN, cần mô hình 2 tầng riêng. Microcopy `BrandKitPanel.tsx:497-501` ("đợt sau sẽ lưu Brand Kit theo từng dự án") LỖI THỜI MỘT NỬA — B3 đã làm nhưng MỘT CHIỀU (xuất thủ công, nguồn vẫn localStorage global) — đề xuất câu thay thế nói đúng hiện trạng: *"Brand Kit lưu trên máy này (localStorage), dùng chung cho mọi dự án. Mỗi dự án có thể giữ 1 bản sao brand-kit.json (tự ghi kèm khi bấm Lưu) để mang sang máy khác — đây là bản xuất MỘT CHIỀU, không tự đồng bộ ngược; đổi máy phải bấm 'Nhập từ thư mục dự án' thủ công."* — CHƯA áp dụng, chờ Hoà chốt hướng đảo nguồn trước (đổi copy trước rồi đổi lại sau khi có kiến trúc mới là làm 2 lần). | N | ⬜ khám xong, CHƯA CODE — chờ Hoà quyết 3 câu hỏi trên | 4.1.e | báo cáo trực tiếp Hoà (khám phát sinh từ B5) |
...
| 4.6 | Sao lưu + đóng gói `.ifpack` — tự backup định kỳ | N | ✅ done — bản gốc (30/07) `lib/cad/auto-backup.ts`: `setInterval` 10 phút + trigger mỗi lần autosave IDB ghi xong, ghi ra thư mục thứ 2 do người dùng chọn 1 lần (File System Access API), giữ đúng 5 bản gần nhất (`namesToPrune()`). **SỬA CÙNG ĐÊM (B3, 30/07 khuya, Hoà chốt TRƯỚC khi thử sập)** — "5 bản × 10 phút = chỉ 50 phút lịch sử, sai từ hôm qua là mất": thay bằng `lib/cad/backup-diff.ts` mới (thuần, 50 test) — thang thời gian (1h giữ mọi bản → 24h 1/giờ → 30 ngày 1/ngày → xa hơn 1/tuần KHÔNG giới hạn, `RETENTION_TIERS` CONFIG) + lưu CHÊNH LỆCH theo entity (không phải byte, `diffSheets()`/`applyDiff()`) thay vì bản đầy đủ mỗi lần, mốc đầy đủ mỗi 20 bản (`FULL_SNAPSHOT_EVERY`). Bất biến bắt buộc test kỹ: bản GIỮ mà chuỗi dẫn tới nó bị xoá 1 phần → tự ĐÚC thành đầy đủ TRƯỚC khi xoá (`planRetention()`, kể cả ca 2 đại diện khác bucket cùng gãy phải đúc ĐỘC LẬP cả 2); phục hồi gặp entry thiếu/hỏng → dừng ngay, lùi về mốc TỐT NHẤT trước đó, không throw, không trả bản sai mà giả vờ đúng (`reconstructUpTo()`). Lối vào UI MỚI (trước không có): `BackupRecoveryModal.tsx` liệt kê backup theo thang hiển thị (`formatBackupRelativeTime()` — "10 phút trước · hôm qua 15:20 · thứ Hai"), menu "Khôi phục từ backup…" → tạo dự án MỚI không đè dự án đang mở (đúng nguyên tắc `.ifpack` cũ). `namesToPrune()`/`auto-backup.test.ts` cũ đã xoá, thay hẳn. Verify: 50/50 test `backup-diff.test.ts` (round-trip đủ kiểu đổi, tỉa đúng thang, **diff giữa chuỗi mất → lùi đúng mốc trước đó không throw** — đúng kịch bản B3 yêu cầu thử) + browser thật end-to-end (dispatch sự kiện thật → project mới tạo → entity/layer khớp CHÍNH XÁC dữ liệu gửi, xem `docs/VERIFY-B3.md`) + `ifpack.test.ts` corrupt-zip có sẵn vẫn PASS. ⚠️ **Giới hạn công cụ, disclose rõ** (`docs/VERIFY-B3.md`): không tự động hoá được `showDirectoryPicker()` (bắt buộc gesture người dùng thật) và `kill -9` tiến trình Electron thật trong môi trường phiên này — bù bằng 50 test mô phỏng đúng kịch bản hỏng/mất giữa chuỗi; kèm hướng dẫn 3 bước Hoà tự làm 1 lần để xác nhận mức OS thật (bằng chứng cho phần trình BGĐ "mất dữ liệu: rủi ro thấp"). | không | `SPEC-FILE-MANAGER.md`, `docs/VERIFY-B3.md` |
...
| 7.1.19 | Lark provider — Wiki `app_token` resolution: `resolveWikiAppToken()` (`GET /open-apis/wiki/v2/spaces/get_node?token={node_token}&obj_type=wiki` → đọc `obj_token`, cache) + tách 3 biến `LARK_ATLAS_NODE_TOKEN`/`LARK_ATLAS_APP_TOKEN`/`LARK_WORK_APP_TOKEN`, giữ tương thích `LARK_BASE_APP_TOKEN` | N | 🟡 **code xong, CHƯA verify bằng call thật** (làm ở worktree phụ song song 30/07, merge vào main sạch — không conflict, đụng đúng 3 file `lark.ts`/`.env.example`/`INTEGRATIONS.md`, không lấn `2.2.86`). `resolveWikiAppToken()` + 3 biến đã có thật trong `lib/integrations/providers/lark.ts` (xác nhận đọc code sau merge), comment bắt buộc "node_token ≠ app_token" có trong code, `LARK_WORK_APP_TOKEN` fallback `LARK_BASE_APP_TOKEN` đúng yêu cầu tương thích ngược. **CHỜ**: `.env.local` chưa có `LARK_APP_ID`/`LARK_APP_SECRET`/`LARK_ATLAS_NODE_TOKEN` — chưa gọi được API thật để xác nhận host `open.larksuite.com` dùng được với tenant shard `.jp.` (đúng như brief yêu cầu "báo kết quả, đừng đoán" — DỪNG đúng chỗ thay vì bịa). tsc+103/103 test sạch SAU merge (lớp kiểm duy nhất code này chưa từng qua, worktree phụ không chạy được bộ test). **KHÔNG đánh ✅ tới khi có key thật verify xong.** | không | `docs/REVIEW-SPEC-BOQ-LARK-2026-07-30.md` §3 |
...
| 7.1.21 | Khai bộ test vào `package.json` — thêm script `"test"` (hiện KHÔNG có; 100+ file `*.test.ts` chỉ chạy được qua `node_modules/.bin/sucrase-node <file>` gõ tay từng file — đúng lý do `7.1.19` ship được mà chưa qua lớp test nào ở worktree phụ) | N | ⬜ đề xuất 30/07 (lúc merge `7.1.19`), **số ĐÃ XÁC NHẬN 30/07 khuya (Luật #12)** — vẫn CHƯA CODE, chưa tới lượt trong hàng chờ (③ sau `2.2.70`/`2.1.9.q`) | không | STATUS.md 30/07 khuya |
| 7.1.22 | Bộ nhớ đo đạc — Tầng 1 (Không AI) "Đo món đồ" (`2.2.87`/`2.2.88`) tự học theo thời gian bằng dữ liệu nhà (bảng Prisma `MeasurementSample`: kích thước theo loại đồ TRUNG VỊ+độ lệch · tỉ lệ bộ phận · ⭐ chiều cao máy ảnh thật theo preset studio), KHÔNG bằng model — tất định, 0đ, dữ liệu không rời máy. Ngưỡng áp dụng n<5 dải chuẩn/n≥5 trộn/n≥20 ưu tiên dữ liệu nhà, luôn hiện nguồn số, sửa số tại chỗ không ma sát (không modal/không nút Lưu) | N | ⬜ đề xuất 30/07 (ngay sau khi cascade 4-bậc "không-bao-giờ-fail" của `2.2.87` xong), **CHƯA CODE — theo đúng yêu cầu "báo mã, chờ xác nhận"**. Số **XÁC NHẬN 30/07 khuya** — `7.1.21` = script test (đề xuất trước), `7.1.22` = Bộ nhớ đo đạc (đúng Luật #12 mới, chỉ Claude Code cấp mã). | 2.2.87 (cascade 4 bậc phải xong trước, ĐÃ xong 30/07 khuya) | tin nhắn Hoà 30/07 khuya (Bộ nhớ đo đạc) |
| 7.1.23 | ⭐ Luật CHỮ VIỆT trên UI (kiến trúc nền, áp cho MỌI chặng) — `2.2.85` (bỏ font mono nhãn node) là 1 TRIỆU CHỨNG của lớp lỗi này, không phải chuyện thẩm mỹ đơn lẻ: dấu chồng tiếng Việt (dấu phụ+dấu thanh) MANG NGHĨA (phân biệt thanh điệu), chữ/khoảng cách/chữ hoa ép dấu = MẤT NGHĨA. Quét 5 thứ: ① `text-transform:uppercase`/class `uppercase` — CẤM khi chuỗi CÓ DẤU (chữ hoa chật chỗ đặt dấu), CHO PHÉP chuỗi kỹ thuật không dấu (PDF/DXF/A3/CAD/BIM); ② `line-height<1.5` trên chữ Việt — `leading-none`(1)/`leading-tight`(1.25)/`leading-snug`(1.375) đều VI PHẠM, tối thiểu `leading-normal`(1.5); ③④⑤ **CHƯA NHẬN ĐỦ** — tin nhắn Hoà bị cắt giữa mục ②, đã báo lại xin gửi tiếp, KHÔNG tự suy đoán nội dung 3 mục còn lại | N | ⬜ đề xuất 30/07 khuya, **CHƯA CODE — chờ Hoà xác nhận mã + gửi tiếp ③④⑤**. Quét sơ bộ ①: 46 file có class `uppercase` trong `components/`+`app/` — CHƯA phân loại CÓ DẤU/không dấu kỹ thuật (việc lớn, để đợt code chính thức). 1 chỗ Hoà đã chỉ đích danh: `StatusBadge` trong `components/notebook/NotebookSourcesSidebar.tsx` (nhãn "Đang xử lý"/"Sẵn sàng"/"Lỗi" — CÓ DẤU, phải bỏ `uppercase`) — CHƯA sửa, gộp cùng đợt code chính thức khi đủ 5 mục. | 2.2.85 (nguồn phát hiện lớp lỗi) | tin nhắn Hoà 30/07 khuya (nguồn: vietnamesetypography.com — Donny Trương, googlefonts diacritics guide) |
| 7.1.24 | Khoá bộ học "Gu" (2 model perceptron Present/CAD) tách THEO NGƯỜI DÙNG — khoá cũ `PRESENT_TEMPLATE_MODEL_KEY`/`CAD_LAYOUT_OPTION_MODEL_KEY` là chuỗi CỐ ĐỊNH, máy chung studio trộn lẫn trọng số nhiều người | N | ✅ done (31/07, ĐỢT A code phụ, đề xuất mã trong commit — Claude Code xác nhận không trùng, ghi chính thức vào cây 31/07) — `presentTemplateModelKey(userId)`/`cadLayoutOptionModelKey(userId)` (`lib/gu/feature-dict.ts`, `lib/cad/ai-layout-feedback.ts`) trả `'<khoá cũ>.<userId>'`, `null` nếu chưa có userId (không rơi về khoá dùng chung — xác nhận đọc code: `LayoutShelf.tsx`/`AiBriefPanel.tsx` đều `if (!modelKey) return`/guard trước khi đọc/ghi localStorage). Khoá CŨ giữ nguyên, KHÔNG di trú dữ liệu cũ (`docs/QUYET-DINH-HA-TANG-2026-07-31.md` §0a — có thể đã bẩn, `minPairs=10` nên mất mát chỉ ~10 lượt). Banner 1 lần báo "bắt đầu lại" khi phát hiện khoá theo-user chưa từng tồn tại. 9 test mới. Verify độc lập (code chính, không tin báo cáo mù): đọc trực tiếp `feature-dict.ts`/`LayoutShelf.tsx` xác nhận đúng behavior mô tả; tsc+eslint+test+build sạch sau merge `feat/dot-a-ha-tang`. | không | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` §0a |
| 7.1.27 | `FlowVersion` — đổi cò: ngừng chụp tự động mỗi lượt "Chạy flow", chỉ ghi khi bấm "Đánh dấu bản này" + thang lưu giữ (Đợt B4, `docs/QUYET-DINH-HA-TANG-2026-07-31.md` §④ phương án C, đề xuất mã trong commit) | N | ✅ (01/08, PHẦN B code phụ) `lib/execution.ts` bỏ hẳn `snapshotOnStart`/auto-snapshot trong `executeRun()` (không phải cờ tắt — xoá khỏi `FlowRun` type luôn, tránh cờ chết) · `lib/workspace.ts snapshotFlow()` giờ CHỈ gọi từ hành động tay, thêm chặn `isReadOnly()` (trước đó gián tiếp được `runFlow()` chặn, giờ đứng riêng nên phải tự chặn) · lệnh mới "Đánh dấu bản này" trong `CommandPalette.tsx` (cạnh "Chạy flow", vì nút rời trên bar đã bị Hoà xoá 30/07 — 2.2.86) · `lib/flow-version-retention.ts` — thang lưu giữ THUẦN, tái dùng ĐÚNG `RETENTION_TIERS` của `lib/cad/backup-diff.ts` (giữ 1h → 1/giờ trong 24h → 1/ngày trong 30 ngày → 1/tuần sau đó), không tái dùng `planRetention()` cả khối vì đó xử lý chuỗi diff/full CAD không áp dụng cho `FlowVersion` (đã là `graphJson` đầy đủ) · tỉa chạy trong `app/api/flows/[id]/route.ts` ngay sau khi tạo bản mới, lỗi tỉa không chặn việc đánh dấu. 11 test mới `flow-version-retention.test.ts`. tsc/eslint sạch. | 0c (đo trước khi quyết) | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` §④ |
| 7.1.26 | Bộ học "Gu" (perceptron CAD, `AiBriefPanel.tsx`) chuyển từ CHỈ localStorage → Prisma LÀ NGUỒN + xuất/nhập `.json` (Đợt C, `docs/QUYET-DINH-HA-TANG-2026-07-31.md` §③ phương án C, đề xuất mã trong commit) | N | 🟡 **code xong (01/08, ĐỢT B/PHẦN B code phụ), CHỜ Hoà chạy `prisma db push` tay trên máy thật** (sandbox FUSE mount không cho SQLite khoá file đúng POSIX — `disk I/O error`, đã chứng minh 2 lần, không phải giả thuyết; xem `docs/HANDOFF-GUMODEL-DB-PUSH-2026-08-01.md`) — model `GuModel` trong `prisma/schema.prisma` (`@@unique([userId,kind])`, kind whitelist `cad-layout-option`/`present-template`) + `app/api/gu/[kind]/route.ts` (GET/PUT, auth `getSessionUser()`) + `lib/gu/gu-model-sync.ts` (fetch wrapper KHÔNG BAO GIỜ NÉM + xuất/nhập .json thuần, 20 test mới) + cắm vào `AiBriefPanel.tsx` (đối chiếu 2 chiều theo `pairsSeen`, không mất tiến độ học offline) + `components/settings/GuModelSettings.tsx` (nút Xuất/Nhập, wire vào `/settings`). ⛔ **CỐ Ý CHƯA cắm** `LayoutShelf.tsx` (kind `present-template`) — file thuộc `components/present-editor/`, ngoài phạm vi code phụ đợt này (Phần A đang kiểm cùng lúc), để đợt sau. `dev.db` KHÔNG di trú trọng số cũ (bắt đầu lại từ 0, đúng quyết định 0a). tsc+eslint sạch (2 lỗi `prisma.guModel` chỉ do client sandbox chưa generate được — đã tự chứng minh qua scratch-generate riêng, không phải lỗi code). | 7.1.24 (khoá theo userId) | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` §③ |
| 7.1.25 | Brand Kit — xuất/nhập `.json` (phao tạm chuyển máy, trước khi ĐỢT B đưa `brand-kit.json` vào thư mục dự án) — Brand Kit hiện CHỈ ở localStorage, không bản sao nào | N | ✅ done (31/07, ĐỢT A code phụ, đề xuất mã trong commit — ghi chính thức vào cây 31/07) — `lib/present-editor/brand-kit.ts`: `buildBrandKitExport()`/`parseBrandKitExport()`/`mergeBrandKits()` THUẦN (lọc bỏ kit hỏng riêng lẻ, không từ chối cả file) + `exportBrandKitsJson()`/`importBrandKitsJson()` chạm localStorage. `BrandKitPanel.tsx` thêm nút Xuất/Nhập, `window.confirm` hỏi ghi-đè/gộp trước khi ghi (không im lặng đè). Mode `merge`: kit trùng id → cấp ID MỚI (bản sao), KHÔNG đè kit đang có. 40 test mới. Verify độc lập: đọc `mergeBrandKits()` xác nhận đúng — nhánh `merge` không bao giờ ghi đè entry cùng id có sẵn; tsc+eslint+test+build sạch sau merge. Các hàm THUẦN tái dùng nguyên cho `4.1.c` (B3, ĐỢT B). | không | `docs/QUYET-DINH-HA-TANG-2026-07-31.md` §0b |
...
| 7.3.33 | Sprint "Lộ nền" VIỆC 1 — bảng tra phím tắt toàn app: đọc MỘT NGUỒN `lib/shortcuts.ts` (không viết tay ở UI), lệnh gõ tay CAD (`XL`/`AR`/`ARP`…) lấy tự động từ `lib/cad/command-aliases.ts` (tách khỏi `CadToolbar.tsx`/`CadEditor.tsx`), mở bằng ⌘/ hoặc `?`, đóng bằng `onDismiss` của `Popover` (khuôn `2.2.89`), có mục trong CommandPalette (⌘K) | N | ✅ done (31/07) — 6 file mới/sửa: `lib/cad/command-aliases.ts` (mới, `CAD_COMMANDS`+`groupedCadCommands()` chuyển nguyên từ `CadEditor.tsx`, autocomplete dòng lệnh hành vi không đổi) · `lib/shortcuts.ts` (mới, `SHORTCUTS[]` 4 scope toàn cục/cad/render/present, `formatShortcutKeys()` token hoá 'mod'/'shift' tránh hardcode ⌘ — SSR-safe cùng `useMacAfterMount()` export mới ở `lib/kbd.ts`) · `lib/shortcuts.test.ts` (16 test: 0 trùng phím/scope, label không rỗng, format Mac/Win đúng, alias gộp đúng) · `components/ShortcutsPanel.tsx` (mới, dùng `Popover`+`onDismiss`, neo giữa-ngang gần đỉnh — không phải flip-theo-click-point như context menu) · `AppChrome.tsx` (state `shortcutsOpen` + listener ⌘//`?`/CustomEvent `shortcuts:open`, universal 4 route — ĐÚNG chỗ vì `CommandPalette.tsx` CHỈ sống ở `HomeScreen.tsx`, ⌘K KHÔNG hoạt động ở /cad·/present, disclose thẳng trong bảng) · `CommandPalette.tsx` (mục "Phím tắt" phát `shortcuts:open`, cầu CustomEvent vì không chung state với AppChrome). **PHÁT HIỆN khi khám (Luật #4/#5) — 2 chỗ brief mô tả sai**: ① VIỆC 4 gốc nói CAD "chưa có phím/nút Vừa khung" — SAI, đã có SẴN phím `f` (`CadCanvas.tsx:2063`) VÀ nút toolbar "Zoom Extents (F)" (`CadToolbar.tsx:318`, event `cad:zoom-extents` wired đủ khắp app) — chỉ thiếu `⌘0` discoverable, không phải thiếu từ đầu; khai cả `f` cũ lẫn `⌘0` mới trong bảng, không xoá `f`. ② ⌘K brief liệt "toàn cục" nhưng thật ra CHỈ mount ở `HomeScreen.tsx` (grep 0 nơi khác), không hoạt động /cad·/present — ghi rõ cảnh báo ngay trong entry "Bảng lệnh nhanh" của bảng, không giấu. Khai ĐỦ hơn brief gốc (không chỉ 3 lệnh mẫu XL/AR/ARP): CAD ~46 nhóm lệnh gõ tay + 18 phím tắt, Render 8 phím tắt (⌘D/⌘G/⌘Z/⌘⇧Z/⌘Y/Space/V/H — `FlowCanvas.tsx`, brief không yêu cầu nhưng "khai đủ" là tinh thần sprint), Present 13 phím tắt (thêm ⌘=/⌘-/⌘0/⌘⇧]/⌘⇧[ ngoài danh sách brief liệt). 7.1.23 ⑤ tự-tuân-thủ: 3 chỗ ban đầu viết fontSize 10.5/11px trên chuỗi có dấu ("Sửa"/"Toàn cục"/"lệnh gõ tay") — tự phát hiện, sửa lên 12px trước khi commit, không đợi sweep hệ thống (`7.1.23` chính vẫn CHỜ HOÀ GẬT, không đụng). Browser-verify thật 4 route (`/cad`,`/render`,`/present` qua `HomeScreen`, ⌘K→"Phím tắt"): mở/đóng qua ⌘/, `?`, click ngoài, Escape, click từ CommandPalette — đều đúng, đúng scope hiện đúng chặng. tsc+eslint sạch (2 lỗi `CadEditor.tsx` xác nhận PRE-EXISTING qua `git stash` đối chiếu, không phải lỗi mới), test 16/16 (+ toàn bộ suite không hồi quy), `npm run build` thành công. 🔄 **CẬP NHẬT cùng ngày** — Hoà gửi bản mẫu HTML layout (cột phím cố định 118px/hàng cố định 30px/2 cột cân bằng/2 tab "Lệnh gõ tay"·"Phím tắt" tách riêng/ô tìm/thanh gợi ý cuối), dựng lại `ShortcutsPanel.tsx` theo đúng layout đó bằng token màu thật của app (`var(--accent-warm)` cho tiêu đề nhóm — trùng khớp `--warm` bản mẫu, không cần bịa token mới) thay vì bảng cũ 1 cột. Thêm `group: CadCommandGroup` vào `CAD_COMMANDS` (`command-aliases.ts`, 5 nhóm Vẽ/Kích thước & mặt cắt/Biến đổi/Nhìn/Chọn·xoá·hoàn tác — khớp bản mẫu) + `groupedCadCommandsByCategory()`. Ô tìm dùng `normalizeSearch()` có sẵn (`lib/nodes/search.ts`, bỏ dấu tiếng Việt) — tái dùng, không viết chuẩn hoá mới. Không nạp JetBrains Mono riêng như bản mẫu — giữ `ui-monospace` hệ thống đã dùng khắp app, tránh thêm phụ thuộc webfont. 5 test mới (21/21). Browser-verify thật: 2 tab đúng số đếm thật (66 lệnh gõ tay/25 phím tắt CAD), ô tìm lọc đúng ("hatch"→2 kết quả cả 2 tab), Escape/click-ngoài vẫn đóng đúng (cơ chế `onDismiss` không đổi). tsc+eslint+test+build sạch. | 2.2.89 (✅ — nguồn `onDismiss` tái dùng) | `docs/KIEM-NEN-2026-07-31.md` |
| 7.3.34 | Sprint "Lộ nền" VIỆC 5 — dọn chữ `CommandPalette.tsx`: đồng bộ ngôn ngữ (đang trộn 'Run flow'/'Fit view'/'Undo'/'Redo' tiếng Anh giữa danh sách tiếng Việt) · sửa `'Snap lưới: TẮT/BẬT'` (chữ hoa có dấu, vi phạm 7.1.23①) | N | ✅ done (31/07) — 'Run flow'→'Chạy flow' · 'Fit view'→'Vừa khung' · 'Undo'→'Hoàn tác' · 'Redo'→'Làm lại' (keywords giữ cả bản Anh để tìm ⌘K không mất, thêm từ khoá gốc trước khi đổi label). `'Snap lưới: TẮT/BẬT'`→`'Bắt điểm lưới: đang bật/đang tắt'` — **tự bắt lỗi khi code**: đổi từ nhãn HÀNH ĐỘNG (chữ mô tả việc BẤM sẽ làm) sang nhãn TRẠNG THÁI (chữ mô tả đang Ở trạng thái nào) làm ĐẢO ngược điều kiện 3 ngôi cần thiết (`s.snapGrid` true = đang BẬT chứ không phải sẽ TẮT) — phát hiện tự sửa trước khi verify, không phải Hoà chỉ ra. `CadToolbar.tsx` "Bắt điểm (snap): BẬT" tương tự CHƯA sửa — ngoài phạm vi VIỆC 5 (chỉ nêu `CommandPalette.tsx`), không quét lan. tsc+eslint sạch, test 0 fail, `npm run build` thành công. | 7.1.23 (luật gốc, đã có) | `docs/KIEM-NEN-2026-07-31.md` |
| 7.3.32 | AppChrome cụm phải (Tasks/Home/⋯/UserChip, `hidden…sm:flex`) tràn viewport ở 640-1023px — nhảy thẳng ẩn→đủ desktop ngay tại `sm`=640, dải 640-1023 chưa từng được thiết kế cho cụm này | N | ✅ done (31/07, ĐỢT DEMO GẤP) — đo thật 30/07 xác nhận đúng bug (`header.scrollWidth` 819px cả 640/768px, tràn 179px@640 · 51px@768, route render; 1024+ sạch). Sửa: đẩy mốc "hiện cụm phải" từ `sm`(640)→`lg`(1024) ở `AppChrome.tsx:191` (`hidden…sm:flex`→`hidden…lg:flex`) + đẩy `MobileMenu.tsx:55` cùng mốc (`sm:hidden`→`lg:hidden`) — KHÔNG viết UI mới, bottom-sheet `MobileMenu` đã sẵn Tasks(`TasksRow`)+Home(`PhaseRow`)+account+actions, chỉ cần đồng bộ điểm chuyển giao để không có dải nào "cả 2 cùng ẩn" hoặc "cả 2 cùng hiện". Browser-verify thật route `/render` (nơi có bug) + `/cad` (route demo thật): `headerScrollWidth-viewport` = 0 tại 640/768/1024px (trước: 179/51/0). Bấm "⋯" ở 640px xác nhận sheet mở đủ "CHẶNG LÀM VIỆC" (Home) + "VIỆC" (Tasks) — không mất tính năng, chỉ đổi vị trí. Không đụng bất biến 1024px priority+ (`2.2.60`, đã verify vẫn 0 tràn). tsc+eslint sạch, test xanh (0 fail), `npm run build` thành công. | 7.3.31 (✅ — cấu trúc AppChrome hợp nhất) | đo thật browser 31/07, xem trên |
...
**12 · Luật cấp mã** (Hoà chốt 30/07, sau va số `7.1.21` giữa 2 việc khác nhau — lỗi grep của
Cowork lúc Claude Code chưa kịp ghi dòng mới vào cây): **CHỈ Claude Code cấp mã số.** Cowork (hay
...
**12b · Bổ sung — hai phiên code** (Hoà chốt 31/07, khi ĐỢT B lớp lưu trữ chạy song song ĐỢT A
"code phụ" — `feat/dot-a-ha-tang` tự gán `7.1.24`/`7.1.25` trong commit message trước khi cây kịp
ghi): giờ có HAI phiên code cùng lúc (code chính — phiên này, làm ĐỢT B; code phụ — phiên khác,
làm ĐỢT A/C/D). **Code phụ chỉ ĐỀ XUẤT mã** (được phép viết số dự kiến vào commit message để có
dấu vết, NHƯNG đó là ĐỀ XUẤT không phải mã chính thức). **Code chính là nơi DUY NHẤT ghi mã vào
`docs/IF-FEATURE-TREE.md`** — khi merge nhánh của code phụ, code chính PHẢI tự kiểm đề xuất đó có
trùng mã nào đã có trong cây không; trùng thì code chính cấp lại số khác và báo Hoà, không trùng
thì xác nhận và ghi chính thức vào cây (như đã làm cho `7.1.24`/`7.1.25`, 31/07).
