...
| 2.2.87 | ⭐ Engine đo món đồ từ 1 ảnh (single-view metrology) — **Lát cắt 1** (`docs/TU-VAN-ANH-SANG-BAN-VE-2026-07-30.md` §4): CHỈ bước ①②③⑦ của dây chuyền 7 bước, KHÔNG dựng mesh 3D (⑤), KHÔNG sinh góc phối cảnh mới (⑥) — đợt 2. Thuần lib, 0 credit, tất định, KHÔNG UI (mặt tiền ở `2.2.88`) | N | ✅ done (checklist 6 bước đủ, 30/07) — `lib/vision/single-view-metrology.ts` mới, 2 lớp tách bạch: lớp TOÁN test được tất định (`calibrateFromVanishingPoints()`, `anchorScale()`, `measureObject()` — nhận toạ độ điểm tụ/ảnh, không đọc pixel) + lớp ẢNH heuristic (`detectLineSegments()` Sobel+Hough thô, `calibrateFromImage()` gom cụm Manhattan). **(a)** `calibrateFromVanishingPoints()`: công thức Caprile-Torre suy tiêu cự từ 2 điểm tụ trực giao, trực giao hoá ma trận xoay (Gram-Schmidt có hướng, không cần SVD tổng quát), `confidence` đo độ lệch trực giao thật. **(b)** `anchorScale()`: 3 nguồn neo (`ANCHOR_CONFIG` — cameraHeight 1500-1600/door 2000-2200/stairRiser 150-180/tileModule 300-800/tableTop 750/seatHeight 400-450/outlet 300-1200, TẤT CẢ vào CONFIG không rải rác), khớp ±5% (`ANCHOR_AGREEMENT_TOLERANCE`) → tin; lệch → `needsManualScale=true` KHÔNG tự chọn; `depthMetric` CHỈ kiểm chéo, không bao giờ vào candidates chính (test riêng xác nhận không kéo lệch kết quả dù cố ý lệch xa). **(c)** `measureObject()`: `{valueMm, toleranceMm, kind:'measured'|'inferred', basis}` — width/height LUÔN 🟢 measured (điểm chạm-sàn trái/phải qua mặt sàn hiệu chỉnh, đáy→đỉnh qua VP dọc), depth LUÔN 🟡 inferred (prior tỉ lệ sâu/rộng 0.55×, trừ khi có `silhouette.side`) — đúng phân nhóm ĐO/SUY tài liệu yêu cầu, không lẫn lộn. Test: 28/28 pass, verify bằng **cảnh 3D tổng hợp chiếu qua camera pinhole biết trước** (không phải raster ảnh thật — giải thích lý do ở đầu file) — khôi phục đúng tiêu cự+hướng camera (khớp chính xác đến float), suy hộp 600×400×800mm sai số width 0%/height 0.4% (<5% yêu cầu), ảnh không neo→`needsManualScale` đúng, 2 nguồn lệch (2100mm vs khai sai 2600mm)→cảnh báo đúng không tự chọn, depthMetric lệch xa không kéo lệch kết quả 2 nguồn đo thật. Bug thật bắt được NHỜ test cảnh tổng hợp (không phải suy luận suông): camera pitch-only (không yaw) làm world X song song mặt ảnh → điểm tụ tại vô cực/NaN (đã thêm yaw vào test, đúng tình huống ảnh thật); heuristic "dải sàn theo y" ban đầu vỡ khi 2 góc đáy trái/phải khác y ảnh dưới phối cảnh xiên (width ra 0) — sửa dùng điểm trái/phải/cao nhất THẬT của viền, giữ đúng toạ độ y riêng từng điểm. tsc+eslint sạch. | không | `docs/TU-VAN-ANH-SANG-BAN-VE-2026-07-30.md` |
| 2.2.88 | Mặt tiền "Đo món đồ" cho `2.2.87` — thẻ Tool Mode chặng 2 thứ 7, hiện R×S×C 🟢/🟡 phân màu, xuất SPEC SHEET (dấu cảnh báo bắt buộc) | N | ✅ done (checklist 6 bước đủ, 30/07) — node `vision.measureobject` mới (`lib/nodes/defs/metrology.ts`, 0 credit, wiring qua `defs/index.ts` đúng cơ chế barrel "chỉ thêm 1 import+1 spread"): tách món TÁI DÙNG `extractForeground()` (CHÍNH hàm `ai.furnitureextract` gọi ở tầng lõi, KHÔNG viết engine mới, Luật #6) → `calibrateFromImage()` + `anchorScale()` (2.2.87) → `measureObject()`. **Scope thu hẹp có chủ đích** (ghi rõ trong code, không giấu): (1) KHÔNG dựng chuỗi 3-node qua `ai.furnitureextract` làm node riêng — `tool-mode-graph.ts` hiện chỉ hỗ trợ đúng 2-node dùng CHUNG 6 thẻ khác, mở rộng rủi ro ảnh hưởng 6 thẻ đó, chọn gọi thẳng hàm lõi trong 1 node; (2) neo thang đo CHỈ 1 nguồn (chiều cao máy ảnh, mốc mạnh nhất theo tài liệu) qua 1 slider số — chưa dựng UI khoanh-2-điểm cho vật chuẩn khác (cửa/bậc thang/gạch), để lại đợt sau nếu cần độ tin cao hơn. `TASK_CARDS` thêm thẻ thứ 7 `measureobject`. `ToolModeForm.tsx` thêm nhánh hiển thị riêng (`MeasurementPanel`) cho thẻ này — width/height `var(--success)` (🟢 ĐO), depth `var(--warning)`+`~` (🟡 SUY), tooltip=`basis`, dải cảnh báo cố định "Mặt khuất là suy diễn — kiểm tra trước khi sản xuất." (không có tuỳ chọn tắt). `lib/render-studio/measurement-spec-sheet.ts` mới: vẽ panel [ảnh+bảng số đo+dấu cảnh báo] bằng canvas rồi đưa vào `composeBoard()` (TÁI DÙNG engine `out.board`, không sửa hàm đó — 300dpi/khung/tiêu đề đi qua nguyên vẹn, tránh rủi ro 6 chỗ khác đang gọi nó). Verify: browser thật — thẻ hiện đúng trong lưới 7 thẻ, form hiện đúng 2 tham số (chiều cao máy ảnh/ngưỡng nền), chạy THẬT qua đúng `runNode()` (không phải giả lập) trên ảnh tổng hợp thiếu cấu trúc kiến trúc → trả lỗi rõ ràng "Không hiệu chỉnh được camera... Đây LÀ kết quả đúng (không đoán bừa)" — đúng đường "ảnh không đủ neo" của tài liệu, xác nhận end-to-end qua UI thật (không chỉ unit test). Luật #11: 0 tràn ngang ở cả 5 mốc 640/768/1024/1180/1440 (`document.body.scrollWidth`). Sửa kèm: `ToolModeHome.tsx` tooltip "phức tạp hơn 6 thẻ" → "các thẻ" (hết cứng số, tránh lỗi thời khi thêm thẻ tiếp). 104/104 test, tsc+eslint sạch. ⚠️ **CHƯA verify** đường ĐO THÀNH CÔNG (ảnh có đủ cấu trúc → ra số thật, `MeasurementPanel` hiện số 🟢/🟡) bằng ảnh thật trong browser — chỉ verify bằng code review + 28 test toán học độc lập của `2.2.87` (cùng hàm) + đường lỗi thật đã chạy qua UI thành công; khuyến nghị Hoà thử 1 ảnh phòng thật trước khi tin số đầu tiên. | 2.2.87 | `docs/TU-VAN-ANH-SANG-BAN-VE-2026-07-30.md` |
