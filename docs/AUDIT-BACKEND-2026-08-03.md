...
| R2 | **Trừ credit tại server cho `/api/jobs`.** Gọi `spendCredits()` trước `submitJob`, `refundCredits()` khi submit lỗi. Bảng giá theo `task+tier` để ở server, không tin `def.creditCost`. **✅ ĐÃ CHỐT GIÁ 05/08 (Hoà giao TỔNG quyết, hết "cần Hoà chốt")**: 3 task `removeBg`·`materialSwap`·`segment` MIỄN PHÍ khi luồng lớn gọi nội bộ (cờ `internal:true` — idmask/localedit/smartselect), TÍNH PHÍ khi người dùng mở thẳng công cụ; cả luồng trừ MỘT LẦN ở kết quả cuối. Đánh đổi đã ghi ở docblock `TASK_CREDIT_COST`: cờ do client khai, curl `internal:true` chạy free được đúng 3 task whitelist này (task đắt render/video vẫn bị chặn) | `app/api/jobs/route.ts:50-55` · `lib/server/credits.ts` · bảng giá mới trong `lib/ai/tiers.ts` | `curl -X POST /api/jobs` với tài khoản 0 credit → 402; với tài khoản có credit → `GET /api/credits` giảm đúng số |
