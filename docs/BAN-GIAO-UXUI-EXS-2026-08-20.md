...
**⭐ ÁNH XẠ ẢNH-THAM-CHIẾU → DS (16 ref Hoà gửi 3 đợt 20/08) — nguồn chính tắc là
`docs/nc/REF-VISUAL-EXS-2026-08-20.md`**, mỗi ref đủ 4 cột: cơ chế → áp vào đâu trong IF → token →
luật ràng. Bản vẽ = board G (R1–R5) · H (R6–R14) · I (R15–R16), đều APPROVED. Trạng thái từng nhóm:
· **Đã nâng thành LUẬT/chốt**: R15 collage = layout-preset moodboard (Hoà chỉ định) · R16 "cubic
  nét, sang, đơn giản" = visual direction Vẽ 3D (vào Motion Law I.2) · R8 thẻ-việc-AI = khuôn
  Vitals V3 Engage · R3 stack-lùi-sau = khuôn Peek >3 insight · R6 progress-card = chuẩn upload.
· **Đã map, chờ thi công theo phiếu**: R1 thước vạch → MaterialPbrEditor/FOV/GR-núm · R4
  folder-as-object → Collection+/Files · R14 filter-có-ô-tìm (vẫn nợ code từ 16/08) · R9/R10
  rail-nở + meeting-insight.
· **Còn treo chờ Hoà** (đã nằm ở GATES): R2 direct↔proximity · R5 quầng presence màu gì ·
  model picker đặt đâu. Phiên sau đụng vùng nào thì MỞ SỔ REF đọc đúng mục — đừng suy lại từ ảnh.
...
**ERRATA 22/08 (Lane B khai, session này re-verify grep = đúng):** `VitalsPill.tsx` **0 call
site** — không mount ở đâu (11 chỗ nhắc tên đều là comment; AppChrome:362 tự khai "đã thôi
mount"). **LIVE VITALS = `components/studio/VitalsAperture.tsx`** (top-edge, code thật) + hành vi
canonical ở `VitalsChatSurface.tsx`; VitalsPill chỉ còn vỏ trình bày 67 dòng. ⇒ Phiên UX kế:
CẤM lấy VitalsPill làm căn cứ thiết kế Home/Vitals · CẤM remount khi chưa có product decision
của Hoà · board EXS-C/E chỗ nào ghi "VitalsPill" đọc thành "Vitals Aperture".
