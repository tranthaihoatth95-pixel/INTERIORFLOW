...
| Ca | Thí nghiệm | Kết quả TRƯỚC khi siết |
|---|---|---|
| `g4-moat:414-416` hai mắt nhãn **"IndexedDB"** | `mv lib/sheets-persist.ts /tmp/` | **cổng vẫn 63/63 ĐẠT** — `grep sheets-persist` = **0 lời gọi**, khẳng định là **tautology tuyệt đối** `JSON.stringify(round-trip(x)) === JSON.stringify(x)` |
| `frontier-registry:23` `h4-picker` | thay 185 dòng bằng chữ `x` | **vẫn 0 LỆCH** — mẫu là `.` nên bằng chứng thật ra là *"tệp không rỗng"* |
...
🟡 **Con số này là SÀN, không phải trần**: máy phủ 177/~198 khẳng định; hai bộ lái trình duyệt (`g1`, `g2`) máy **không phủ được** nên hạng của chúng đến từ **đọc tay**; đột biến chỉ chạy **7 lần chọn tay**, không phải mutation testing đầy đủ; và **ba mắt "so gương" chưa siết** (hai vế cùng đi qua một hàm hỏng nên luôn bằng nhau — bẻ `computeBoq` trả rỗng, chúng vẫn xanh).

🟡 **Hai vùng mù đo được, mốc hiện tại là SÀN**: `soi-hinh-hoc` chỉ quét `components/` ⇒ `app/` còn **19 radius ngoài thang** (32 → thật **≥51**) · `soi-thao-tac` luật hex chỉ khai `components` ⇒ `app/` còn **59 hex** (193 → thật **≥252**). Mở vùng quét cần một lượt riêng.


| Cổng | Trạng thái | Chủ sở hữu | Còn thiếu đúng cái gì |
|---|---|---|---|
| **G1 · DATA SAFE** | 🟢 PASS | 01 CORE | 12 ca trên app thật (2D · Trình chiếu · 3D), khe đăng xuất đã đóng. ✅ **05/09 — hợp nhất định danh (D8)**: trước vá **2/5 màn** có `store.user`; sau vá **5/5**, và `store` tới **cùng mili giây** với bộ đệm ⇒ chứng minh **một lượt giải duy nhất**, không còn cơ chế song song. Ca `LockScreenSettings:22` **MẤT THẬT, không phải chậm**: gõ 42 → ô hiện **42** → đĩa **`null`** → mở lại về **15**; giao diện nói dối và **không một dòng báo**. 🔧 Đính chính tiền đề của MAIN: cơ chế định danh là **BA** chứ không phải năm — 4 trong "5 chỗ gọi `/api/auth/me`" là **DELETE (đăng xuất)**, một chỗ **vứt kết quả**. Gộp bảy chỗ vào một rọ là đúng họ lỗi *"mẫu quét gộp nhiều cơ chế rồi báo quá tay"* đã ghi ba lần. Gỡ **1** đường thừa (`PresentStageScreen`), giữ **4** có lý do từng cái; `/present` **nhanh hơn** 3345 → 2335 ms vì thôi chờ lượt fetch thứ hai. `photo` là màn **chưa từng đo** và nó ❌ trước vá. 🟡 Chưa đo: `/notebook` · `/overview` · đường đăng xuất→đăng nhập chéo người dùng sau khi thêm nạp-store · nhánh 503/mạng đứt. |
...
| **G4 · DESIGN TRUTH / MOAT** | 🟡 **CHỜ MẮT HOÀ** | 05 ASSET + 01 CORE | ✅ Danh tính đi trọn đường UI — ba mắt **13/13** sau khi ĐÓNG HẲN trình duyệt (hình `#b98a54→#5a3a26` · danh tính `ps-kiem-go-soi→ps-kiem-go-ocho` · BOQ **68.817.600**), hiệu chuẩn bằng **gỡ dây thật** 5/13 ↔ 13/13. ✅ Mặt tiền thứ năm đã cắm · ✅ mặt vẽ 2D trả lại trọn (**439 → 0**, ba hộp lồng nhau) · ✅ bấm giữa lòng vùng tô chọn được. ⭐ **Bộ moat 58/59 → 63/63** — nhưng chuyện đáng ghi là **mắt cũ đang TỰ CHẤM ĐIỂM**: nó hỏi `hasOwnProperty(commerce,'specId')` trên object **chính bộ moat vừa viết ra 12 dòng trên**, mà `exportIdfc`/`importIdfc` là pass-through ⇒ thêm một khoá vào fixture là mắt xanh, **0 dòng mã sản xuất đổi**. Nay **đo hành vi** (đổi `sku` rồi hỏi liên kết còn không) chứ không đo hình dạng. Lỗ thật: `asset-family.ts:565` chép `brand/sku/vendor` mà **bỏ đúng hai khoá bất biến**. Additive thuần, **cố ý KHÔNG bump `IDFC_VERSION`** (bump chỉ khiến bản IF cũ **từ chối** tệp mới), 6 ca round-trip khoá **tệp v3 cũ mở nguyên vẹn, không bịa `specId`**. 58 mắt kia còn xanh, hai hiệu chuẩn cũ vẫn đạt. 🔴 **Còn lại**: ① **chưa qua mắt Hoà** — nợ nghiệm thu mắt **77 xong-máy / 1 qua mắt**, đây là nút cổ chai thật của cổng này ② `resolveIdfcCommerceToSpec()` **0 nơi gọi trong sản phẩm** (MAIN tự grep xác nhận) ⇒ theo đúng thước của chính bộ moat (*"moat chỉ tồn tại dưới dạng type/hàm mà người dùng không trải nghiệm được ⇒ chưa hoàn thành"*), phần này mới ở nấc **"dây có, chưa cắm điện"** — mắt Đ2 hỏi *".idfc có nối bằng khoá bất biến không"*, **không** hỏi *"chỗ nào trong app đang dùng liên kết đó"*. |
