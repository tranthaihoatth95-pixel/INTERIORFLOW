...
| **G2 · PROFESSIONAL FLOW** | 🔴 **CHẶN** | 02 WORKFLOW + 04 DESIGN | Cột ĐÃ LƯU **4/22 → 7/22** (J07 · J12 · J06 thêm vào; J18 xong tầng cơ chế; J22 sạch). Bộ chạy `nghiem-thu-g2-hanh-trinh.mjs` nay **10 mục khai báo**, khung `chayMot()` không đổi một dòng. Mã sản phẩm sửa **0 dòng**. 🔴🔴 **HAI LỖI CHẶN Ở ĐƯỜNG VÀO — người dùng mới KHÔNG TẠO ĐƯỢC DỰ ÁN**: **D-J04a** `components/home/XuongHome.tsx:398` khai ba nhãn *"Tạo dự án mới" · "Mở dự án có sẵn" · "Nhập từ tệp"* nhưng `:184,187,190` **cả ba dùng chung một `onClick={onMo}`**, và `onMo` chỉ đẩy sang `hienVat.href` — bấm xong `Project` 20→20, `Flow` 12→12, API khác GET chỉ `POST /api/cursors`. **D-J04b** `components/studio/ProjectScopeEmptyState.tsx:64-77` — `handleCreate` đặt `busy=true`, gọi `goToStage(routeId)` đẩy về **đúng URL đang đứng** nên không remount, mà `setBusy(false)` **chỉ nằm trong `catch`** ⇒ máy chủ sinh `Flow` 0→1 thật nhưng màn **kẹt "Đang tạo…" vô hạn** ở **cả `/cad` lẫn `/present`**; tải lại trang mới vào được. ⚠️ **Không máy nào bắt được**: `tsc` 0 · `npm test` 0 fail · `soi:cong-cu-chet` báo *"H3 · TAY CẦM RỖNG — 0"*. 📌 Ma trận **ghi sai từ đầu** cột ĐÃ LƯU của J07 (*"Flow version"*) — `grep "api/flows"` trong `components/cad/` = **0**; đường thật là `lib/cad/luu-len-may-chu.ts` → `POST /api/project-files`. **Còn 15 hành trình trống.** |
...
| **G4 · DESIGN TRUTH / MOAT** | 🟡 **DÂY ĐÃ NỐI, CHƯA ĐÓNG ĐƯỢC** | 05 ASSET + 01 CORE | ✅ Đứt gãy tìm ra lúc 20:15 **đã nối**: `applyMaterial` mang `specId` (`lib/cad/store.ts`), `replaceMaterial` là vỏ mỏng gọi đúng `replaceMaterialReferences` cũ, cửa duyệt đếm bằng `inspectMaterialImpact` — **cùng một cỗ máy**, không đẻ engine thứ hai. **Ba mắt cùng đổi và cùng sống sót, 13/13 sau khi ĐÓNG HẲN trình duyệt**: hình `#b98a54 → #5a3a26` · danh tính `ps-kiem-go-soi → ps-kiem-go-ocho` · con số BOQ **68.817.600** = 26,55 m² × 1,08 × 2.400.000, đọc từ IndexedDB và `POST /api/boq`. ⭐ Hiệu chuẩn bằng **gỡ dây thật**: xoá 2 dòng ghi `specId` → **5/13**, cắm lại → **13/13**, và **mắt HÌNH xanh cả hai lần** ⇒ bộ đo phân biệt được *vẽ ra hình* với *mang danh tính*. Override cục bộ không bị nuốt và không rò sang vật liệu mới. Giá **không** chép vào `MaterialPbr` (`grep priceVnd` = 0). 🔴 **Chưa đóng được, thiếu ba thứ**: ① **chưa qua mắt Hoà** (6 ảnh ở `anh-duyet-mat/g4-danh-tinh/`) ② Đ2 *".idfc nối bằng khoá đổi-được"* vẫn đỏ ở bộ tầng-mô-hình (58/59) ③ 🔴 **mặt tiền thứ tư chưa cắm** — `components/cad/MaterialPalette.tsx:23` chỉ đọc `MATERIALS`, **0 dòng chạm tầng hạt giống**, nên máy sạch mở mặt vẽ 2D ra **không có vật liệu nào để chọn** (bốn mặt tiền kia đã cắm — xem `G4-MOAT-SLICE.md`). |
