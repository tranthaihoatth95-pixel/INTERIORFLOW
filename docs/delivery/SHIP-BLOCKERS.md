...
| **G2 · PROFESSIONAL FLOW** | 🟡 PARTIAL | 02 WORKFLOW | **Bộ chạy hành trình** đã có: khung sở hữu bất biến, hành trình là **một mục khai báo**, chạy theo lô. Cột ĐÃ LƯU **1/22 → 4/22**. ⭐ Lý do mắt này chưa từng chứng minh được: bộ cũ dùng `newContext()` — **IndexedDB bị vứt lúc đóng**, nên "mở lại" vô nghĩa **từ định nghĩa**; nay `launchPersistentContext` trên hồ sơ đĩa thật. J17 dùng **SIGKILL** giữa lúc vẽ. 🔴 J20 mở tệp PDF ra soi và bắt được **D5 — nay ĐÃ ĐÓNG**: cổng `CHUAN_DAU_RA` cho deck (`lib/present-editor/export-checks.ts`) cắm vào **cả 3 cửa xuất** (PDF · PPTX · 300dpi), cảnh báo **nêu đúng trang và số ô**, không xoá hộ, không chặn tuyệt đối. Hiệu chuẩn: gỡ luật ⇒ đỏ, cắm lại ⇒ 16/16. ⚠️ Chỉ bắt **một** loại placeholder (mặc định `makeText`) — `{{ }}` · lorem · ảnh xám mẫu **chưa bắt**; PNG/`.zip`/`.idfp` **chưa có cổng**. **18 hành trình còn trống.** |
