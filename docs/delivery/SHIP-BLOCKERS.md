...
| **G4 · DESIGN TRUTH / MOAT** | 🟡 **DÂY ĐÃ NỐI, CHƯA ĐÓNG ĐƯỢC** | 05 ASSET + 01 CORE | ✅ Danh tính đi trọn đường UI: `applyMaterial` mang `specId`, `replaceMaterial` gọi đúng `replaceMaterialReferences` cũ, cửa duyệt đếm bằng `inspectMaterialImpact` — **cùng một cỗ máy**. **Ba mắt 13/13 sau khi ĐÓNG HẲN trình duyệt**: hình `#b98a54→#5a3a26` · danh tính `ps-kiem-go-soi→ps-kiem-go-ocho` · BOQ **68.817.600**. Hiệu chuẩn bằng **gỡ dây thật** (5/13 ↔ 13/13) và **mắt HÌNH xanh cả hai lần** ⇒ bộ đo phân biệt *vẽ ra hình* với *mang danh tính*. Giá không chép vào `MaterialPbr`. ✅ **Mặt tiền thứ năm đã cắm** — `MaterialPalette.tsx:39,90` `tronPickHatGiong` (thêm vào **đúng module** bốn mặt tiền kia dùng, không đẻ engine thứ hai). Máy sạch: `/api/specs` trả `{"specs":[]}` mà bảng chọn 2D có **2 dòng**, tô xong đóng hẳn mở lại còn `specId=hat-giong:f77b3a78-…`. 🔴 **Còn lại**: ① **chưa qua mắt Hoà** ② Đ2 *".idfc nối bằng khoá đổi-được"* đỏ ở bộ tầng-mô-hình (58/59) ③ 🔴 **7,9% mặt vẽ 2D bị hộp TRONG SUỐT nuốt** (439/5546 điểm) — thuộc `components/studio/AppShell.tsx` ④ 🔴 **bấm giữa lòng vùng tô không chọn được** — là **luật chọn hình học** (`lib/cad/query.ts:366` thiếu kiểm điểm-trong-đa-giác), **không** phải lớp che; đo được: lòng ⇒ *"Chưa chọn đối tượng nào"*, biên ⇒ *"1 đối tượng"*. |
