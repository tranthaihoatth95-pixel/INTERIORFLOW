...
| **G4 · DESIGN TRUTH / MOAT** | 🔴 **MỞ LẠI** | 05 ASSET + 01 CORE | Chủ dự án chốt G4 PASS **và cho phép mở lại nếu hồi quy tìm đứt gãy MỚI** — hồi quy G6 đợt 2 tìm ra một cái. 🔴 **Đổi vật liệu ở đường NGƯỜI DÙNG không mang theo danh tính**: `replaceMaterialReferences()` (`lib/materials/impact.ts:117`) có **0 nơi gọi thật** trong `app/` + `components/`; đường UI duy nhất là `applyMaterial` (`lib/cad/store.ts:379`), mà chữ ký của nó **không có `specId`**. Đo trên app thật: hình đổi `ANSI31 → ANSI37`, **danh tính đứng yên `ps-van-soi`**, BOQ vẫn tính theo vật liệu cũ. Vị trí và override **không mất** — thứ mất là **danh tính**, tức đúng chỗ lời hứa *đồng bộ* sống. ⚠️ Lát cắt dọc trước đây PASS ở **tầng lib**; lỗ này chỉ lộ khi **lái qua UI thật** — cùng họ với ca "có trong mã nhưng không tới được người dùng". **Việc kế đáng nhất là chỗ này, KHÔNG phải nhân thêm vật liệu.** |
