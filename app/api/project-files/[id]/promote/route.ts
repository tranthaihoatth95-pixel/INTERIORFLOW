...
async function postHandler(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({
      daCo: kq.daCo,
      // `dungLai: true` = tệp KHÁC trùng nội dung đã có trong Thư viện ⇒ dùng lại vật đó, chỉ gắn
      // thêm nguồn gốc. Vẫn 200: không nhân bản KHÔNG phải lỗi. UI phải nói câu khác `daCo`.
      dungLai: kq.dungLai,
      ...
      // Siêu dữ liệu server TỰ TRÍCH được (null khi không sinh vật mới). `ghiChu` khai thẳng chỗ
      // nào không đo được — đừng để UI hiểu `w: 0` là "ảnh rộng 0".
      meta: kq.meta,
    });
  } catch (e) {
    return loiJson(e, 'POST');
