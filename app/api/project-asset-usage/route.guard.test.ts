...
async function main() {
  ...
  // 🔴 22/08 — GỐC CỦA CÁI FLAKE `npm test` ~1/5 LƯỢT, tái hiện được rồi mới sửa.
  // TRƯỚC: `assert.equal(truoc, sau)` trên count() TOÀN BẢNG. Ý ĐỊNH là *"test này dọn sạch
  // phần của mình"*, nhưng câu lệnh lại khẳng định *"KHÔNG AI trong cả pool ghi vào bảng này"*.
  // `npm test` chạy `xargs -P8`, và `lib/server/promote.test.ts:197` cũng `projectAssetUsage
  // .create(...)` — hai tệp khác thư mục nên không ai để ý chúng dùng CHUNG `prisma/dev.db`.
  // Chạy song song 3 vòng: vòng 2 đỏ (actual 18 ≠ mốc). Không phải khoá SQLite — là ĐẾM CHUNG.
  // NAY: hỏi đúng câu cần hỏi — CÒN SÓT HÀNG NÀO CỦA CHÍNH TEST NÀY KHÔNG. `finally` ở trên đã
  // dọn theo `projectId` nên đây là lưới an toàn thật, và nó miễn nhiễm với tệp chạy song song.
  // `truoc` giữ lại CHỈ để in — đừng khôi phục phép so sánh, nó là cái bẫy vừa gỡ.
  const sau = await prisma.projectAssetUsage.count();
  const sotCuaTest = await prisma.projectAssetUsage.count({
    where: { project: { is: { name: 'Dự án guard' } } },
  });
  assert.equal(sotCuaTest, 0, 'còn sót hàng projectAssetUsage của chính test này — finally dọn hụt');
  ok(`dọn sạch phần của mình: 0 hàng sót (bảng có ${sau} hàng, mốc vào ${truoc} — chỉ để tham khảo, KHÔNG so)`);
