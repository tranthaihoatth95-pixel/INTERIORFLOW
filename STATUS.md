...
## Nợ kỹ thuật
- 🐛 **Presenting, deck IKI VILLAGE chữ tiêu đề rối ("IKDEMOAGE") — ĐÃ ĐIỀU TRA, KHÔNG PHẢI BUG RENDER:** loại trừ cả 3 nghi vấn (layer trùng toạ độ; double-render React; TextFx auto-fit). Root cause thật: field `text` của layer tiêu đề đã HỎNG SẴN trong autosave IndexedDB (`interiorflow-sheets`), verify qua React Fiber (`el.text` = chuỗi hỏng, không phải 2 phần tử đè nhau). Dựng lại deck từ đầu trên nhiều origin sạch → LUÔN đúng, mã nguồn không có lỗi ghi/đè chữ — không tái hiện được thao tác gốc gây hỏng. Nếu tái hiện: gõ lại tiêu đề 1 lần để autosave ghi đè. Nợ phụ (chưa sửa): `TextToolbar.tsx` check sai tên mã lỗi (`NVIDIA_NOT_CONFIGURED` vs route trả `NO_TEXT_PROVIDER`).
