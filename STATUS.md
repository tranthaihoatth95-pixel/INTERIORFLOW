...
- **Sprint 2 (2 đợt, đợt 1 ngắt session-limit → resume)**: cả 3 nhánh XONG:
  ...
  · `feat/ui-motion` (Agent 2, bị user dừng ở bước verify) MERGED code-complete: Liquid Glass + LoginBackdrop + StageTransition (wallpaper/veil nối Header+StudioBar) + Unified Dock + FoldableDualPane. tsc 0 — integrator ĐÃ verify browser trên nhánh gộp (14/07, kết quả bên dưới).
- **✅ CỔNG SPRINT 2 (integrator 14/07, nhánh gộp `feat/present-layout-ml-p1`)**: merge ml-ui (conflict STATUS.md đã hoà gộp 2 bên) + ui-motion (merge sạch) · **20 file test PASS (492 test)** · tsc 0 · **verify browser** (dev PORT=3700, host 127.0.0.1 để KHÔNG đụng cookie phiên thật trên localhost; account admin mới `integrator@ttt.vn` seed bằng scripts/seed-admin.ts):
  1. Login PASS — card Liquid Glass (backdrop `saturate(1.8) blur(40px)`), picker nền 4 preset (Đêm ấm/Mực đêm/Đá ấm/Lụa sáng) + upload đổi sống không crash, Ghi nhớ có, không intro, light+dark OK.
  2. Đăng nhập integrator@ttt.vn → Gallery PASS (register vẫn khoá, seed-admin idempotent chạy ngon).
  3. Unified Dock PASS — `.if-dock` kính mờ (saturate 1.6 blur 14px) đồng nhất Header+StudioBar, đổi CAD↔Render↔Present mượt (StageEnter/Veil mount), 0 lỗi console.
  4. /cad-editor PASS — demo căn hộ 1PN nạp, panel Kiểm chuẩn: detect "residential (59%) — 1 giường → nhà ở", Áp nhóm explainable, checker vẫn bắt đúng Bếp "BẾP" 5.7m² < 10m² (TCVN 4451:2012) cả với bộ rule residential.
  5. /present-editor PASS — GenerateFlow (text → deck) chạy, LayoutShelf hiện 21 card gợi ý nhóm BÌA/BÌA PHỤ/NỘI DUNG CHÍNH với cặp nút Nhận/Bỏ + tooltip "Vì sao hợp"; guardrail layout-check nổi toast "Slide 1: Trống quá 77%"; menu Export mở đủ PDF · PowerPoint (.pptx) · Ảnh PNG (không bấm xuất thật).
  6. Nút "Đưa sang Present →" trên node slide.* — SKIP: nút chỉ hiện trên node slide.* CÓ output đã render; dựng cần chạy flow AI thật (fal.ai, tốn credit) — code đã pass test handoff 11/11, đã verify browser ở phiên Agent 1.
  7. FoldableDualPane PASS — `?dualpane=1` + rộng ≥840: 2 pane (CAD + ReferencePane 1515 ảnh); thu 700px → về single (secondary unmount), canvas + sheet giữ nguyên state; nới lại → dual trở lại. Sheet-persist IDB sống qua reload/route (log `[cad-sheets] IDB ghi 13.6 KB`).
  8. Console PASS — 0 lỗi MỚI; chỉ còn hydration ⌘Z/Ctrl+Z đã biết (nợ kỹ thuật). Network 200 sạch.
...
- **`window.prompt` trong Dashboard** — `components/Dashboard.tsx:138` (nút Dự án mới) dùng `window.prompt` → crash "prompt() is not supported" trong webview/embedded browser (browser thật OK). Code CŨ (không thuộc Sprint 2), integrator phát hiện 14/07 — nên thay bằng inline input/dialog.
