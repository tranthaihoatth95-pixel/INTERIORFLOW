...
## Nợ kỹ thuật
- ✅ **FIX Presenting — chữ tiêu đề chồng/echo (đợt 2, `fix/presenting-text-overlap-v2`):** root cause = sửa chữ TRÊN CANVAS (double-click vào text) mở `<textarea>` nổi đè đúng khung `frame`, nhưng `EditorCanvas.tsx` vẫn vẽ `<Element>` tĩnh (chữ CŨ, chưa commit) của chính element đó bên dưới → 2 lớp gần trùng vị trí/cỡ chữ, lệch vài pixel do khác box-model div/textarea = "echo". Sửa qua Inspector không qua overlay này nên luôn sạch — đúng như user quan sát, chỉ ra chính xác hướng. Sửa: `EditorCanvas.tsx` dòng ~313, bỏ qua vẽ `<Element>` khi `editing?.id === el.id`. Verify: tsc sạch · 64 test pass · browser thật — double-click tiêu đề, gõ chữ mới, không còn ghost; blur commit đúng, quay lại vẽ bình thường 1 lớp.
