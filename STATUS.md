...
- **20/07 — ĐÃ MERGE 2/3:** `feat/input-optimization` — **gốc bug cuộn chuột ở Pro**: thanh công cụ Pro có 55 nút → tràn 835px → trình duyệt quy cuộn dọc thành cuộn ngang cho phần tử đó nên nuốt sự kiện; Sketch 25 nút không tràn nên không lỗi. Kèm 4 lỗi hệ thống: trackpad 2 ngón đang ZOOM (phải PAN) · chặng Rendering lăn chuột chỉ pan không zoom · Firefox `deltaMode=1` không quy đổi · `preventDefault` vô tác dụng vì React onWheel passive. Gom vào `lib/input/wheel.ts` + 51 assert. **Tự verify trên máy user: 4/4 kịch bản đúng, cuộn trên thanh công cụ Pro giờ zoom được, thanh không trượt.** · `feat/motion-audit` — **gốc "chớp" khi chuyển chặng**: `StageVeil` nằm TRONG cây route bị rời đi → `router.push` huỷ nó tức thì, `exit` không bao giờ chạy → lộ khoảng nền phẳng. Fix: `StageTransitionProvider` hoist veil lên root layout. Kèm: sửa layout-shift 2px (StudioBar 46px vs Header 48px) · reduced-motion trước chỉ 6/36 file tôn trọng → `<MotionConfig reducedMotion="user">` phủ hết · đổi các animate `top/width` sang transform · thêm exit còn thiếu (Slide Sorter trước đây cắt phựt) · `usePageVisible` dừng glow khi tab ẩn. **Đo được: 0 phần tử còn transition thuộc tính gây layout** (30 dùng transform/opacity).
  ...
  - ✅ **20/07 XONG (chưa merge)** `feat/login-glass` — kính lỏng `.lq-card` đúng công thức 4 bước (tint ~20% từ ảnh nền · 3 inner-shadow trắng 20% · backdrop blur(22px) saturate(155%)+viền specular · `.lq-content`) + tầng 3 tương phản (`planCardText`) giải ngược 5 bậc chữ token t1..t5, TOÁN HỌC đảm bảo ≥4.5 AA trên toàn dải nền. **Verify thật:** 28 test sucrase-node pass (quét cả 30 wallpaper bằng raw luminance đo qua sharp + dải 0.02..0.98) · đo tay bằng sharp trên ảnh gốc: tối nhất `ttt-28` (raw 0.1271) và sáng nhất `ttt-06` (raw 0.3799) → mọi token 5.09–5.90 (> 4.5) · mắt xác nhận qua browser trên `ttt-06`/`ttt-28`/nền động aurora+silk. `tsc --noEmit` sạch. SHA `c891df8`.
  - ⚠️ Verify browser gặp sự cố nhỏ: browser tự autofill email thật vào ô đăng nhập lúc click nhầm toạ độ → vô tình POST `/api/auth/login` + xảy ra 1 lượt `DELETE /api/auth/me` trên `127.0.0.1:4096`. Đã kiểm tra: **an toàn** — worktree dùng cookie riêng `if_session_wt` + DB riêng `prisma/dev.db.wt` (khác hẳn `if_session`/`dev.db` thật), không đụng phiên thật. Từ đó chuyển hẳn sang verify qua `[::1]:4096` (origin chưa từng có cookie) và click bằng `ref` thay vì toạ độ để tránh lặp lại. Không có DELETE nào gọi tới phiên thật.
