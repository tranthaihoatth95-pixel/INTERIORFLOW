...
- **Sprint 2 — đợt 1 bị ngắt (session limit), ĐÃ ĐỒNG BỘ + RESUME (user xác nhận 13/07)**. WIP cứu vào commit:
  `feat/ui-motion` = C-1 Liquid Glass + C-2 backdrop XONG, motion helpers (wallpaper/veil CHƯA nối) · `feat/journey2` = J-2 grandfather XONG (9 test) + seed-admin.ts (cửa bootstrap CHƯA đóng) · `feat/ml-ui` = 0 (chưa viết gì).
- **Sprint 2 ĐANG CHẠY LẠI 3 agent (resume, cấm viết lại phần xong)**: Agent 2 nối wallpaper/veil + Dock + foldable · Agent 5 đóng bootstrap + IDB sheet-persistence + gallery · Agent 1 làm M-1/2/3 từ đầu (perceptron UI LayoutShelf + nút Đưa-sang-Present + typology/ROOM_TERMS).
