...
## Nợ kỹ thuật
- 🐛 **20/07 — Presenting, deck IKI VILLAGE chữ tiêu đề rối ("IKDEMOAGE") — ĐÃ ĐIỀU TRA, KHÔNG PHẢI BUG RENDER:** loại trừ cả 3 nghi vấn (layer trùng toạ độ trong `templates.ts`; double-render React; TextFx auto-fit — title không set `fx`). Root cause thật: field `text` của layer tiêu đề đã HỎNG SẴN trong autosave IndexedDB (`interiorflow-sheets`), verify qua React Fiber (`el.text` = chuỗi hỏng vd `"IKI jinj"`, không phải 2 phần tử đè nhau). Dựng lại `makeAkhIkiDeck()` từ đầu trên nhiều origin sạch → LUÔN đúng "IKI VILLAGE", ổn định qua nhiều lần reload. Mã nguồn KHÔNG có lỗi ghi/đè chữ — không tái hiện được thao tác gốc gây hỏng (đã thử: gõ dở rồi rời trang, F5 dồn dập, undo/redo, AI "Tạo content"). Không sửa code (không có gì để sửa tại gốc). Nếu tái hiện: gõ lại tiêu đề 1 lần để autosave ghi đè bản đúng. Nợ phụ phát hiện (chưa sửa): `TextToolbar.tsx` check sai tên mã lỗi (`NVIDIA_NOT_CONFIGURED` vs route trả `NO_TEXT_PROVIDER`) nên thông báo "chưa nối NVIDIA" không hiện đúng lúc.
