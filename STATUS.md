...
- **1 worktree đang sống** (1/3 slot): `interiorflow-wt-text-toolbar-ux` nhánh `fix/text-toolbar-clip-and-deselect` (local `ee98e25`) — CHỜ user duyệt merge vào `feat/present-layout-ml-p1`. Nội dung: sửa TextToolbar bị cắt khi textbox sát mép slide (toolbar render ra overlay không bị `overflow:hidden` của stage) + click ra ngoài canvas/Inspector (sidebar trái, header) giờ tự bỏ chọn (global pointerdown passive, không preventDefault) + đổi vật liệu toolbar sang kính mờ tối (backdrop-filter blur 28px + nền đen alpha .62, chữ màu cố định không theo theme) — đã verify browser thật (kéo textbox nhiều vị trí, nền sáng/tối, export/tab-switch vẫn chạy). tsc 0 lỗi, 30 file test PASS. Xoá worktree này sau khi merge.
- Friction UX khác phát hiện thêm lúc sửa (CHƯA sửa, ngoài phạm vi task): chưa thấy gì đáng kể ngoài 2 bug đã giao.
