...
- **Hàng đợi**: ① **AI local Ollama (user GO 15/07 — ĐANG PHÓNG AGENT)** — thêm tầng giữa: Cloud AI → Ollama local → lõi tất định; `lib/ai/providers/ollama.ts` theo pattern nvidia.ts (OpenAI-compat `localhost:11434`), app tự dò có/không (KHÔNG bundle model), model đề xuất Qwen 2.5 7B; CHỈ việc chữ (mô tả/concept/tóm tắt/giải thích quy chuẩn), KHÔNG sinh ảnh local · ② Agent-B đại tu UX canvas chặng 2 · ③ Sprint 3 QA stress · ④ deploy Vercel/Supabase thật (cần user đăng nhập).
