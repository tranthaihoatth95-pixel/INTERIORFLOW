...
- **20/07 — ĐÃ MERGE 2/3:** `feat/input-optimization` — **gốc bug cuộn chuột ở Pro**: thanh công cụ Pro có 55 nút → tràn 835px → trình duyệt quy cuộn dọc thành cuộn ngang cho phần tử đó nên nuốt sự kiện; Sketch 25 nút không tràn nên không lỗi. Kèm 4 lỗi hệ thống: trackpad 2 ngón đang ZOOM (phải PAN) · chặng Rendering lăn chuột chỉ pan không zoom · Firefox `deltaMode=1` không quy đổi · `preventDefault` vô tác dụng vì React onWheel passive. Gom vào `lib/input/wheel.ts` + 51 assert. **Tự verify trên máy user: 4/4 kịch bản đúng, cuộn trên thanh công cụ Pro giờ zoom được, thanh không trượt.** · `feat/motion-audit` — **gốc "chớp" khi chuyển chặng**: `StageVeil` nằm TRONG cây route bị rời đi → `router.push` huỷ nó tức thì, `exit` không bao giờ chạy → lộ khoảng nền phẳng. Fix: `StageTransitionProvider` hoist veil lên root layout. Kèm: sửa layout-shift 2px (StudioBar 46px vs Header 48px) · reduced-motion trước chỉ 6/36 file tôn trọng → `<MotionConfig reducedMotion="user">` phủ hết · đổi các animate `top/width` sang transform · thêm exit còn thiếu (Slide Sorter trước đây cắt phựt) · `usePageVisible` dừng glow khi tab ẩn. **Đo được: 0 phần tử còn transition thuộc tính gây layout** (30 dùng transform/opacity).
  - ⚠️ **CHƯA đo được frame-timing trước/sau** — cả agent lẫn tôi đều gặp `document.visibilityState==='hidden'` ở khung trình duyệt tự động → `requestAnimationFrame` đóng băng, không có gì để đo. Agent KHÔNG bịa số (đúng). **Cần user mở trình duyệt thật xem vài lần chuyển chặng.**
  - ⏳ Còn chạy: `feat/login-glass` (kính lỏng + tương phản card + 5 nền động).
- **20/07 — 2 BÁO CÁO NGHIÊN CỨU ĐÃ MERGE, CHỜ USER QUYẾT:**
  - `docs/RESEARCH-ACCESS-CONTROL.md` — phương án chọn: bảng `ProjectMember{projectId,userId,role}` (5 role owner/crea/drafter/bim/viewer khớp dây chuyền CREA→Hoạ viên→BIM) + `Project.currentStage`/`stageLocked` làm GATE; additive, backfill owner. **🔴 3 LỖ HỔNG AN NINH P0 phát hiện kèm (đã kiểm chéo, THẬT):** ① `/api/comments` KHÔNG auth (cả DELETE?all=1 xoá sạch) ② `/api/dashboard` trả `shareToken` + PII toàn team cho mọi user ③ `/api/cursors` + 5 route AI không auth (ai cũng đốt được balance). §8 có 10 câu hỏi chờ user (nổi bật: Q2 brief NDA ai được đọc · Q5 xác nhận 5 role · Q8 GATE có quay ngược). Ước ~9 ngày công, thứ tự: vá P0 → schema `db push` → backfill → helper `assertProjectAccess` + test quét route → bật lọc.
  - `docs/RESEARCH-MOBILE-DISTRIBUTION.md` — iOS/iPadOS: TestFlight KHÔNG phải kênh vĩnh viễn (build chết 90 ngày); Enterprise bị loại (đòi ≥100 nhân viên); **đường đúng = Custom App qua Apple Business Manager** (private, 99USD/năm) nhưng phải qua App Review 4.2 → cần chất native thật, không bọc WebView trần. **macOS: BỎ Mac App Store** (sandbox chặn spawn Blender + đọc file tuỳ ý) → giữ `.dmg` Developer ID + notarize. **Android: Internal Testing track** (100 tester, 25USD một lần, không cần review). **PWA client ĐÃ CÓ SẴN trong code** (manifest+sw.js+PWARegister, 5-7/07) — STATUS trước ghi thiếu; cái còn thiếu là server HTTPS cố định. Việc chỉ user làm được: mua Apple Dev Program + D-U-N-S (~30 ngày), Play Console 25USD, ABM, privacy policy.
