...
- **✅ 17/07 merge text-toolbar-ux**: TextToolbar hết bị cắt + click-ra-ngoài tự bỏ chọn + kính mờ tối. Panel trái/phải resize (splitter+localStorage)/ẩn-hiện (Photoshop/Canva) + zoom in/out canvas (Ctrl/Cmd+lăn chuột, Ctrl/Cmd+0 Fit). Verify: zoom+panel collapse qua screenshot; chấm màu clamp mới review code (chưa ảnh riêng).
- **✅ 17/07 merge Sprint 4** (`lib/cad/`): Ctrl+C/Ctrl+V copy-paste nội bộ + **Auto-label phòng thật** (`room-autolabel.ts`, đề xuất từ nội thất, chỉ áp khi bấm Apply) + badge Total GFA/đếm phòng. Agent tự sửa 1 bug thật (nhãn "HÀNH LANG" giả quanh khung cửa). Multi-select/Move/Rotate/Mirror đã có sẵn — không làm lại.
- **✅ 17/07 merge Sprint 5**: **Material palette** (`lib/cad/materials.ts`, 13 preset — gạch/gỗ/đá/sơn, swatch CSS/pattern vì CHƯA có ảnh thật, ghi rõ trong code để thay sau) nối vào Hatch. **Circle 3-điểm** (`circumcircle()` dùng chung với Arc 3-điểm cũ, refactor gọn) + **Arc tâm+góc** (`arcFromCenterStartEnd()`) — cả 2 verify đúng bằng toạ độ tính tay. 20 test mới, toàn bộ suite PASS, tsc 0 lỗi.
