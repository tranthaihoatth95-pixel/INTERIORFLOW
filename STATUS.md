...
- **✅ 17/07 PS-2 (nhánh `feat/present-ps2-templates`, CHƯA merge) — "Lưu slide này thành template"**: `custom-templates.ts` (mới, chi tiết → CHANGELOG) + `LayoutShelf.tsx` nút "Lưu mẫu"/mục "Của tôi". tsc 0 · 44 file test (mới 31 ok) · browser 4042 lưu→hiện→reload OK.
