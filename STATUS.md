...
- **p14** (worktree `interiorflow-wt-p14`, nhánh `feat/p14-build-ops-ui`) — 3D: BVH + bắt điểm 7 nấc
  + workPlane + lưới đổi mật độ ĐÃ XONG (`docs/M-3D-NOI-OUT.md`); **VIỆC 4-6 còn treo** (VCB · cụm
  nút + 3 icon sai · nối 11 hàm build-ops). 🔴 `ad2d23b` (Hoà 22:02 07/08) cuốn bản GIỮA CHỪNG —
  phần chốt còn ở working tree.
- **p3c** (worktree `interiorflow-wt-p3c`, nhánh `feat/p3c-bang-kiem`) — khung kiểm `lib/review/` +
  `components/review/` (chốt 07/08 mục 12).
- **p2** (worktree `interiorflow-wt-p2`, nhánh `feat/p2-don-tran-sheet`) — dọn trần "5 sheet" + cắt
  STATUS.md → `docs/M-DON-TRAN-OUT.md`.
- **p13** — gộp Thư viện về `.idfc` v2 (G-M16, chốt 07/08 mục 11): `docs/M-IDFC-2-OUT.md` ·
  `M-IDFC-OUT.md`. VIỆC 4-5 (sidebar theo kind · lỗi thẻ a-e) để phiếu sau; **chờ Hoà quyết kind
  `preset`** cho 5 thumb `light-*`.
...
`main` · `interiorflow-wt-p14` · `interiorflow-wt-p3c` · `interiorflow-wt-p2`.
