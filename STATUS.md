...
- **Sprint 2 — Agent 1 `feat/ml-ui` XONG (chờ integrator)**: M-1 perceptron feedback UI tại LayoutShelf (feature-dict.ts từ điển FEATURE chung 3 chặng + nút Nhận/Bỏ + re-rank khi ≥10 cặp, degrade giữ heuristic, tooltip explainable, lưu localStorage — browser verify: dark-cover 3→1 sau 10 cặp, sống qua reload) · M-2 nút "Đưa sang Present →" trên node slide.* (NodeExtras → stashPresentHandoff → toast "Đã gửi N ảnh" → /present-editor, ảnh vào rổ Reference nhóm render-handoff — browser verify 2 ảnh deck) · M-3 gu-features.ts (occupancy 8×8 + adjacency không-kề-qua-góc + typology 5 nhãn; typology nối classifyOperator ADDITIVE — signal 'layout', không truyền = y cũ) + ROOM_TERMS/subject vào gu.ts (guToPrompt mở đầu "bedroom interior…"). Test 483/19 file pass (cũ 413 + 70 mới: gu-features 27 · feature-dict 25 · gu +9 · operator +9), tsc 0. ⚠️ lưu ý bàn giao: trong lúc test browser lỡ đăng-xuất phiên QA Bot (cookie localhost dùng chung) và flow "Render test" của QA Bot bị autosave ghi đè bằng demo slide — khôi phục từ FlowVersion nếu cần.
