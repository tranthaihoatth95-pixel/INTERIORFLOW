...
- **ĐỢT SAU (chưa giao, cần nghiên cứu trước):** Chat/cộng tác — CAD+Rendering = comment/ghim bất đồng bộ kiểu Miro · Presenting = real-time (đã chốt dùng CRDT/Yjs, ngược tài liệu IF1_IF2_BIGPICTURE cũ). **Báo cáo nghiên cứu xong:** `docs/RESEARCH-TEAM-COLLABORATION.md` (nhánh `feat/research-team-collaboration`) — Phần A (comment CAD+Rendering) rẻ làm ngay; Phần B (real-time Presenting) 🔴 Presenting hiện KHÔNG có server source-of-truth nào cho deck (chỉ IndexedDB client) — cần dựng `Deck` model TRƯỚC khi bàn CRDT.
