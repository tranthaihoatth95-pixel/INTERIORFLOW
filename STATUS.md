...
- **19/07 khuya — ĐỢT MỚI user giao:**
  ...
  - **Đợt 2 — ② Present ĐÃ XONG** (nhánh `feat/present-typography`, CHƯA merge): font tải từ máy (nhúng theo deck + thư viện IDB máy, validate magic-number, cảnh báo giới hạn PPTX) · bảng **Hiệu ứng chữ** 8 preset + tinh chỉnh sâu (viền/chữ rỗng/bóng nhiều lớp/gradient/uốn cung/blend/giãn từ/hoa-thường) áp cả DOM lẫn canvas export · **sửa lỗi thumbnail**: `SlideStrip` xưa vẽ chữ thành thanh 3px `background:currentColor` (placeholder skeleton cố ý, KHÔNG phải lỗi font/scale) → nay dựng bằng chính `Inner` của Element.tsx + `containerType:'size'`. 45 test mới ở `lib/present-editor/text-fx.test.ts`.
  - **Đợt 2 CHƯA giao:** ① Sketch mode = cảm ứng kiểu ArcSite · Pro mode = tối ưu chuột/phím · chuyển 3 chặng mượt + phân định rõ (gộp luôn UI cảm ứng cho F8/F12/gõ lệnh/Space — CAD hiện vẫn cần bàn phím vật lý).
