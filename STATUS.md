...
- **20/07 — ĐÃ MERGE:** `feat/input-optimization` (gốc bug cuộn Pro: toolbar 55 nút tràn 835px nuốt wheel; trackpad nay PAN, pinch/Firefox/passive fix; `lib/input/wheel.ts` + 51 assert — **tự verify trên máy user 4/4 kịch bản**) · `feat/motion-audit` (gốc "chớp" chuyển chặng: `StageVeil` chết cùng route cũ → hoist lên root; reduced-motion 6/36→phủ hết; 0 phần tử còn animate layout). Chi tiết → CHANGELOG.
  ...
  - **20/07 16h — ĐÃ MERGE:** `fix/api-auth-p0` (3 lỗ hổng P0 vá xong: comments/dashboard/cursors+5 route AI đòi đăng nhập; **tự verify bằng curl thật**: 8/8 route 401 không cookie, 200 có cookie, dashboard hết lộ shareToken/email, `/api/share` public không bị vạ lây).
...
- **20/07 — 2 BÁO CÁO NGHIÊN CỨU ĐÃ MERGE, CHỜ USER QUYẾT:**
  ...
  - `docs/RESEARCH-MOBILE-DISTRIBUTION.md` — iOS: Custom App qua Apple Business Manager (TestFlight chết 90 ngày, Enterprise cần ≥100 người); phải qua Review 4.2, không bọc WebView trần. **macOS: BỎ Mac App Store** (sandbox chặn Blender/file) → `.dmg` notarize. **Android (ĐÍNH CHÍNH 20/07 — quy mô KHÔNG giới hạn): Managed Google Play Private App** (toàn tổ chức, không trần user; Internal Testing 100-tester chỉ còn là làn thử). **PWA client ĐÃ CÓ SẴN** (manifest+sw+register) — chỉ thiếu server HTTPS cố định. Việc của user: Apple Dev 99USD + D-U-N-S (~30 ngày), Play Console 25USD, ABM, privacy policy.
- `docs/RESEARCH-COMFYUI-LESS.md` (20/07) — chạy app KHÔNG cần ComfyUI local mà vẫn kế thừa extension: tier 2 "oneAI" mở thành chuỗi **LAN ComfyUI (0đ, ưu tiên 1) → RunPod serverless `worker-comfyui` (CÙNG file workflow JSON, ~$0.01/ảnh, ~$10–18/tháng full tải) → model-API NVIDIA/fal (phủ 4/4 workflow qua FLUX.1-dev base/canny/depth, mất extension)**; failover 2a→2b tự động, tụt 2c thì báo. Dự phòng zero-ops: Replicate any-comfyui-workflow (~$0.021/ảnh, đã kiểm chạy được cả node custom DepthAnythingV2). ComfyDeploy loại (ngừng nhận khách). Công tích hợp ~2.5–3 ngày. **CHỜ USER DUYỆT phương án.**
