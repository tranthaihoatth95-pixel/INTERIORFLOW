...
- **fix/auth-leak-notebook-mismatch** (`8d7246f`): fix Notebook 404 slug↔cuid mismatch (approach C — `resolveNotebookProjectId` accept cả slug/cuid, auto find-or-create bucket ẩn `__nb:<slug>` per-user); defensive filter `NOT { name startsWith '__nb:' }` cho `/api/flows` + `/api/dashboard` project list. Bug 1 auth leak `/api/flows` — NHẦM attribution (code đã filter đúng). 75/75 test.
- **feat/installer-4-platforms-plan** (`7b0a004`): `docs/RESEARCH-INSTALLER-4-PLATFORMS.md` (~500 dòng) + thư mục `installers/` (README/mac-notarize/windows-electron-builder/android-bubblewrap/ios-pwa-vs-capacitor). Windows scaffold sẵn build unsigned; Mac cần Apple Dev $99/năm; Android TWA chưa init cần Google Play $25; iOS MỚI hoàn toàn. Sprint 1 (2-3 ngày, KHÔNG cần credential): Windows unsigned + PWA iOS/Android optimize + LAN QR. Sprint 2 (~1 tuần, chờ credential): Mac notarize + Android TWA + iOS PWA public. 10 câu hỏi §6.
- **feat/library-upgrade-research** (`7619a80`): `docs/RESEARCH-LIBRARY-UPGRADE.md` (~570 dòng). Data thật 1515 asset: tag toàn 2 nhãn cứng lặp (user CHƯA gõ tag thật), 1450 có caption VLM nhưng flatten mất cấu trúc, 0 asset có content PDF. Đề xuất: giữ `LibraryAsset` + thêm 4 cột JSON optional (non-breaking) `{kind, stageUsage[], domain{style,roomType,material[],mood}, projectId, userTags[]}`. Auto-classify khi upload MỚI qua NVIDIA VLM mở rộng prompt. Filter Reference pane theo `stageUsage` khớp `workspace`. M1 schema+upload mới (~1 tuần) · M2 backfill Phase A không tốn AI + hiển thị theo chặng (~3-5 ngày) · M3 batch+description+Vitals assist + Phase B opt-in VLM (~3-5 ngày). 10 câu hỏi §9.
