...
## Nợ kỹ thuật
- 🐛 **MỚI 21/07 — Presenting chữ tiêu đề rối, VẪN CÒN, kết luận cũ SAI:** user tái hiện lại với tiêu đề MỚI "VILLAGE" (không phải "IKI VILLAGE"), field "Nội dung" + layer panel đều hiện chuỗi SẠCH "VILLAGE" — nhưng canvas vẫn vẽ chữ chồng/echo lệch vài pixel (mỗi ký tự như nhân đôi). Bác bỏ kết luận trước ("dữ liệu hỏng sẵn trong IndexedDB") vì dữ liệu lần này rõ ràng sạch. Nghi vấn mới: agent trước chỉ test bằng dựng lại deck object (`akh-sample.ts`), CHƯA test đúng luồng tay thật (chọn template "Trang phân mục" từ panel Mẫu → gõ chữ vào ô Nội dung Inspector) — đây là đường tái hiện CHƯA được thử. Có thể liên quan cơ chế tải font tuỳ chỉnh (`EditorDeck.customFonts`) swap DOM lúc font chưa load xong.
