...
- **Sprint 2 (2 đợt, đợt 1 ngắt session-limit → resume)**: cả 3 nhánh XONG:
  ...
  · `feat/ml-ui` (Agent 1) MERGED: perceptron feedback UI tại LayoutShelf (feature-dict.ts + Nhận/Bỏ + re-rank ≥10 cặp + tooltip explainable) · nút "Đưa sang Present →" trên node slide.* (NodeExtras) · gu-features.ts (occupancy 8×8/adjacency/typology 5 nhãn) additive vào operator · ROOM_TERMS/subject vào gu.ts. 483 test/19 file.
  · `feat/ui-motion` (Agent 2, bị user dừng ở bước verify) MERGED code-complete: Liquid Glass + LoginBackdrop + StageTransition (wallpaper/veil nối Header+StudioBar) + Unified Dock + FoldableDualPane. tsc 0 — ⚠️ CHƯA verify browser, QA flow-check sẽ phủ.
- **⚠️ Sự cố (13/07, ĐÃ SỬA)**: agent verify làm autosave ghi đè graph flow "Render test" trên dev.db chính → restore từ FlowVersion snapshot 11/07, bản-bị-đè giữ thành version 4. BÀI HỌC: browser pane + cookie localhost DÙNG CHUNG → agent verify browser phải TUẦN TỰ, không song song trên DB thật.
