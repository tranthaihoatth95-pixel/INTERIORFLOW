...
(function () {
  ...
  window.__dungDeckIF = async function (userId, projectId) {
    ...
    const slides = [
      ...
      slide([img(F(CHAIR), { frame: { x: 0, y: 0, w: 50, h: 100, rotation: 0 }, href: '/library' }), txt({ text: 'ẢNH → SPEC', fontSize: 1.6, bold: true, tracking: 3, color: MUT, frame: { x: 57, y: 12, w: 34, h: 4, rotation: 0 } }), txt({ text: 'Ghế bành', role: 'title', fontSize: 5.5, bold: true, frame: { x: 57, y: 17, w: 38, h: 9, rotation: 0 } }), txt({ text: 'Rộng  ≈ 1 206 mm\nSâu   ≈ 825 mm\nCao   ≈ 825 mm', fontSize: 3, lineHeight: 1.7, frame: { x: 57, y: 31, w: 36, h: 24, rotation: 0 } }), cap('SUY RA — chờ người xác minh', 57, 58, 34, SOFT), txt({ text: 'Máy đọc ảnh, suy kích thước kèm độ tin.\nNgười xác minh rồi mới thành sự thật.', fontSize: 1.9, color: MUT, lineHeight: 1.55, frame: { x: 57, y: 65, w: 36, h: 11, rotation: 0 } }), live('/library', 57, 78, '→ Mở trong Thư viện')]),
      ...
      slide([meta('BẢNG VẬT THỂ — HERO + SUPPORT'), img(F(CHAIR), { frame: { x: 4, y: 14, w: 34, h: 74, rotation: 0 }, href: '/library' }), cap('GHẾ BAR LINCOLN 327 · AI-SINH', 4, 89, 40),
      ...
      slide([meta('DEMO INDEX — TRANG CỦA NGƯỜI TRÌNH BÀY'),
        ...[['Trang chủ · Resume','/'],['Thiết kế 2D',`${P}/cad`],['Thiết kế 3D',`${P}/render`],['Files','/files'],['Thư viện','/library']].map(([ten, href], i) => { const col = i % 2, row = Math.floor(i / 2); return txt({ text: `→  ${ten}`, fontSize: 3.2, bold: true, color: INKW, href, frame: { x: 6 + col * 46, y: 16 + row * 10, w: 42, h: 7, rotation: 0 } }); }),
