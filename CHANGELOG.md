...
- **Lô duyệt mắt #1** (`docs/duyet-mat/LO-1-2026-08-13.md`): 48 mục xong-MÁY gộp 7 trạm ~20 phút (Trạm 0 = 9 mục nền đề xuất flip thẳng), phản hồi theo trạm, T flip loạt — cơ chế §8.1 lần đầu chạy thật.
- Kết: tsc 0 · frontier 48 xong-MÁY/28 chờ/0 lệch · hinh-hoc 0 · tu-dien 0 · check:chot 0 · npm test 0 fail.

## 13/08/2026 khuya — Đợt T #3: P5 FeatureContract máy + kênh PDF Gói Hồ Sơ + thao-tac 3 luật về 0

- **P5 feature-contract-may** (FC, `09ab5f0`): contract-registry 22 entry đủ 4 câu (20 có-dây · 2 chờ-dây: capture-sequence · lux-l6) + `soi:contract` 3 chiều (regress / mất-dây / sổ-quên), day-pattern import-based né comment. ⭐PHÁT HIỆN: **12/14 kho DOI-CHIEU 08/08 hoá ra ĐÃ MỞ** (eyedropper · vcb · export V-Ray/D5 · brief-PDF · T2-recipe · build-ops union · registry lệnh · accent-warm · GuProfile 2 caller + 4 kho đã biết) — sổ giấy mốc trong 5 ngày, từ nay máy canh dây. Hoà gật lộ trình P5-P6 qua chat ("ok tiếp"); entry `if-rna-v0` (P6) đã mở chờ.
- **present-editor hoàn thiện #1** (PE, `55bf658`): export.ts tách `buildDeckPdfDoc` → `exportDeckToPdfBlob` additive (hàm cũ 0 đổi hành vi, browser-only khai thật) · Toolbar nối kênh PDF vào packHoSoSong fail-open · trả 7 chỗ thao-tac đã nhường (esc-only EditorCanvas · guard SlidePlayer/BoqScreen giữ native-undo ô số · 3 nhãn "tự động") → **3 luật cơ khí soi:thao-tac VỀ 0 toàn app**, còn đúng 2 hàng đợi mắt-design. Gói mẫu có `out/ho-so.pdf` — T mở PDF bằng mắt: hợp lệ.
- Kết: tsc 0 · frontier 50 xong-MÁY/28 chờ/0 lệch · contract 20/2/0 · thao-tac 2 lệch hàng đợi · hinh-hoc 0 · tu-dien 0 · check:chot 0.

## 14/08/2026 — Đợt T #4: chuỗi nền P1→P6 ĐÓNG TRỌN + 2 finding duyệt mắt của Hoà xử tại trận

- **Finding duyệt mắt** (Hoà đi Lô #1): ①nhãn "Về Thư viện dự án" trùng ngữ nghĩa → "Home" (4 chỗ + từ điển cấm, `55c5f55`) ②lag về Home → đo rồi sửa (NL `0b3fed0`: 15 component gated sang dynamic qua heavy-panels.tsx, chunk / −4,4MB dev/−274 module, three-bvh ra hẳn, load 1209→1041ms) — Hoà bấm lại xác nhận hết lag = **mục xong-mắt đầu tiên** (👁 1).
...
## 14/08/2026 — Đợt DF2/DF3: dogfood vòng tròn PDF→deck→render→PDF + ghế 3D từ ảnh + ship map

- **DF2 C4 Westlake — VÒNG TRÒN ĐÓNG TRỌN trên app thật** (DF, 3 job fal): import PDF 477 trang lấy trang chọn → deck phả hệ → ✨ ảnh bếp → mask 6 vùng → phiếu duyệt (giữ đèn ray ✓) → inpaint → "Nhận ảnh đã chỉnh" thay đúng khung → XUẤT PDF 11 trang (scratchpad/DF2-C4-out.pdf). Checklist tick mục 2·3·7. Chuỗi vá trước đó: D1 lớp ảnh bbox thật (81→107 test) · D2 nút ✨ + đường về bán tự động + provenance · DP ảnh ra kho ngoài deck (deck 189MB→0,05MB) + chọn trang. Findings mới F3-F9 vào entry df2-va-findings (nặng nhất: F9 PDF xuất mất lớp chữ, F4 mất slide sau reload, F5 autosave 190MB).
- **DF3 Lincoln 327 — proof ảnh→3D-có-tham-số** (GI, 1 job Trellis 25s): GLB 1,65MB/15.538 tam giác bắt được cả vòng tay vịn đồng; .idfc kind furniture — params VERIFIED per-trường từ spec hãng (T fetch mezzocollection) + phân loại/mesh INFERRED có requestId; task imageTo3d vào models.ts; viewer tạm public/__lincoln-viewer.html Hoà xoay được. = mặt tiền đầu của auto-define.
- **SHIP MAP sống** (npm run ship:map): 90+ task một khung nhìn từ registry, done tick xanh giữ nguyên flow — Hoà đặt, chạy cuối mỗi đợt. Kèm: entry lỗi dwg-dxf-import (Hoà bắt) · nhãn Home + lag (xong-mắt đầu tiên) · DS đợt A (font Times gốc bệnh + radius 334→10) · NC-1/NC-2 (18 NT + khuôn KB + K13-K17, chờ Hoà duyệt) · kịch bản intro Dây-chuyền · DesignSync nối claude.ai/design + project mới.
