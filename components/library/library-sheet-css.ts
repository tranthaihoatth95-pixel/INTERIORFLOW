...
//     Các biến còn lại (--nen-mo-card/--blur-strong/--shadow-sheet/--nen-mo-overlay/--vien-mo/
...
//  Ngoài ra `.nen-mo-sheet` chưa có trong globals.css nên khai ở đây đúng công thức mock
//  (= công thức `.nen-mo-card` sẵn có: nền --nen-mo-card + blur-strong).
...
.if-lib-root .nen-mo-sheet{background:color-mix(in srgb, var(--panel) 97%, transparent)}
.if-lib-root *{box-sizing:border-box}

/* 🔴 SỬA 04/08 (Hoà: "nền sau sheet phải tối xuống rõ"): z-index cũ 20/21 chép từ mock — nhưng
   trong app THẬT .nen-mo-header là z-30 nên header ĐỨNG TRÊN scrim, nửa màn không hề tối (đo bằng
...
.if-lib-root .lib{position:fixed;left:50%;
     ...
     border-radius:var(--radius-lg);border:1px solid var(--vien-mo);
...
.if-lib-root .shelfcta{margin-top:auto;flex:none;position:sticky;bottom:0;z-index:1;
       padding:10px 3px 2px;border-top:1px solid var(--vien-mo);background:var(--panel)}
...
.if-lib-root .chips{flex:none;display:flex;gap:5px;padding:9px 14px;border-bottom:1px solid var(--vien-mo);overflow-x:auto;scrollbar-width:none}
...
.if-lib-root .library-spotlight .th{height:auto;border:0;border-right:1px solid var(--vien-mo)}
...
.if-lib-root .libft{flex:none;height:38px;display:flex;align-items:center;gap:10px;padding:0 14px;
       border-top:1px solid var(--vien-mo);font-size:var(--fs-2xs);color:var(--t4)}
...
.if-lib-root .speccol .sphead{padding:14px 12px 12px;border-bottom:1px solid var(--vien-mo)}
...
.if-lib-root .speccol .spsec{padding:12px;border-bottom:1px solid var(--vien-mo)}
...
.if-lib-root .speccol .splink{padding:10px 12px;display:flex;flex-wrap:wrap;align-items:center;gap:6px;
       font-size:var(--fs-2xs);line-height:1.5;color:var(--t3);border-bottom:1px solid var(--vien-mo)}
...
.if-lib-root .droprow{display:grid;grid-template-columns:16px 1fr 74px 62px 22px;align-items:center;gap:8px;
      padding:7px 10px;font-size:var(--fs-2xs);color:var(--t2);border-bottom:1px solid var(--vien-mo)}
...
.if-lib-root .ingestft{flex:none;height:38px;display:flex;align-items:center;gap:10px;
      border-top:1px solid var(--vien-mo);margin-top:10px;font-size:var(--fs-2xs);color:var(--t4)}
