...
export function MauVatLieuLon({
  ...
  /* Tấm vân sinh THEO NẤC, không phải một tấm 96 px kéo giãn cho cả ba. `nenVanNac` đã kẹp
     cạnh tấm trong [96, 384] nên không có đường nào sinh ra một vòng lặp triệu pixel. */
  const van = useMemo(
    () => (xemTruoc?.def && nen ? materialTextureDataUrl(xemTruoc.def, nen.canhTile) : ''),
    [xemTruoc?.def, nen?.canhTile],
  );
    ...
    <div ref={khungRef}>
      ...
      <dl style={{ margin: 0, marginTop: 8, display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 4, fontSize: 12, lineHeight: 1.5 }}>
        ...
        <Dong nhan={tr('Ảnh chất liệu', 'Texture maps')}>
          {soMap > 0
            ? tr(`${soMap} trên ${KHOA_MAP.length} map`, `${soMap} of ${KHOA_MAP.length} maps`)
