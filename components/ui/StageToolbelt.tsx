  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
      ...
      <ToolbarBar>
        ...
        <ToolbarChip
          icon={<CommandIcon name="ImagePlus" size={17} />}
        ...
        {chips.map((c) => (
          <ToolbarChip
            key={c.id}
            icon={<CommandIcon name={c.icon} size={17} />}
            label={c.label[0]}
            desc={c.desc[0]}
            disabled={!c.enabled}
            disabledReason={c.disabledReason}
            // Chỉ `visual-generate` có tay thi hành ở lượt này. Các năng lực khác đã MỜ kèm lý do
            // thật ở `workingSetChips` — không gắn `onClick` giả cho chúng (§9 cấm nút giả).
            onClick={c.id === 'visual-generate' ? () => setMo(true) : undefined}
          />
        ))}
      </ToolbarBar>
