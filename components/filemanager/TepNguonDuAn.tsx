        ...
        <div style={{ ...the, display: 'grid', gap: 8 }}>
          ...
          {asset && !dungO && (
            <NutLyDo lyDo={null} idLyDo={`${idNhan}-lydo-dungo`} onClick={xemDungO} testid="tepnguon-tra-dungo">
              <Link2 size={18} strokeWidth={1.8} aria-hidden />{tr('Tra quan hệ sử dụng', 'Look up usage')}
            </NutLyDo>
          )}
          {dungO?.dangTai && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ marginLeft: 'auto', display: 'inline-flex', gap: 8 }}>
          <button type="button" style={nutNho} onClick={() => void napTep()} data-testid="tepnguon-reload">
            <RefreshCw size={18} strokeWidth={1.8} aria-hidden />{tr('Đọc lại', 'Reload')}
          </button>
          <NutLyDo
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {!c.dangTaiLen && (
                        <button type="button" aria-label={tr('Bỏ lượt thử này', 'Dismiss this attempt')}
                          ...
                          <X size={18} strokeWidth={1.8} aria-hidden />{tr('Bỏ qua', 'Dismiss')}
                        </button>
                      )}
                    </div>
                  ...
                  <li key={tep.id} data-testid={`tepnguon-row-${tep.id}`}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      ...
                      <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 'var(--fs-2xs)', color: 'var(--t2)', cursor: 'pointer' }}>
                        ...
                        <Eye size={14} strokeWidth={1.8} aria-hidden />{tr('Đã xem', 'Reviewed')}
                      </label>
                      ...
                      >
                        <Library size={18} strokeWidth={1.8} aria-hidden />
                      ...
                      >
                        <Library size={18} strokeWidth={1.8} aria-hidden />{tr('Mở tài sản trong Thư viện', 'Open asset in Library')}
                      </NutLyDo>
                      ...
                      >
                        <Link2 size={18} strokeWidth={1.8} aria-hidden />{tr('Đang dùng ở đâu', 'Where used')}
                      </NutLyDo>

                      <button type="button" style={{ ...nutNho, marginLeft: 'auto' }} data-testid={`tepnguon-xoa-${tep.id}`}
                        onClick={() => void xoa(tep)}>
                        <Trash2 size={18} strokeWidth={1.8} aria-hidden />{tr('Xoá', 'Delete')}
                      </button>
                    </div>
