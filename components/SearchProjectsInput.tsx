...
export default function SearchProjectsInput() {
  ...
  /* Trạng thái THỊ GIÁC, tách hẳn khỏi `expanded`. Hai thứ khác nhau:
     `expanded` = ở màn hẹp, ô ĐÃ BUNG ra (và kéo theo tự-focus).
     `reVao`    = con trỏ đang ở trên, CHỈ để vẽ nền/viền — không đụng tiêu điểm.
     Trộn hai thứ này là lỗi tôi vừa tự tạo rồi tự bắt: dùng `expanded` cho hover thì trên
     màn rộng rê một lần là ô dính viền vĩnh viễn (không có đường về), còn trên màn hẹp thì
     rê chuột sẽ cướp tiêu điểm bàn phím. */
  ...
  /* 🔴 26/08 — NGHỈ THÌ TAN, BUNG KHI CÓ Ý ĐỊNH.
     Bản cũ luôn là một Ô: nền + viền, thường trực ở thanh trên. Hoà soi app thật ngày 26/08:
     thanh trên phải **gần như tan vào môi trường** khi ít ngữ cảnh — một ô có viền đứng đó cả
     ngày là một vật đòi chú ý mà chưa ai gọi tới.
     ⭐ Đây KHÔNG phải thu nhỏ cho gọn. Khác biệt là ở NGHĨA: ô có viền nói *"đây là một trường
     nhập"*; chữ trần nói *"gõ được nếu anh muốn"*. Thứ hai đúng với vai của nó trên Trang chủ —
     nơi tìm kiếm là lối phụ, không phải việc chính.
     Nền + viền chỉ hiện khi CÓ Ý ĐỊNH: con trỏ rê tới, tiêu điểm bàn phím, hoặc đã gõ chữ.
     `focus-within` lo bàn phím, nên lối phím không mất gì. */
  const coYDinh = reVao || expanded || query.length > 0;

  return (
    <div
      className="group flex shrink-0 items-center gap-2 rounded-full px-3 py-1.5 transition-colors duration-[var(--nhip-bam)]"
      style={{
        background: coYDinh ? 'var(--nen-mo-header, var(--panel))' : 'transparent',
        border: `1px solid ${coYDinh ? 'var(--border)' : 'transparent'}`,
