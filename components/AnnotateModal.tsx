...
export function AnnotateModal() {
  ...
  // vẽ/chú thích). 2.2.90 ĐỢT 3 — chuyển sang useDismissable dùng chung; `guard` tái hiện đúng
  // logic 2 nấc cũ (chặn đóng + bật cảnh báo ở nấc 1, để lọt ở nấc 2). Bản gốc KHÔNG có
  // bấm-ra-ngoài-để-đóng (chỉ Escape/nút X) — giữ nguyên bằng `outside: false`.
  useDismissable({
    open: !!nodeId,
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-[var(--border)] px-4 py-3">
          ...
          {tool === 'text' && (
            <input
              ...
              className="w-52 rounded-[10px] border border-[var(--border)] bg-[var(--field)] px-2 py-1.5 text-xs text-[var(--t1)] focus:border-[var(--focus-ring)]"
            />
          )}
