 ...
 *
 * File này KHÔNG import package `@mlightcad/libredwg-web` (GPL-3.0/GPL-2.0) — việc đó CHỈ xảy
 * ra bên trong lib/cad/dwg-worker.ts (Web Worker riêng, xem đầu file đó để biết lý do cô lập).
 * Ở đây ta chỉ nhận dữ liệu ĐÃ PARSE THÔ (JSON thuần, hình dạng DwgRawDoc — định nghĩa LẶP LẠI
 * cục bộ bên dưới, cố tình KHÔNG import type từ dwg-worker.ts để giữ ranh giới 2 file tách biệt
 * hoàn toàn, không phụ thuộc lẫn nhau ngoài "hợp đồng" JSON qua postMessage) rồi map sang
 ...
 *
 * ⚠️ Biện pháp cô lập worker ở trên là GIẢM THIỂU RỦI RO theo khuyến nghị cộng đồng, KHÔNG PHẢI
 * bảo đảm pháp lý. Xem docs/LICENSE-NOTES.md trước khi dùng tính năng này cho khách hàng thật.
 */

import type { Doc, Entity, HatchPattern, Layer, Pt } from './model';
import { aciToHex, dxfNameToLineType } from './dxf';

/* ───────────────────────── hợp đồng JSON với worker (LẶP LẠI có chủ đích — xem đầu file) ── */
