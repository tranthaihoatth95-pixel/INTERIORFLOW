 ...
 * click / trượt ngang = chuyển chặng như cũ · kéo XUỐNG vượt ngưỡng = gọi Vitas.
 * Chạy: node_modules/.bin/sucrase-node lib/input/stage-drop.test.ts
 */
import {
  createStageDragTracker,
  VITAS_DROP_THRESHOLD_PX,
  DRAG_SLOP_PX,
} from './stage-drop';
...
console.log('Kéo XUỐNG vượt ngưỡng → vitas');
{
  const t = createStageDragTracker();
  ...
  ok(`${VITAS_DROP_THRESHOLD_PX}px xuống → vitas`, t.move(2, VITAS_DROP_THRESHOLD_PX) === 'vitas');
  ok('progress = 1 khi bắn', t.progress() === 1);
  ok('sau khi bắn giữ nguyên vitas', t.move(0, 50) === 'vitas');
}

console.log('Trượt NGANG (hành vi thanh chặng cũ) → locked, không bao giờ thành vitas');
{
  const t = createStageDragTracker();
...
{
  const t = createStageDragTracker();
  const d = VITAS_DROP_THRESHOLD_PX + 4;
  const v = t.move(d, d); // 45°: dy không > dx*1.2
  ok('chéo 45° vượt ngưỡng → locked hoặc pending, KHÔNG vitas', v !== 'vitas');
}

console.log('Kéo chéo chủ yếu DỌC (dy thắng rõ) → vẫn vitas');
{
  const t = createStageDragTracker();
  ok('dx=8, dy=30 → vitas', t.move(8, 30) === 'vitas');
}
