...
- **CÒN LẠI (autonomous phiên sau)**: restyle nốt Apple (InteriorNode/MaskPainter/Lightbox/BottomToolbar/FlowCanvas); **làm lại responsive header + panel bottom-sheet trên nền Apple** (đã hoãn khi merge pwa — header đang chật, tên flow bị co); (tuỳ chọn) wire empty-state canvas nạp starter theo chặng (`phase.demo`).
